...
type Phase = 'sanctuary' | 'story' | 'question' | 'reflection';

export default function OnboardScreen({ navigation }: Props) {
  const completeOnboarding = useAppStore(s => s.completeOnboarding);

  const [phase,       setPhase]       = useState<Phase>('sanctuary');
  ...
  function handleEnter() {
    if (selectedKey === 'E') {
      completeOnboarding('E', freeText, undefined);
    } else {
      const choice = story.choices.find(c => c.key === selectedKey) ?? story.choices[0];
      completeOnboarding(choice.key, undefined, choice.feedTag);
    }
  ...
  if (phase === 'sanctuary') {
    return (
      <View style={styles.fullScreen}>
        ...
        <Animated.View style={[styles.sanctuaryInner, animStyle]}>
          <Text style={styles.sanctuaryHeadline}>He Is Risen.</Text>
          <Text style={styles.sanctuaryBody}>
            Every piece of genuine peace, beauty, or hope you have ever felt has a source.
          </Text>
          <TouchableOpacity
          ...
          >
            <Text style={styles.primaryBtnText}>Step inside →</Text>
          </TouchableOpacity>
        </Animated.View>
      </View>
    );
...
const styles = StyleSheet.create({
  ...
  primaryBtn: {
    alignSelf: 'flex-start', borderWidth: 1, borderColor: colors.gold,
    borderRadius: radius.sm, paddingVertical: 14, paddingHorizontal: 36,
    marginTop: spacing.xl,
  },
  primaryBtnDisabled: { borderColor: colors.borderDim },
  primaryBtnText: {
    fontSize: 15, fontFamily: 'Georgia', color: colors.gold, letterSpacing: 0.5,
  },
