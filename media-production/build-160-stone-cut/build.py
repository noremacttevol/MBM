#!/usr/bin/env python3
"""Assemble Story Video #161 — Called of God, as was Aaron (Hebrews 5:1-6).
MEMBER shelf verse-video → Gospel Library topic: Priesthood (on the closing card).
...
Face-never (the #1 Law): Christ is never rendered anywhere — he is referenced by the
narrator only; where the text speaks of him (kv5) the picture is light and the veil,
empty of any figure (gate PASS).

TWO SACRED SILENCES (the music bed dies to true silence for both):
  kv4  Heb 5:4   "no man taketh this honour unto himself" — the verse card
  kv5  Heb 5:5   "so also Christ glorified not himself" — even Christ was called
...
Output: hebrews-5_called-of-god.mp4, 1080x1920 H.264 30fps, <30MB.
"""
...
S1 = "s1-daniel-before-the-king.jpeg"
S2 = "s2-the-great-image.jpeg"
S3 = "s3-the-stone-cut-without-hands.jpeg"
S4 = "s4-the-stone-strikes.jpeg"
S5 = "s5-the-image-shatters.jpeg"
S6 = "s6-the-stone-becomes-a-mountain.jpeg"
S7 = "s7-fills-the-whole-earth.jpeg"
S8 = "s8-a-kingdom-forever.jpeg"
...
GL_POINTER = "Learn more — Gospel Library: Kingdom of God"

BEATS = [
    ("n1", S1, "in"),
    ("n2", S2, "in"),
    ("n3", S3, "in"),
    ("kv45", S4, "in"),                      # SACRED SILENCE 1 — the stone cut without hands (Dan 2:45)
    ("n4", S4, "out"),
    ("n5", S5, "in"),
    ("n6", S6, "in"),
    ("kv44", S7, "in"),                      # SACRED SILENCE 2 — a kingdom that shall stand for ever (Dan 2:44, named)
    ("n7", S7, "out"),
    ("n8", S8, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (stone cut without hands, Dan 2:45): kv45 at {start_of['kv45']:.1f}s",
          flush=True)
    print(f"sacred silence 2 (a kingdom for ever, Dan 2:44): kv44 at {start_of['kv44']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["kv45"] + spoken["kv45"]
    p2_end = start_of["kv44"] + spoken["kv44"]
    beds = [
        (0.0, start_of["kv45"] - 1.2, "b"),
        (p1_end + 1.0, start_of["kv44"] - 1.0, "a"),
    ...
    OUT = "daniel-2_stone-cut-without-hands.mp4"
    A_KBPS, MUX = 96, 20
