#!/usr/bin/env python3
"""Assemble MEMBER Verse Video #180 — Before I Formed Thee in the Belly I
Knew Thee (Jeremiah 1:5-8).
...
CONTENT-CARE: clean — calling and reassurance, no fear. Jesus does not
appear (Jeremiah; God calls the young prophet). No divine figure.

TWO SACRED SILENCES: s1 — Jeremiah 1:5 (before I formed thee, the centerpiece)
and s2 — 1:8 (be not afraid, I am with thee), scripture voice, cream italic.
n2 gets the draft's extended hold before the s1 verse. n3 recorded in two
halves so the eight beats fill the eight stills; stills matched semantically.
...
paths in segs/. Output: jeremiah-1_before-i-formed-thee.mp4,
...
ST1 = "s1-too-small-for-the-job.jpeg"
ST2 = "s2-only-a-child.jpeg"
ST3 = "s3-the-presence-settles.jpeg"
ST4 = "s4-a-calling-older-than-his-life.jpeg"
ST5 = "s5-known-before-born.jpeg"
ST6 = "s6-resolve-beginning.jpeg"
ST7 = "s7-the-road-opens.jpeg"
ST8 = "s8-the-child-who-became-a-prophet.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1", "s2"}

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Foreordination"

BEATS = [
    ("n0", ST1, "in"),           # too small for the job
    ("n1", ST4, "in"),           # a calling older than his life
    ("n2", ST3, "in"),           # already chosen, blessing — the presence settles
    ("s1", ST5, "in"),           # SACRED SILENCE 1 — Jer 1:5, known before born
    ("n3a", ST2, "in"),          # Jeremiah: only a child
    ("n3b", ST7, "in"),          # the LORD: go, do not fear — the road opens
    ("s2", ST6, "in"),           # SACRED SILENCE 2 — Jer 1:8, be not afraid — resolve
    ("n4", ST8, "in"),           # He walks with you — the child who became a prophet
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap override: n2 = the draft's sacred-silence beat before Jer 1:5.
HOLD = {"n2": 1.60}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silences: s1 (Jer 1:5) at {start_of['s1']:.1f}s, s2 (1:8) at {start_of['s2']:.1f}s", flush=True)
    ...
    OUT = "jeremiah-1_before-i-formed-thee.mp4"
    A_KBPS = 96
    MUX = 20
