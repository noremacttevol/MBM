...
Output: matthew-5_the-beatitudes.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S2 = "s2-blessed-are-the-poor-in-spirit.jpeg"
S3 = "s3-they-that-mourn.jpeg"
S4 = "s4-the-meek.jpeg"
S5 = "s5-hunger-for-righteousness.jpeg"
S6 = "s6-the-merciful.jpeg"
S7 = "s7-the-pure-in-heart.jpeg"
S8 = "s8-the-peacemakers.jpeg"
S9 = "s9-the-persecuted.jpeg"
S10 = "s10-the-upside-down-kingdom.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"jv3", "jv456", "jv78", "jv910"}

# BEATS: (segment_name, still, zoom_dir). Zoom alternates in/out on a shared still.
BEATS = [
    ("n1", S1, "in"),
    ("jv3", S2, "in"),          # poor in spirit — sacred silence 1
    ("n2", S3, "in"),
    ("jv456", S4, "in"),        # mourn, meek, hunger
    ("n3", S5, "in"),
    ("jv78", S6, "in"),         # merciful, pure in heart
    ("n4", S7, "in"),
    ("jv910", S8, "in"),        # peacemakers, persecuted — sacred silence 2
    ("n5", S9, "in"),
    ("n6", S10, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (poor in spirit): jv3 at {start_of['jv3']:.1f}s", flush=True)
    print(f"sacred silence 2 (peacemakers): jv910 at {start_of['jv910']:.1f}s", flush=True)
    ...
    p1_end = start_of["jv3"] + spoken["jv3"]
    p2_end = start_of["jv910"] + spoken["jv910"]
    beds = [
        (0.0, start_of["jv3"] - 1.2, "b"),
        (p1_end + 1.0, start_of["jv910"] - 1.0, "a"),
        (p2_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out at {start_of['jv3'] - 1.2:.1f}s, silent through jv3 "
          f"(ends {p1_end:.1f}s); returns; silent again through jv910 "
    ...
    OUT = "matthew-5_the-beatitudes.mp4"
    A_KBPS = 96
    MUX = 20
