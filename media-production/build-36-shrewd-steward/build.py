#!/usr/bin/env python3
"""Assemble Story Video #36 — The Shrewd Steward (Luke 16:1-13).
...
Face-never (#18): this is a parable told from off-screen — the rich owner, the
steward, and the debtors are all shown fully; there is NO figure of the Lord in
any still.

Milk framing: the Lord praises the manager's URGENCY, not his dishonesty — be
that urgent about eternal things. And the good news: he is not after your money,
he is after you.

Timing is COMPUTED from measured mp3 durations (no hand offsets to drift).
Music: a warm detuned-pair bed opens and fades to FULL SILENCE just before j2
("...that they may receive you into everlasting habitations" — the heart line,
the peak), so it lands in true silence; a warm bed returns after and is out
before the closing card. No-Dead-Air: the only silences >2.5s are the hush
around j2 and the card's tail.

Output: luke-16_shrewd-steward.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-accused.jpeg"
S2 = "s2-worried.jpeg"
S3 = "s3-called-in.jpeg"
S4 = "s4-oil-bill.jpeg"
S5 = "s5-wheat-bill.jpeg"
S6 = "s6-commended.jpeg"
S7 = "s7-welcomed.jpeg"
S8 = "s8-two-masters.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}
PEAK = "j2"   # "...everlasting habitations" — music goes to silence here
...
def main():
    ...
    beds = [
        (0.0, peak_start - 1.2, "b"),                     # out just before peak
        (peak_start + audio_dur[PEAK] + 1.0, card_start - 0.8, "a"),  # returns
    ]
    ...
    OUT = "luke-16_shrewd-steward.mp4"
