#!/usr/bin/env python3
"""Assemble Video #59 — Feeding the Four Thousand (Mark 8:1-9).
...
generation). He appears in s1-s6, s8, s9; s7 (the seven baskets) has no divine figure.
Captions are VERBATIM spoken text from make_narration.SEGMENTS. TWO sacred silences (music
bed dies): the red-letter compassion line jv2 (cream italic) and the blessing of the loaves
(nbless, narrator-told).

CARE FLAGS: none — GREEN. A tender miracle of compassion; the fish are simple food, never
gory. Distinct from #58: three days, wilderness, seven loaves, seven baskets, four thousand.
...
Output: mark-8_feeding-4000.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <30MB.
"""
import os
import shutil
import subprocess
...
S1 = "s1-three-days-with-him.jpeg"
S2 = "s2-i-have-compassion.jpeg"
S3 = "s3-in-the-wilderness.jpeg"
S4 = "s4-seven-loaves.jpeg"
S5 = "s5-he-gave-thanks.jpeg"
S6 = "s6-all-were-filled.jpeg"
S7 = "s7-seven-baskets.jpeg"
S8 = "s8-four-thousand.jpeg"
S9 = "s9-who-he-is.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
# jv2 is the one red-letter line (cream italic). The blessing beat is narrator text but
...
BEATS = [
    ("n1", S1, "in"),
    ("jv2", S2, "in"),                          # I HAVE COMPASSION — full silence 1
    ("n2", S3, "in"),
    ("n3", S4, "in"),
    ("nbless", S5, "in"),                       # GAVE THANKS AND BRAKE — full silence 2
    ("n4", S6, "in"),
    ("n5", S7, "in"),
    ("n6", S8, "in"),
    ("n7", S9, "in"),
...
def main():
    ...
    print(f"sacred silence 1 (compassion): jv2 at {start_of['jv2']:.1f}s", flush=True)
    print(f"sacred silence 2 (gave thanks): nbless at {start_of['nbless']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["jv2"] + spoken["jv2"]
    p2_end = start_of["nbless"] + spoken["nbless"]
    beds = [
        (0.0, start_of["jv2"] - 1.2, "b"),
        (p1_end + 1.0, start_of["nbless"] - 1.0, "a"),
        (p2_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out before jv2 (ends {p1_end:.1f}s); returns; silent again "
          f"through the blessing (ends {p2_end:.1f}s)", flush=True)
