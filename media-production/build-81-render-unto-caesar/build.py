#!/usr/bin/env python3
"""Assemble Story Video #81 — Render Unto Caesar (Mark 12:13-17).
...
(Mark 12:15b, 12:16, 12:17 — exact KJV; the officials' "Caesar's" is paraphrased).
...
j1 "why tempt ye me / bring me a penny" [SILENCE] on s4; j2 "whose image" [SILENCE]
on s6, n4 (the trap swings shut) on s6-out; j3 "render to Caesar" [SILENCE] on s7
(its exact match), n5 (you bear God's image) on s8 (two images). No HUSH — n5 closes
straight into the card.

Caption box black@0.55 — warm interior lamplight, verified on the lightest frame.

Output: mark-12_render-unto-caesar.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-flatterers.jpeg"
S2 = "s2-the-mask.jpeg"
S3 = "s3-he-saw-through-it.jpeg"
S4 = "s4-bring-me-a-penny.jpeg"
S5 = "s5-the-image-on-the-coin.jpeg"
S6 = "s6-whose-image.jpeg"
S7 = "s7-the-answer.jpeg"
S8 = "s8-two-images.jpeg"
...
BEATS = [
    ("n0", [(S1, "in")], None),    # officials came with a trap, flattering first
    ("n1", [(S2, "in")], None),    # is it lawful — no safe answer
    ("n2", [(S3, "in")], None),    # Jesus saw straight through it
    ("j1", [(S4, "in")], None),    # "why tempt ye me? bring me a penny" [SILENCE]
    ("n3", [(S5, "in")], None),    # a small silver coin, held up
    ("j2", [(S6, "in")], None),    # "whose is this image..." [SILENCE]
    ("n4", [(S6, "out")], None),   # they said Caesar's — the trap swung shut
    ("j3", [(S7, "in")], None),    # "render to Caesar / to God" [SILENCE]
    ("n5", [(S8, "in")], None),    # you bear God's image — what you owe
]
...
def main():
    ...
    OUT = "mark-12_render-unto-caesar.mp4"
    A_KBPS, MUX = 96, 20
