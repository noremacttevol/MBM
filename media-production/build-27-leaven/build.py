#!/usr/bin/env python3
"""Assemble Story Video #27 — The Leaven (Matthew 13:33).
...
segment (imported from make_narration.SEGMENTS and word-wrapped). The KJV
(Jesus) line renders in cream italic.

Face-never (#18): this is a one-verse parable told from off-screen — there is NO
Jesus figure in any still; the whole picture is an ordinary woman baking bread.

Timing is COMPUTED from the measured mp3 durations (no hand offsets to drift).
Music: a warm detuned-pair bed opens and fades to FULL SILENCE just before n6
(the risen dough — the transformation, the emotional peak), so it lands in true
silence; a warm bed returns after and is out before the closing card.

Output: matthew-13_leaven.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S5 = "s5-risen.jpeg"
S6 = "s6-baked-bread.jpeg"
S7 = "s7-bread-to-table.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}
PEAK = "n6"   # the risen/transformed dough — music goes to silence around this
...
def main():
    ...
    for name, still, zdir in BEATS:
        reverent = name in KJV or name == PEAK
