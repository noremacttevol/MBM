#!/usr/bin/env python3
"""Assemble Video #57 — Jairus's Daughter (Mark 5:21-24, 35-43).

PHASE-1 STILLS-ONLY build (Law E): nine painted stills with slow Ken Burns drift,
narration, serif captions (CAPTION v2), cream-italic KJV lines, closing invitation card.
NO AI motion clips.

FACE LAW v3: Jesus's face IS shown, locked to JESUS-MASTER-REF (attached as --ref at
generation). He appears in s1, s2, s4-s9; s3 (the messengers) has no divine figure.
Captions are VERBATIM spoken text from make_narration.SEGMENTS; his two red-letter lines
(jv36 "Be not afraid, only believe"; jv41 "Damsel...arise") render in cream italic and are
the two sacred silences.

CARE FLAGS: R (RESTRAINT). The dead child is shown with dignity — peaceful as if asleep,
never morbid, no gore. Hope-beat is the girl up and walking (s8), and the tender "give her
something to eat."
...
Output: mark-5_jairus-daughter.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <30MB.
"""
import os
import shutil
import subprocess
...
S1 = "s1-jairus-at-his-feet.jpeg"
S2 = "s2-on-the-way.jpeg"
S3 = "s3-the-worst-news.jpeg"
S4 = "s4-only-believe.jpeg"
S5 = "s5-not-dead-but-sleeping.jpeg"
S6 = "s6-into-the-room.jpeg"
S7 = "s7-talitha-cumi.jpeg"
S8 = "s8-she-arose.jpeg"
S9 = "s9-give-her-to-eat.jpeg"
...
BEATS = [
    ...
    ("n2", S2, "in"),
    ("n3", S3, "in"),
    ("jv36", S4, "in"),                         # BE NOT AFRAID, ONLY BELIEVE — silence 1
    ("n4", S5, "in"),
    ("n5", S6, "in"),
    ("jv41", S7, "in"),                         # DAMSEL, ARISE — silence 2
    ("n6", S8, "in"),
    ("n7", S9, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (only believe): jv36 at {start_of['jv36']:.1f}s", flush=True)
    print(f"sacred silence 2 (damsel arise): jv41 at {start_of['jv41']:.1f}s", flush=True)
    ...
    p1_end = start_of["jv36"] + spoken["jv36"]
    p2_end = start_of["jv41"] + spoken["jv41"]
    beds = [
        (0.0, start_of["jv36"] - 1.2, "b"),
        (p1_end + 1.0, start_of["jv41"] - 1.0, "a"),
        (p2_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out before jv36 (ends {p1_end:.1f}s); returns; silent again "
          f"through jv41 (ends {p2_end:.1f}s)", flush=True)
    ...
    OUT = "mark-5_jairus-daughter.mp4"
    A_KBPS = 96
    MUX = 20
