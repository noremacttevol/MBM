#!/usr/bin/env python3
"""Assemble Story Video #125 — "I Never Knew You" (Matthew 7:21-23).

PHASE-1 STILLS-ONLY (Law E): all 7 stills cut in as beats (no separate breath
still), Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW §5),
cream card.

CARE-J (judgment held with mercy): the solemn line is grave but the face is
tender, never angry; mercy is spoken (n3 + card are an invitation, no
fear-question). Face-shown Jesus locked to master (s2/s5/s6/s7), only-Jesus-in-
cream held. Two-voice: narrator en-US-AndrewNeural; Jesus
en-US-ChristopherNeural exact KJV Matt 7:21 (j1), 7:22 (j2), 7:23 (j3).
Ear-check pass (j1 tie-break on medium.en).

Each image matched to its words: j2 (many will say... works) → recounting the
deeds (s4); n2 (knowing him shown by love) → the clasped hands over the cup
(s6); j3 (I never knew you) → the solemn line (s5, the pack's marked SILENCE).

SACRED SILENCES: the marked s3 hold after n1 (the listener) and the marked s5
hold on j3. KJV holds on all three lines.

NO MUSIC BED (HUM PURGE law). Bright-day: caption box black@0.60.

Output: matthew-7_i-never-knew-you.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-teaching-on-the-mountainside.jpeg"
ST2 = "s2-the-warning.jpeg"
ST3 = "s3-a-listener-thinks-hard.jpeg"
ST4 = "s4-recounting-the-deeds.jpeg"
ST5 = "s5-the-solemn-line.jpeg"
ST6 = "s6-knowing-shown-by-love.jpeg"
ST7 = "s7-the-door-is-knowing.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2", "j3"}   # Matt 7:21/22/23, Jesus voice — cream italic
# Long holds: all three KJV lines + the marked s3 silence after n1.
LONG_HOLD = {"j1", "j2", "j3", "n1"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0", ST1, "in"),     # teaching on the mountainside
    ("j1", ST2, "in"),     # KJV Matt 7:21 — the warning
    ("n1", ST3, "in"),     # a listener thinks hard [SILENCE]
    ("j2", ST4, "in"),     # KJV Matt 7:22 — recounting the deeds
    ("n2", ST6, "in"),     # knowing him is shown by love
    ("j3", ST5, "in"),     # KJV Matt 7:23 — I never knew you [SILENCE]
    ("n3", ST7, "in"),     # the door is knowing — an invitation
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after each KJV line + n1; "
    ...
    OUT = "matthew-7_i-never-knew-you.mp4"
    A_KBPS, MUX = 96, 20
