#!/usr/bin/env python3
"""Assemble Story Video #73 — This Day Fulfilled (Luke 4:16-21).
...
(Luke 4:18-19 the Isaiah reading, Luke 4:21 the fulfillment — exact KJV).
NO music bed of any kind (HUM PURGE 2026-07-16): narration + intentional silence only.
CONTENT-CARE: GREEN — ends at v21; the cliff/rejection is row 105's story.

SPLIT BEATS: j1 (the long Isaiah reading) plays over TWO stills — s3 (he reads) then
s4 (the room listens); n3 plays over s7 (wonder) then s8 (the open scroll, "Today.").
The video cut lands exactly on a caption-chunk boundary so the caption on screen
always matches what is being said.

Caption box black@0.60 — bright Sabbath-morning story, tuned to the lightest frame.

Output: luke-4_this-day-fulfilled.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
CREAM = "0xF7F2E9"
INK = "0x3B2A1E"
BOX_ALPHA = "0.60"   # brightest-frame tuning (pale synagogue stone, morning light)

ENC = ["-c:v", "libx264", "-preset", "medium", "-crf", "16",
       "-pix_fmt", "yuv420p", "-r", str(FPS), "-an"]

S1 = "s1-back-to-nazareth.jpeg"
S2 = "s2-the-scroll-handed.jpeg"
S3 = "s3-he-began-to-read.jpeg"
S4 = "s4-the-room-listens.jpeg"
S5 = "s5-and-sat-down.jpeg"
S6 = "s6-this-day.jpeg"
S7 = "s7-wonder-in-the-room.jpeg"
S8 = "s8-the-open-scroll.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}
CARD_TEXT = TEXT["card"]

# (audio_name, [(still, zdir), ...]) — more than one still = split beat: the audio
# plays once, the picture cuts at a caption-chunk boundary near the midpoint.
BEATS = [
    ("n0", [(S1, "in")]),
    ("n1", [(S2, "in")]),
    ("j1", [(S3, "in"), (S4, "in")]),   # the Isaiah reading — SACRED SILENCE after
    ("n2", [(S5, "in")]),               # rolled it up, sat down, breath held
    ("j2", [(S6, "in")]),               # "This day..." — SACRED SILENCE after
    ("n3", [(S7, "in"), (S8, "out")]),  # wonder in the room → the open scroll
]
...
def main():
    ...
    # Timeline: each beat may be one or two video segments; audio plays once.
    segments = []          # (seg_id, still, zdir, dur, chunks, kjv, first)
    audio_place = []
    t = 0.0
    start_of = {}
    for bi, (name, stills) in enumerate(BEATS):
        kjv = name in KJV
        ...
        if len(stills) == 1:
            ...
            # Split beat: cut at the caption-chunk boundary nearest the midpoint,
            # so no caption straddles the picture cut.
            if len(chunks) < 2:
                raise SystemExit(f"split beat {name} has <2 caption chunks")
            target = vdur / 2
            tcut = min((c[2] for c in chunks[:-1]), key=lambda b: abs(b - target))
            first_chunks = [c for c in chunks if c[2] <= tcut + 1e-6]
            second_chunks = [(c, cs - tcut, ce - tcut)
                             for (c, cs, ce) in chunks if cs >= tcut - 1e-6]
            (stA, zdA), (stB, zdB) = stills
            segments.append((f"{name}a", stA, zdA, tcut, first_chunks,
                             kjv, bi == 0))
            segments.append((f"{name}b", stB, zdB, vdur - tcut, second_chunks,
                             kjv, False))
            print(f"split {name}: cut at {tcut:.2f}s of {vdur:.2f}s "
                  f"({len(first_chunks)}+{len(second_chunks)} chunks)", flush=True)
        t += vdur
    ...
    OUT = "luke-4_this-day-fulfilled.mp4"
    A_KBPS, MUX = 96, 20
