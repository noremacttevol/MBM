#!/usr/bin/env python3
"""Assemble MEMBER Verse Video #186 — Heirs of God, Joint-Heirs with
...
CONTENT-CARE: clean. Jesus does not appear (epistle teaching) — inheritance
carried by doorway/companion light, no master face.

TWO SACRED SILENCES: s1 — Rom 8:16 (children of God) and s2 — Rom 8:17
(heirs, joint-heirs), scripture voice, cream italic. n2 (suffer with him /
glorified together) gets the draft's extended quiet hold. n3 is recorded in
two halves (n3a/n3b) so each thought lands on its own still (s6 closing-line,
s7 dawn) per the CAPTION LAW. n1 rides the family table (room for all) and n2
the co-heirs walk — story-order over filename-order, per the draft's beats.
...
paths in segs/. Output: romans-8_heirs-of-god.mp4,
...
ST1 = "s1-writing-to-rome.jpeg"
ST2 = "s2-children-of-god.jpeg"
ST3 = "s3-the-doorway-of-inheritance.jpeg"
ST4 = "s4-walking-as-co-heirs.jpeg"
ST5 = "s5-room-for-all.jpeg"
ST6 = "s6-the-closing-line.jpeg"
ST7 = "s7-dawn-over-the-city.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1", "s2"}
# Cameron 2026-07-18: "only Jesus's words in red." These verses are
# Paul (Romans 8:16-17) speaking/writing — NOT Jesus — so they get the narrator voice and a
...
GL_POINTER = "Learn more — Gospel Library: Divine Nature"

BEATS = [
    ("n0", ST1, "in"),           # Paul writing to Rome
    ("s1", ST2, "in"),           # SACRED SILENCE 1 — Rom 8:16, children of God
    ("s2", ST3, "in"),           # SACRED SILENCE 2 — Rom 8:17, the doorway
    ("n1", ST5, "in"),           # the family includes you — room for all
    ("n2", ST4, "in"),           # share his suffering, share his glory — the walk
    ("n3a", ST6, "in"),          # received by belonging — the closing line
    ("n3b", ST7, "in"),          # heirs together — dawn over the city
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap overrides: n2 = the draft's sacred-silence beat; n3b = a
# longer breath on the last image before the card (< 2.5s dead-air law).
HOLD = {"n2": 1.60, "n3b": 1.90}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silences: s1 (Rom 8:16) at {start_of['s1']:.1f}s, s2 (8:17) at {start_of['s2']:.1f}s", flush=True)
