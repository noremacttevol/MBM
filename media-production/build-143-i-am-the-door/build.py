#!/usr/bin/env python3
"""Assemble Story Video #143 — "I Am the Door" (John 10:7-9).
...
Face-shown Jesus locked to master (s1/s2/s4/s6). The climbing figure (s3) is
kept at a dignified distance — CARE gentle. Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV John 10:7 (j1),
10:9 (j2). Ear-check pass. n1 split so the climbing-figure and safe-fold
stills each carry their half.

SACRED SILENCES: the marked s3 hold after n1a (two ways in) and the pasture
rest after j2 (on s5). KJV holds after j1 and j2. s7 (the open fold door at
sunset) is the sanctioned short breath before the card.

NO MUSIC BED (HUM PURGE law). Bright pastoral video: caption box black@0.60.

Output: john-10_i-am-the-door.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-one-opening.jpeg"
ST2 = "s2-the-door-of-the-sheep.jpeg"
ST3 = "s3-the-wall-and-the-door.jpeg"
ST4 = "s4-in-safe.jpeg"
ST5 = "s5-go-in-and-out-find-pasture.jpeg"
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after j1 / n1a / j2 + breath on s7; "
