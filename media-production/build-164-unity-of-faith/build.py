#!/usr/bin/env python3
"""Assemble Story Video #164 — Till we all come in the unity of the faith
(Ephesians 4:11-14). MEMBER shelf verse-video → Gospel Library topic: Apostles.
...
Face-never (the #1 Law): the risen Lord / God is never rendered anywhere — he is
referenced by the narrator only; where the text speaks of him (kv11 the giver, kv13
the Son of God / the fulness) the picture is warm light from heaven, empty of any
figure (gate PASS).

TWO SACRED SILENCES (the music bed dies to true silence for both):
  kv11  Eph 4:11  "And he gave some, apostles..." — the fivefold gifts
  kv13  Eph 4:13  "Till we all come in the unity of the faith..." — the named verse
A warm bed carries it in, dies before the gifts verse, returns, dies again before
the unity verse, then returns for the invitation.
...
Output: ephesians-4_unity-of-the-faith.mp4, 1080x1920 H.264 30fps, <30MB.
"""
...
S1 = "s1-gifts-given.jpeg"
S2 = "s2-the-fivefold.jpeg"
S3 = "s3-equipping-the-saints.jpeg"
S4 = "s4-the-body-built.jpeg"
S5 = "s5-coming-into-unity.jpeg"
S6 = "s6-full-stature.jpeg"
S7 = "s7-tossed-no-more.jpeg"
S8 = "s8-grow-up-together.jpeg"
...
GL_POINTER = "Learn more — Gospel Library: Apostles"

BEATS = [
    ("n1", S1, "in"),
    ("kv11", S2, "in"),                      # SACRED SILENCE 1 — apostles, prophets, evangelists, pastors, teachers (Eph 4:11)
    ("n2", S3, "in"),
    ("n3", S4, "in"),
    ("n4", S5, "in"),
    ("kv13", S6, "in"),                      # SACRED SILENCE 2 — unity of the faith (Eph 4:13, named)
    ("n5", S7, "in"),
    ("n6", S7, "out"),
    ("n7", S8, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (apostles/prophets, Eph 4:11): kv11 at {start_of['kv11']:.1f}s",
          flush=True)
    print(f"sacred silence 2 (unity of the faith, Eph 4:13): kv13 at {start_of['kv13']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["kv11"] + spoken["kv11"]
    p2_end = start_of["kv13"] + spoken["kv13"]
    beds = [
        (0.0, start_of["kv11"] - 1.2, "b"),
        (p1_end + 1.0, start_of["kv13"] - 1.0, "a"),
    ...
    OUT = "ephesians-4_unity-of-the-faith.mp4"
    A_KBPS, MUX = 96, 20
