#!/usr/bin/env python3
"""Assemble Story Video #134 — "Other Sheep I Have" (John 10:16).

PHASE-1 STILLS-ONLY (Law E): 10 stills painted, 8 cut in (s2 gentle-and-
watchful and s8 staff-across-two-pens redundant mood shots — precedented
drop), Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW
§5), cream card.

Face-shown Jesus locked to master (s1/s5/s6/s7/s10). Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV John 10:16.
Ear-check pass all segments.

SACRED SILENCES: the marked s4 hold after n1b (sheep from another hillside)
and the one-fold hold after j1. s10 (one shepherd, one flock at sunrise) is
the sanctioned short breath before the card.

NO MUSIC BED (HUM PURGE law). Bright pastoral video: caption box
black@0.60 (pack directive).

Output: john-10_other-sheep-i-have.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-shepherd-teaching.jpeg"
ST3 = "s3-the-familiar-fold.jpeg"
ST4 = "s4-sheep-from-another-hillside.jpeg"
ST5 = "s5-gathering-not-excluding.jpeg"
ST6 = "s6-one-flock.jpeg"
ST7 = "s7-inclusion-not-boundary.jpeg"
ST9 = "s9-at-peace-in-one-field.jpeg"
ST10 = "s10-one-shepherd-one-flock.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}   # John 10:16, Jesus voice — cream italic
# Long holds: the marked s4 silence after n1b and the one-fold hold
# after j1.
LONG_HOLD = {"n1b", "j1"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]
...
BEATS = [
    ("n0", ST1, "in"),     # the shepherd teaching
    ("n1a", ST3, "in"),    # the familiar fold
    ("n1b", ST4, "in"),    # sheep from another hillside [SILENCE]
    ("n2", ST5, "in"),     # gathering, not excluding
    ("j1", ST6, "in"),     # KJV John 10:16 — one flock
    ("n3", ST7, "in"),     # inclusion, not boundary
    ("n4", ST9, "in"),     # at peace in one field
]

# s10 (one shepherd, one flock): the sanctioned short breath before card.
BREATH_STILL = ST10
BREATH_DUR = 1.2
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after n1b / j1 (KJV) + breath on s10; "
    ...
    OUT = "john-10_other-sheep-i-have.mp4"
    A_KBPS, MUX = 96, 20
