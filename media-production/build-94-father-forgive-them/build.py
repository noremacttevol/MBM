#!/usr/bin/env python3
"""Assemble Story Video #94 — "Father, Forgive Them" (Luke 23:33-34).
...
CONTENT-CARE — CRUCIFIXION (ARC, R): cross visible but NO gore, and the nails
are NEVER shown driving (the text says "driving the nails"; the picture shows
his face toward heaven, the soldiers casting lots, the prayer — not the act).
The story IS mercy — forgiveness spoken from the cross. FACE LAW v3: Jesus
face-shown (master-locked, only he wears cream).

ONE SACRED SILENCE: j1 — Luke 23:34 ("Father, forgive them..."), Jesus voice,
cream italic. n1 gets the draft's extended hold. n4 recorded in halves so the
...
paths in segs/. Output: luke-23_father-forgive-them.mp4,
...
ST1 = "s1-the-place-called-calvary.jpeg"
ST2 = "s2-his-face-toward-heaven.jpeg"
ST3 = "s3-casting-lots.jpeg"
ST4 = "s4-the-prayer.jpeg"
ST5 = "s5-caught-by-the-words.jpeg"
ST6 = "s6-light-toward-the-center-cross.jpeg"
ST7 = "s7-mercy-reaching-down.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Forgiveness"

BEATS = [
    ("n0", ST1, "in"),           # the place called Calvary
    ("n1", ST2, "in"),           # every reason to curse — face toward heaven; hold
    ("n2", ST3, "in"),           # a prayer for those casting lots
    ("j1", ST4, "in"),           # SACRED SILENCE — Luke 23:34, the prayer
    ("n3", ST5, "in"),           # forgive them now — caught by the words
    ("n4a", ST6, "in"),          # how far his mercy reaches — light to the cross
    ("n4b", ST7, "in"),          # nothing it can't cover — mercy reaching down
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap override: n1 = the draft's sacred-silence beat before the prayer.
HOLD = {"n1": 1.60}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silence: j1 (Luke 23:34) at {start_of['j1']:.1f}s", flush=True)
    ...
    OUT = "luke-23_father-forgive-them.mp4"
    A_KBPS = 96
    MUX = 20
