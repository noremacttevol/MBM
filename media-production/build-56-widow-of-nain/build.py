#!/usr/bin/env python3
"""Assemble Video #56 — The Widow of Nain's Son (Luke 7:11-17).
...
generation). He appears in s1, s3-s9; s2 (the funeral coming out) has no divine figure.
Captions are VERBATIM spoken text from make_narration.SEGMENTS; his two KJV lines (jv13
"Weep not", jv14 the raising) render in cream italic and are the two sacred silences.

CARE FLAGS: R (RESTRAINT). The funeral and dead body are shown with dignity, never gory;
the young man wrapped in muted grey linen, peaceful as if asleep. Hope-beat is the son
restored to his mother (s7-s8).
...
Output: luke-7_widow-of-nain.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <30MB.
"""
import os
import shutil
import subprocess
...
S1 = "s1-nearing-nain.jpeg"
S2 = "s2-the-funeral.jpeg"
S3 = "s3-he-saw-her.jpeg"
S4 = "s4-weep-not.jpeg"
S5 = "s5-he-touched-the-bier.jpeg"
S6 = "s6-young-man-arise.jpeg"
S7 = "s7-he-sat-up.jpeg"
S8 = "s8-gave-him-to-his-mother.jpeg"
S9 = "s9-they-glorified-god.jpeg"
...
BEATS = [
    ...
    ("n2", S2, "in"),
    ("n3", S3, "in"),
    ("jv13", S4, "in"),                         # WEEP NOT — full silence 1
    ("n4", S5, "in"),
    ("jv14", S6, "in"),                         # YOUNG MAN, ARISE — full silence 2
    ("n5", S7, "in"),
    ("n6", S8, "in"),
    ("n7", S9, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (weep not): jv13 at {start_of['jv13']:.1f}s", flush=True)
    print(f"sacred silence 2 (arise): jv14 at {start_of['jv14']:.1f}s", flush=True)
    ...
    p2_end = start_of["jv14"] + spoken["jv14"]
    beds = [
        (0.0, start_of["jv13"] - 1.2, "b"),
        (p1_end + 1.0, start_of["jv14"] - 1.0, "a"),
        (p2_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out before jv13 (ends {p1_end:.1f}s); returns; silent again "
          f"through jv14 (ends {p2_end:.1f}s)", flush=True)
