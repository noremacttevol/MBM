#!/usr/bin/env python3
"""Assemble Story Video #168 — Born of water and of the Spirit (John 3:5, with 3:1-8).
MEMBER shelf verse-video → Gospel Library topic: Baptism.
...
is the exact spoken text of its segment. KJV scripture lines render cream italic.

Face-never (the #1 Law): the Lord / God / the Spirit is never rendered anywhere — the one
Nicodemus comes to, and the Spirit, are shown only as warm light (never a dove, never a
figure); Nicodemus and the seekers are the human actors (gate PASS — no divine proper noun).

TWO SACRED SILENCES (the music bed dies to true silence for both) — the parallel pair:
  kv3  John 3:3  "...Except a man be born again, he cannot see the kingdom of God."
  kv5  John 3:5  "...born of water and of the Spirit... cannot enter into the kingdom of God."
A warm bed carries it in, dies before the born-again verse, returns, dies again before
the water-and-Spirit verse, then returns for the invitation.

SIZE: 30MB cap — kept for consistency; a verse-video runs well under it.

CAPTION BOX: night + lamplit + daylight mix — box alpha 0.58 per §4b brightest-frame.
...
Output: john-3_born-of-water-and-spirit.mp4, 1080x1920 H.264 30fps, <30MB.
"""
...
S1 = "s1-nicodemus-by-night.jpeg"
S2 = "s2-comes-to-the-light.jpeg"
S3 = "s3-born-again.jpeg"
S4 = "s4-how-can-this-be.jpeg"
S5 = "s5-born-of-water.jpeg"
S6 = "s6-and-of-the-spirit.jpeg"
S7 = "s7-enter-the-kingdom.jpeg"
S8 = "s8-the-gate-for-you.jpeg"
...
GL_POINTER = "Learn more — Gospel Library: Baptism"

BEATS = [
    ("n1", S1, "in"),
    ("n2", S2, "in"),
    ("kv3", S3, "in"),                       # SACRED SILENCE 1 — born again, see the kingdom (John 3:3)
    ("n3", S4, "in"),
    ("n4", S5, "in"),
    ("n5", S6, "in"),
    ("kv5", S7, "in"),                       # SACRED SILENCE 2 — born of water and Spirit, enter the kingdom (John 3:5, named)
    ("n6", S7, "out"),
    ("n7", S8, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (born again, John 3:3): kv3 at {start_of['kv3']:.1f}s",
          flush=True)
    print(f"sacred silence 2 (water and Spirit, John 3:5): kv5 at {start_of['kv5']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["kv3"] + spoken["kv3"]
    p2_end = start_of["kv5"] + spoken["kv5"]
    beds = [
        (0.0, start_of["kv3"] - 1.2, "b"),
        (p1_end + 1.0, start_of["kv5"] - 1.0, "a"),
    ...
    OUT = "john-3_born-of-water-and-spirit.mp4"
    A_KBPS, MUX = 96, 20
