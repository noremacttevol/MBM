...
v3 FULL REBUILD (2026-07-15, Machine A): PHASE-1 STILLS-ONLY, face-shown Jesus locked
to the master ref, continuity locks on the woman, the synagogue and the ruler. Caption-v2
(wide bottom, chunked). Two-voice: narrator en-US-AndrewNeural, KJV en-US-ChristopherNeural.
KJV lines (j1 Luke 13:12, j2 Luke 13:15-16) render cream-italic; the music dies to true
silence around n5b (he laid his hands on her and she stood up straight). Closing card is
an invitation.

Output: luke-13_bent-woman.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S7 = "s7-praise.jpeg"
S8 = "s8-ruler.jpeg"
S9 = "s9-hypocrite.jpeg"
S10 = "s10-daughter.jpeg"
...
PEAK = "n5b"   # he laid his hands on her and she stood up straight — sacred silence
CARD_TEXT = ("Whatever you have carried so long you stopped expecting it to "
             "change — he sees you at the back of the room. And he calls you first.")
...
def build_still(seg_id, src, dur, zdir, spoken_end, cap_text, speaker, first):
    ...
    # CAPTION-LAW engine: band + text drawn directly on the opaque still,
    # each chunk time-gated to its REAL spoken window (audio/<seg>.timing.json).
...
def main():
    ...
    card_start = t
    audio_place.append(("audio/n12.mp3", card_start + LEAD))
    total = t + card_vdur
