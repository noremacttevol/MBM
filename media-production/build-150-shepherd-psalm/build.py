#!/usr/bin/env python3
"""Assemble Story Video #150 — The Shepherd Psalm (Psalm 23:1-6).
...
Old Testament — Jesus does NOT appear as a character; the Shepherd is the
LORD. Narrator en-US-AndrewNeural paraphrases the psalm; the single exact-KJV
line (Psalm 23:1, segment "s1") is carried by the SCRIPTURE voice
(en-US-ChristopherNeural, build-161 precedent), cream-italic, sacred gap,
over the still-waters image breathing out. n2 (right paths) also carries the
sacred-length gap the draft marks. CONTENT-CARE: the valley is reverent low
light, never graphic.

Bright green-pasture daylight story — caption box black@0.60.

Output: psalm-23_the-shepherd-psalm.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-still-waters.jpeg"
ST2 = "s2-he-restoreth.jpeg"
ST3 = "s3-paths-of-righteousness.jpeg"
ST4 = "s4-the-darkest-valley.jpeg"
ST5 = "s5-a-table-before-me.jpeg"
ST6 = "s6-oil-on-the-head.jpeg"
ST7 = "s7-toward-the-dwelling.jpeg"
ST8 = "s8-let-him-lead.jpeg"
...
BEATS = [
    ("n0", ST1, "in"),     # a shepherd-king's song — still waters
    ("s1", ST1, "out"),    # KJV Psalm 23:1 (scripture voice, sacred)
    ("n1a", ST2, "in"),    # quiet water and green places to rest
    ("n1b", ST2, "out"),   # he puts my life back together
    ("n2", ST3, "in"),     # the right paths (sacred gap follows)
    ("n3", ST4, "in"),     # the darkest valley — presence, not terror
    ("n4a", ST5, "in"),    # a table before my enemies
    ("n4b", ST6, "in"),    # oil on the head — my cup runs over
    ("n5a", ST7, "in"),    # goodness and mercy — toward the dwelling
    ("n5b", ST8, "out"),   # home with You forever — let Him lead
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"KJV Ps 23:1 at {j1_start:.1f}s; "
    ...
    OUT = "psalm-23_the-shepherd-psalm.mp4"
    A_KBPS, MUX = 96, 20
