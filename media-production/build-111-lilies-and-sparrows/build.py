...
Output: matthew-6_lilies-and-sparrows.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-anxious-crowd.jpeg"
S2 = "s2-behold-the-birds.jpeg"
S3 = "s3-the-fed-sparrows.jpeg"
S4 = "s4-consider-the-lilies.jpeg"
S5 = "s5-solomon-not-arrayed.jpeg"
S6 = "s6-the-field-of-grass.jpeg"
S7 = "s7-tomorrow-worries.jpeg"
S8 = "s8-how-much-more-you.jpeg"
S9 = "s9-seek-first-the-kingdom.jpeg"
S10 = "s10-do-not-be-afraid.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"jv26", "jv2829", "jv33"}

# BEATS: (segment_name, still, zoom_dir). Zoom alternates in/out on a shared still.
BEATS = [
    ("n1", S1, "in"),
    ("jv26", S2, "in"),         # behold the fowls of the air — sacred silence 1
    ("n2", S3, "in"),           # the fed sparrows
    ("n3", S4, "in"),           # consider the lilies
    ("jv2829", S5, "in"),       # Solomon was not arrayed like one of these
    ("n4", S6, "in"),           # the field clothed by God
    ("n5", S7, "in"),           # how much more you (over the worried woman)
    ("n6", S8, "in"),           # loosening the grip; how much more you
    ("jv33", S9, "in"),         # seek ye first the kingdom — sacred silence 2
    ("n7", S10, "in"),          # you can breathe; you are cared for
]
...
def main():
    ...
    print(f"sacred silence 1 (behold the fowls): jv26 at {start_of['jv26']:.1f}s", flush=True)
    print(f"sacred silence 2 (seek ye first): jv33 at {start_of['jv33']:.1f}s", flush=True)
    ...
    p1_end = start_of["jv26"] + spoken["jv26"]
    p2_end = start_of["jv33"] + spoken["jv33"]
    beds = [
        (0.0, start_of["jv26"] - 1.2, "b"),
        (p1_end + 1.0, start_of["jv33"] - 1.0, "a"),
        (p2_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out at {start_of['jv26'] - 1.2:.1f}s, silent through jv26 "
          f"(ends {p1_end:.1f}s); returns; silent again through jv33 "
    ...
    OUT = "matthew-6_lilies-and-sparrows.mp4"
    A_KBPS = 96
    MUX = 20
