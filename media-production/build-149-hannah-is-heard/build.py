#!/usr/bin/env python3
"""Assemble Story Video #149 — Hannah Is Heard (1 Samuel 1:9-20).
...
Hannah's whispered vow (n2) is a modern paraphrase, so its caption stays
white — but the beat carries the longer sacred gap the draft marks there,
held on the silent-prayer still with an out-drift. CONTENT-CARE: no shame
framing of barrenness; one teardrop max (s2, held). s9 (the lamp at dusk)
is banked, not cut in — logged in QUEUE, no silent cap.

Mixed dim-interior/daylight frames — caption box black@0.60 (brightest-frame
rule, #40 lesson).

Output: 1-samuel-1_hannah-is-heard.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-silent-prayer.jpeg"
ST2 = "s2-sorrow-held.jpeg"
ST3 = "s3-the-other-wife.jpeg"
ST4 = "s4-eli-mistaken.jpeg"
ST5 = "s5-poured-out-not-drunk.jpeg"
ST6 = "s6-her-face-no-longer-sad.jpeg"
ST7 = "s7-samuel-in-her-arms.jpeg"
ST8 = "s8-given-back.jpeg"
...
CARD_TEXT = ("Hannah prayed the prayer no one else could hear, and God "
             "answered. Your quiet prayers are heard too.")

BEATS = [
    ("n0a", ST2, "in"),    # year after year — sorrow held
    ("n0b", ST3, "in"),    # the other wife
    ("n1", ST1, "in"),     # the silent prayer — only her lips moved
    ("n2", ST1, "out"),    # the vow (sacred hold, same still breathing out)
    ("n3a", ST4, "in"),    # Eli mistaken
    ("n3b", ST5, "in"),    # poured out, not drunk
    ("n4", ST6, "in"),     # her face no longer sad
    ("n5", ST7, "in"),     # Samuel in her arms
    ("n6", ST8, "out"),    # given back — she kept her word
]
...
def main():
    ...
    OUT = "1-samuel-1_hannah-is-heard.mp4"
    A_KBPS, MUX = 96, 20
