#!/usr/bin/env python3
"""Assemble Story Video #45 — The Wicked Tenants (Mark 12:1-11).

PHASE-1 STILLS-ONLY (Law E): 13 painted stills, Ken Burns drift, narration, serif
...
s13, staged from directly behind per the prompt sheet (gate PASS). The parable pictures
have no divine figure.

CARE-FLAG R (restraint) + J (mercy-in-judgment). The violence is never the picture: the
servant returns shaken (s5) not struck, the son's death is carried by the father's grief
at the shut gate (s10), and the reckoning is the vineyard given to faithful workers (s11).
The two SACRED SILENCES (music bed dies to true silence) land on the mercy and the hope:
  jv6  Mark 12:6     the beloved son sent — "they will reverence my son" (the heart)
  jv10 Mark 12:10-11 the rejected stone becomes the head of the corner (the turn)
A warm bed carries the setup in (jv1/jv2 play under it), dies before the beloved son,
returns through the reckoning (jv9 under it), dies again for the cornerstone, then the
closing card.

SIZE: 30MB cap (Cameron, 2026-07-14). Output: mark-12_wicked-tenants.mp4, 1080x1920
H.264 30fps, <30MB.
"""
import os
import subprocess
...
S2 = "s2-the-vineyard-made.jpeg"
S3 = "s3-he-goes-away.jpeg"
S4 = "s4-the-first-servant.jpeg"
S5 = "s5-sent-away-empty.jpeg"
S6 = "s6-he-sent-another.jpeg"
S7 = "s7-the-long-patience.jpeg"
S8 = "s8-the-beloved-son.jpeg"
S9 = "s9-they-saw-the-heir.jpeg"
S10 = "s10-the-fathers-grief.jpeg"
S11 = "s11-given-to-others.jpeg"
S12 = "s12-the-cornerstone.jpeg"
S13 = "s13-the-invitation.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"jv1", "jv2", "jv6", "jv9", "jv10"}
...
BEATS = [
    ...
    ("n6", S6, "in"),
    ("n7", S7, "in"),
    ("jv6", S8, "in"), ("n8", S8, "out"),        # SACRED SILENCE 1 — the beloved son
    ("n9", S9, "in"),
    ("n10", S10, "in"),
    ("jv9", S11, "in"), ("n11", S11, "out"),
    ("jv10", S12, "in"), ("n12", S12, "out"),     # SACRED SILENCE 2 — the cornerstone
    ("n13", S13, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (beloved son): jv6 at {start_of['jv6']:.1f}s", flush=True)
    print(f"sacred silence 2 (cornerstone): jv10 at {start_of['jv10']:.1f}s", flush=True)
    ...
    p1_end = start_of["jv6"] + spoken["jv6"]
    p2_end = start_of["jv10"] + spoken["jv10"]
    beds = [
        (0.0, start_of["jv6"] - 1.2, "b"),
    ...
    OUT = "mark-12_wicked-tenants.mp4"
    A_KBPS, MUX = 96, 20
