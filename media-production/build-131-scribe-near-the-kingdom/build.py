#!/usr/bin/env python3
"""Assemble Story Video #131 — The Scribe Near the Kingdom (Mark 12:28-34).

PHASE-1 STILLS-ONLY (Law E): 10 stills painted, 8 cut in (s2 listening and
s8 two-commands-remain were redundant mood shots — precedented drop), Ken
Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW §5), cream
closing card.

Face-shown Jesus locked to master (s1/s4/s6/s7). Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV Mark 12:34.
Ear-check pass ("close" adjective in n4 read right).

SACRED SILENCES: the marked s6 hold after n3 (the two faces in agreement)
and the "not far" hold after j1 on the recognition still. s10 (the open
doorway) is the sanctioned short breath before the card — it flows straight
into "step through the door."

NO MUSIC BED (HUM PURGE law). Daylight-interior video: caption box
black@0.60 (pack directive).

Output: mark-12_scribe-near-the-kingdom.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-a-real-question.jpeg"
ST3 = "s3-wanting-the-truth.jpeg"
ST4 = "s4-the-two-commands.jpeg"
ST5 = "s5-the-answer-lands.jpeg"
ST6 = "s6-honest-agreement.jpeg"
ST7 = "s7-recognition.jpeg"
ST9 = "s9-no-more-questions.jpeg"
ST10 = "s10-not-far-from-the-door.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}   # Mark 12:34, Jesus voice — cream italic
# Long holds: the marked s6 silence after n3 and the "not far" hold
# after j1.
LONG_HOLD = {"n3", "j1"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]
...
BEATS = [
    ("n0a", ST1, "in"),    # a real question — the approach
    ("n0b", ST3, "in"),    # the asking
    ("n1", ST4, "in"),     # the two commands
    ("n2", ST5, "in"),     # the answer lands — the scribe agrees
    ("n3", ST6, "in"),     # honest agreement, two faces [SILENCE]
    ("j1", ST7, "in"),     # KJV Mark 12:34 — recognition [SILENCE]
    ("n4", ST9, "in"),     # the hushed court
]

# s10 (the open doorway): the sanctioned short breath before the card.
BREATH_STILL = ST10
BREATH_DUR = 1.2
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after n3 / j1 (KJV) + breath on s10; "
    ...
    OUT = "mark-12_scribe-near-the-kingdom.mp4"
    A_KBPS, MUX = 96, 20
