#!/usr/bin/env python3
"""Assemble MEMBER Verse Video #183 — Sun, Moon, and Stars (1 Cor 15:40-42).
...
CONTENT-CARE: awe and order, never fear. No divine or human figure — cosmic
light only. Jesus does not appear.

TWO SACRED SILENCES: s1 — 1 Cor 15:41 (one glory of the sun...), on the
star-field's moon still; s2 — 1 Cor 15:42a (so also is the resurrection...),
on the ordered-heavens still. Both scripture voice (Christopher), cream italic.
The draft's third quiet beat (n3, glories plural) gets the same extended hold.
...
paths in segs/. Output: 1corinthians-15_sun-moon-and-stars.mp4,
...
ST1 = "s1-the-waiting-sky.jpeg"
ST2 = "s2-the-glory-of-the-sun.jpeg"
ST3 = "s3-the-glory-of-the-moon.jpeg"
ST4 = "s4-star-differeth-from-star.jpeg"
ST5 = "s5-ordered-not-chaotic.jpeg"
ST6 = "s6-a-canopy-of-care.jpeg"
ST7 = "s7-made-for-light.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1", "s2"}
# Cameron 2026-07-18: "only Jesus's words in red." These verses are
# Paul (1 Cor 15:41-42) speaking/writing — NOT Jesus — so they get the narrator voice and a
...
BEATS = [
    ("n0", ST1, "in"),           # Paul points at the sky
    ("n1", ST2, "in"),           # the sun — not everything shines the same
    ("s1", ST3, "in"),           # SACRED SILENCE 1 — 1 Cor 15:41, on the moon
    ("n2", ST4, "in"),           # no two stars burn alike
    ("s2", ST5, "in"),           # SACRED SILENCE 2 — 1 Cor 15:42a
    ("n3", ST6, "in"),           # glories plural — extended quiet hold
    ("n4", ST7, "in"),           # a brightness prepared for you
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap override: n3 = the draft's sacred-silence hold on the canopy
# (< 2.5s dead-air law once the next beat's LEAD is added).
HOLD = {"n3": 1.60}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silences: s1 (1 Cor 15:41) at {start_of['s1']:.1f}s, s2 (15:42a) at {start_of['s2']:.1f}s", flush=True)
    ...
    OUT = "1corinthians-15_sun-moon-and-stars.mp4"
    A_KBPS = 96
    MUX = 20
