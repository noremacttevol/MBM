#!/usr/bin/env python3
"""Assemble Story Video #128 — The Famine of Hearing (Amos 8:11-12).

PHASE-1 STILLS-ONLY (Law E): 6 painted stills, Ken Burns drift, narration,
serif captions (caption-v2: bottom band only, chunked + synced to the spoken
words — CAPTION LAW §5), cream closing card. NO AI motion clips.

Old Testament prophecy — no Jesus figure. Two-voice: narrator
en-US-AndrewNeural; the KJV verses (Amos 8:11, 8:12) read by the scripture
voice en-US-ChristopherNeural per the build-161 precedent. Ear-check pass,
no SPOKEN overrides needed.

SACRED SILENCES: extended holds after both KJV verses, the ache-hold after
n2, and after n3b on s5 (the listening face — the pack's marked silence
beat). s6 (the whisper-as-light window) is the sanctioned short breath
before the closing card (~1.8s, silent, no caption).

NO MUSIC BED of any kind (HUM PURGE law, 2026-07-16): audio is narration +
intentional silence only.

Dusk-and-lamp video: caption box black@0.50 (pack directive).

Output: amos-8_famine-of-hearing.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-prophet-on-the-hill.jpeg"
ST2 = "s2-the-real-famine.jpeg"
ST3 = "s3-seeking-and-not-finding.jpeg"
ST4 = "s4-the-open-scroll.jpeg"
ST5 = "s5-the-one-who-listens.jpeg"
ST6 = "s6-the-whisper-as-light.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
TEXT["n0a"] = TEXT["n0a"].rstrip(" \u2014")   # drop the orphan em-dash from the caption (audio unchanged)
KJV = {"s1", "s2"}   # the Amos verses (scripture voice) — cream italic
# Long holds: both KJV verses, the ache-hold after n2 (pack directive),
# and the marked SILENCE on s5 after n3b.
LONG_HOLD = {"s1", "s2", "n2"}
MID_HOLD = {"n3b"}   # marked SILENCE on s5, kept tighter — the s6 breath follows
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0a", ST1, "in"),    # Amos on the hill at dusk
    ("n0b", ST2, "in"),    # "not about food" — the empty basket
    ("n1",  ST2, "out"),   # the faraway, aching eyes
    ("s1",  ST3, "in"),    # KJV Amos 8:11 — famine of hearing
    ("s2",  ST3, "out"),   # KJV Amos 8:12 — sea to sea, not find it
    ("n2",  ST4, "in"),    # the ache — while the word waits in lamplight
    ("n3a", ST4, "out"),   # "The cure was never far." — the scroll
    ("n3b", ST5, "in"),    # "...listen..." — the listening face [SILENCE]
]

# s6 (the whisper as light): the sanctioned short breath before the
# closing card — silent, captionless, hopeful (No-Dead-Air Law's allowed
# "short breath before the closing card").
BREATH_STILL = ST6
BREATH_DUR = 1.2
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after s1 / s2 / n2 / n3b + breath on s6; "
    ...
    OUT = "amos-8_famine-of-hearing.mp4"
    A_KBPS, MUX = 96, 20
