#!/usr/bin/env python3
"""Assemble MEMBER Verse Video #182 — The Spirit Returns to God (Eccl 12:1-7).
...
timed to what the narrator is saying), cream-italic KJV verse, closing
invitation card with the Gospel Library pointer (MEMBER shelf). NO motion clips.

CONTENT-CARE: death carried with reverent distance and hope. No divine figure —
God conveyed through received light only. Jesus does not appear.

ONE SACRED SILENCE: s1 — Ecclesiastes 12:7, the scripture voice (Christopher),
cream italic, on the dust-to-dust still. The draft's second quiet beat (the
rising light, n2/s4) gets the same extended hold.

n4 is recorded in two halves (n4a/n4b) so the line's two halves land on their
two storyboard stills (s6 peace-on-the-face, s7 rest-not-terror) — the caption
...
paths in segs/. Output: ecclesiastes-12_the-spirit-returns-to-god.mp4,
...
ST1 = "s1-the-long-light-of-evening.jpeg"
ST2 = "s2-hands-at-rest.jpeg"
ST3 = "s3-dust-to-dust.jpeg"
ST4 = "s4-the-spirit-rises.jpeg"
ST5 = "s5-the-giver-receives.jpeg"
ST6 = "s6-peace-on-the-face.jpeg"
ST7 = "s7-rest-not-terror.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1"}
# Cameron 2026-07-18: "only Jesus's words in red." These verses are
# the Preacher (Eccl 12:7) speaking/writing — NOT Jesus — so they get the narrator voice and a
...
GL_POINTER = "Learn more — Gospel Library: Plan of Salvation"

BEATS = [
    ("n0", ST1, "in"),
    ("n1", ST2, "in"),
    ("s1", ST3, "in"),           # SACRED SILENCE — Eccl 12:7, the verse
    ("n2", ST4, "in"),           # the spirit rises — extended quiet hold
    ("n3", ST5, "in"),
    ("n4a", ST6, "in"),
    ("n4b", ST7, "in"),          # "— with mercy, not anger." / rest, not terror
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap overrides: n2 = the draft's sacred-silence hold on the rising
# light; n4b = a longer breath on the last image before the card (all < 2.5s
# dead-air law once the next beat's LEAD is added).
HOLD = {"n2": 1.60, "n4b": 1.90}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silence (Eccl 12:7): s1 at {start_of['s1']:.1f}s", flush=True)
    ...
    OUT = "ecclesiastes-12_the-spirit-returns-to-god.mp4"
    A_KBPS = 96
    MUX = 20
