#!/usr/bin/env python3
"""Assemble Story Video #79 — The Seventy Sent (Luke 10:1-20).

PHASE-1 STILLS-ONLY (Law E): 8 stills, 7 cut in as beats + s8 as the pre-card
breath, Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW §5),
cream card.

CARE: nothing dark ever depicted — no spirits, no serpents; the joy and the
mission carry it, no fake tears. Face-shown Jesus locked to master (s1/s5/s6/s7),
disciples earth tones, only-Jesus-cream. Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV Luke 10:2 (j1),
10:9 (j2), 10:20 (j3). Ear-check pass — j1's 0.93 is only whisper spelling
"labourers" as "laborers" (identical pronunciation); audio correct.

SACRED SILENCES: the marked s5 hold on j2 (come nigh) and reverent holds on
all three KJV lines. s8 (the starry night — names written in heaven) is the
sanctioned short breath before the card.

NO MUSIC BED (HUM PURGE law). Mixed dawn/day/evening/night: caption box
black@0.60 (lightest-frame rule per §5b).

Output: luke-10_the-seventy-sent.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-sending.jpeg"
ST2 = "s2-two-by-two.jpeg"
ST3 = "s3-welcomed.jpeg"
ST4 = "s4-heal-the-sick.jpeg"
ST5 = "s5-come-nigh.jpeg"
ST6 = "s6-they-returned-with-joy.jpeg"
ST7 = "s7-names-written.jpeg"
ST8 = "s8-written-in-heaven.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2", "j3"}   # Luke 10:2/9/20, Jesus voice — cream italic
LONG_HOLD = {"j1", "j2", "j3"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0", ST1, "in"),     # the sending of the seventy
    ("n1", ST2, "in"),     # two by two, just each other
    ("j1", ST3, "in"),     # KJV Luke 10:2 — the harvest is great
    ("n2", ST4, "in"),     # heal the sick, say the same thing
    ("j2", ST5, "in"),     # KJV Luke 10:9 — the kingdom is come nigh [SILENCE]
    ("n3", ST6, "in"),     # they returned with joy
    ("j3", ST7, "in"),     # KJV Luke 10:20 — your names written in heaven
]

# s8 (the starry night — names written in heaven): the sanctioned breath.
BREATH_STILL = ST8
BREATH_DUR = 1.2
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after each KJV line + breath on s8; "
    ...
    OUT = "luke-10_the-seventy-sent.mp4"
    A_KBPS, MUX = 96, 20
