#!/usr/bin/env python3
"""Assemble Video #120 — Job Answered from the Whirlwind (Job 38-42).
...
TWO SACRED SILENCES (music bed dies to true silence across both back-to-back God lines):
  jvA   Job 38:4    "Where wast thou when I laid the foundations of the earth?..."
  jvB   Job 38:31   "Canst thou bind the sweet influences of Pleiades...?"
No-Dead-Air is unaffected: the narrator never stops, only the music does.

Output: job-38_job-from-whirlwind.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-a-good-man-blessed.jpeg"
S2 = "s2-everything-taken.jpeg"
S3 = "s3-ashes-and-silence.jpeg"
S4 = "s4-cold-comfort.jpeg"
S5 = "s5-the-honest-cry.jpeg"
S6 = "s6-out-of-the-whirlwind.jpeg"
S7 = "s7-where-wast-thou.jpeg"
S8 = "s8-the-morning-stars.jpeg"
S9 = "s9-now-mine-eye-seeth-thee.jpeg"
S10 = "s10-comforted.jpeg"
...
BEATS = [
    ...
    ("n5", S5, "in"),
    ("n6", S6, "in"),
    ("jvA", S7, "in"),          # Where wast thou when I laid the foundations — silence 1
    ("jvB", S8, "in"),          # Canst thou bind the sweet influences of Pleiades — silence 2
    ("n7", S9, "in"),           # Job's answer "now mine eye seeth thee" (narrator, white)
    ("n8", S10, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (Where wast thou): jvA at {start_of['jvA']:.1f}s", flush=True)
    print(f"sacred silence 2 (Pleiades and Orion): jvB at {start_of['jvB']:.1f}s", flush=True)
    ...
    # jvA and jvB are back-to-back beats (7 and 8); the music bed dies before jvA and stays
    # in true silence across BOTH God lines, returning only after jvB up to the card.
    silence_start = start_of["jvA"] - 1.2
    silence_end = start_of["jvB"] + spoken["jvB"]
    beds = [
        (0.0, silence_start, "b"),
        (silence_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out at {silence_start:.1f}s, true silence across jvA+jvB "
          f"(ends {silence_end:.1f}s); returns until the card at {card_start:.1f}s",
          flush=True)
    ...
    OUT = "job-38_job-from-whirlwind.mp4"
    A_KBPS = 96
    MUX = 20
