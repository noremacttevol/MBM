#!/usr/bin/env python3
"""Assemble Story Video #83 — Weeping over Jerusalem (Luke 19:41-44).
...
(FACE-SHOWN, master-locked) speaking ONLY exact KJV — Luke 19:42 (j1). The
weeping-face still (s3) holds after n1b with the sacred pause; j1 holds too.
CONTENT-CARE (GRIEF): sorrow with restraint — wet eyes, no teardrop bead,
no despair; mercy/love is the tone. (Note: CONTENT-CARE.md row-83 is stale,
keyed to the old numbering — it lists the temptations, now row 70. This
story's care is grief-with-restraint per the pack NOTES.)

Warm hazy-gold video: caption box black@0.60 (brightest-frame, #40).

Output: luke-19_weeping-over-jerusalem.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-city-opens-up.jpeg"
S2 = "s2-he-stopped.jpeg"
S3 = "s3-he-wept.jpeg"
S4 = "s4-the-city-unaware.jpeg"
S5 = "s5-if-thou-hadst-known.jpeg"
S6 = "s6-they-watched-him-weep.jpeg"
S7 = "s7-love-looking-on.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}                  # cream-italic Jesus saying (Luke 19:42)
FLOW = {"n1a", "n3a"}         # narration line joins: tight flow gap
SACRED = {"n1b", "j1"}        # the weeping-face hold after n1b + the j1 verse hold
CARD_TEXT = ("He wept because he loved the city that couldn't see him. He "
             "sees you clearly. Don't look away.")

BEATS = [
    ("n0", S1, "in"),      # Jesus over the hill, the city opens up
    ("n1a", S2, "in"),     # he stopped and wept (he stopped at the crest)
    ("n1b", S3, "in"),     # not for himself, for them (his weeping face) [sacred pause after]
    ("n2", S4, "in"),      # the peace walked up, the city too busy (the city unaware)
    ("j1", S5, "in"),      # KJV Luke 19:42: if thou hadst known... hid from thine eyes (His face)
    ("n3a", S6, "in"),     # he saw what was coming (his followers watching)
    ("n3b", S7, "out"),    # the grief of love (the city in gold, love looking on)
]
...
CARD_HOLD = 2.0    # Cameron denial #83 pt.3 (2026-07-18): the old 13s pad left
                   # dead air after the closing question. End ~2s after the voice;
                   # 7 beats still run the video past 60s without a padded tail.
...
def main():
    ...
    OUT = "luke-19_weeping-over-jerusalem.mp4"
    A_KBPS, MUX = 96, 20
