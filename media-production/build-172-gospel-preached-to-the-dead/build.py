#!/usr/bin/env python3
"""Assemble Story Video #172 — The Gospel Preached to the Dead
(1 Peter 4:6; ref 3:18-20).
...
Peter's letter — Jesus does NOT appear as a character. Narrator
en-US-AndrewNeural; the exact-KJV centerpiece (1 Peter 4:6, segment "s1",
"live" SPOKEN-respelled /LIV/, caption keeps the KJV word) is carried by
the SCRIPTURE voice (en-US-ChristopherNeural, build-161 precedent),
cream-italic, sacred gap. n1b carries the sacred-length gap the pack marks
after the not-too-late line. CONTENT-CARE: hope-forward; the departed shown
only as a peaceful luminous figure.

Mixed lamplit/daylight frames — caption box black@0.60.

Output: 1-peter-4_gospel-preached-to-the-dead.mp4, 1080x1920, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-reading-the-letter.jpeg"
ST2 = "s2-peace-in-the-speaking.jpeg"
ST3 = "s3-the-quiet-marker.jpeg"
ST4 = "s4-separated-by-nothing-but-light.jpeg"
ST5 = "s5-the-hand-on-the-promise.jpeg"
ST6 = "s6-light-ascending.jpeg"
ST7 = "s7-the-message-still-going-out.jpeg"
...
BEATS = [
    ("n0", ST1, "in"),     # the letter read to the believers
    ("s1", ST2, "in"),     # KJV 1 Peter 4:6 (scripture voice, sacred)
    ("n1a", ST5, "in"),    # the hand on the promise
    ("n1b", ST3, "in"),    # not in vain, not too late (sacred gap)
    ("n2", ST4, "in"),     # alive by God's measure — the luminous figure
    ("n3a", ST7, "in"),    # death did not close the door
    ("n3b", ST6, "out"),   # the message crossed over — light ascending
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"KJV 1 Pet 4:6 at {j1_start:.1f}s; "
    ...
    OUT = "1-peter-4_gospel-preached-to-the-dead.mp4"
    A_KBPS, MUX = 96, 20
