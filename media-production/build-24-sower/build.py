#!/usr/bin/env python3
"""Assemble Story Video #24 — The Sower (Matthew 13:1-9, 18-23).
...
Face-never (#18): this is a parable told from the boat — the farmer and the four
soils are shown fully; the Lord appears ONLY in the bookend stills (s1, s7) as a
seated teacher in the boat, seen from behind, his face never shown.

Timing is COMPUTED from the measured mp3 durations (no hand offsets to drift).
Music: a warm detuned-pair bed opens and fades to FULL SILENCE just before j3
(the good-ground harvest — the emotional peak), so it lands in true silence; a
warm bed returns after and is out before the closing card.

Output: matthew-13_sower.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-boat-teaching.jpeg"
S2 = "s2-sower-scatter.jpeg"
S3 = "s3-wayside-birds.jpeg"
S4 = "s4-stony-scorched.jpeg"
S5 = "s5-thorns-choked.jpeg"
S6 = "s6-good-harvest.jpeg"
S7 = "s7-boat-close.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2", "j3"}
PEAK = "j3"   # the good-ground harvest — music goes to silence around this line
...
BEATS = [
    ...
    ("n6", S4, "in"), ("n7", S4, "out"),
    ("n8", S5, "in"),
    ("n9", S6, "in"), ("j3", S6, "out"), ("n10", S6, "in"),
    ("n11", S7, "in"), ("j2", S7, "out"), ("n12", S7, "in"),
]
