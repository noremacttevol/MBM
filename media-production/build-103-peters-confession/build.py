...
Output: matthew-16_peters-confession.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-on-the-road.jpeg"
S2 = "s2-the-question.jpeg"
S3 = "s3-what-men-say.jpeg"
S4 = "s4-but-whom-say-ye.jpeg"
S5 = "s5-peter-steps-forward.jpeg"
S6 = "s6-the-confession.jpeg"
S7 = "s7-blessed-art-thou.jpeg"
S8 = "s8-upon-this-rock.jpeg"
S9 = "s9-the-rock-stands.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"jv13", "jv15", "jv17", "jv18"}

# BEATS: (segment_name, still, zoom_dir). Zoom alternates in/out on a shared still.
BEATS = [
    ("n1", S1, "in"),
    ("jv13", S2, "in"),         # "Whom do men say that I ... am?"
    ("n2", S2, "out"),
    ("n3", S3, "in"),           # the disciples' second-hand answers
    ("jv15", S4, "in"),         # "But whom say ye that I am?"
    ("n4", S5, "in"),           # the question stops them; Peter steps forward
    ("np", S6, "in"),           # Peter's confession (narrator voice, white caption)
    ("n5", S6, "out"),
    ("jv17", S7, "in"),         # "Blessed art thou" — sacred silence 1
    ("n6", S7, "out"),
    ("jv18", S8, "in"),         # "upon this rock" — sacred silence 2
    ("n7", S9, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (Blessed art thou): jv17 at {start_of['jv17']:.1f}s", flush=True)
    print(f"sacred silence 2 (upon this rock): jv18 at {start_of['jv18']:.1f}s", flush=True)
    ...
    p1_end = start_of["jv17"] + spoken["jv17"]
    p2_end = start_of["jv18"] + spoken["jv18"]
    beds = [
        (0.0, start_of["jv17"] - 1.2, "b"),
        (p1_end + 1.0, start_of["jv18"] - 1.0, "a"),
        (p2_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out at {start_of['jv17'] - 1.2:.1f}s, silent through jv17 "
          f"(ends {p1_end:.1f}s); returns; silent again through jv18 "
    ...
    OUT = "matthew-16_peters-confession.mp4"
    A_KBPS = 96
    MUX = 20
