#!/usr/bin/env python3
"""Assemble Video #124 — Love Your Enemies (Matthew 5:43-48).
...
Captions are CAPTION v2: wide along the bottom, narrator <=2 lines, KJV <=3, long
segments split into synced chunks. Template: build-123 (this machine's proven pipeline,
incl. the per-line drawtext fix for this box's newline-tofu ffmpeg bug).

Jesus appears in three face-shown frame shots (s1, s3, s7 — LOCK v3, master-ref); the
illustration arc (farmer/neighbor) carries the rest. His exact-KJV lines are in the
scripture voice (Christopher) and render cream-italic; the narrator paraphrases.

TWO SACRED SILENCES (no music bed exists — NO-HUM law 2026-07-16 — so these are pure):
...
Output: matthew-5_love-your-enemies.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-hillside-teaching.jpeg"
S2 = "s2-the-old-wall.jpeg"
S3 = "s3-but-i-say.jpeg"
S4 = "s4-the-wrong.jpeg"
S5 = "s5-lifting-stones.jpeg"
S6 = "s6-lamplight-prayer.jpeg"
S7 = "s7-the-reason.jpeg"
S8 = "s8-sun-on-both.jpeg"
S9 = "s9-meeting-at-the-wall.jpeg"
S10 = "s10-the-shared-table.jpeg"
...
BEATS = [
    ("n1", S1, "in"),           # the mount, the crowd
    ("n2", S2, "in"),           # the old arithmetic: the cold wall
    ("jvA", S3, "in"),          # Matt 5:44 the command — sacred silence 1
    ("n3", S4, "in"),           # the wrong (broken wall, cold neighbor)
    ("n4", S5, "in"),           # lifting the neighbor's stones
    ("n5", S6, "in"),           # lamplight prayer, by name
    ("n6", S7, "in"),           # back to the mount: the reason
    ("jvB", S8, "in"),          # Matt 5:45 sun on both — sacred silence 2
    ("n7", S9, "in"),           # bread across the wall
    ("n8", S10, "in"),          # the shared table
]
...
# ---- CAPTION SYSTEM v2 (copied byte-for-behaviour from build-122) ----
def sentences(text):
    import re
...
def build_still(seg_id, src, dur, zdir, spoken_end, cap_text, kjv, first):
    ...
    # this batch's stills are borderless (QC'd) — no crop needed
...
def main():
    ...
    print(f"sacred silence 1 (the rule): jvA at {start_of['jvA']:.1f}s", flush=True)
    print(f"sacred silence 2 (good measure): jvB at {start_of['jvB']:.1f}s", flush=True)
    ...
    # jvA (beat 2) and jvB (beat 8) are SEPARATE beats; the music bed dies to true silence
    # for each Jesus line on its own and returns in between and after (up to the card).
    a_end = start_of["jvA"] + spoken["jvA"]
    b_end = start_of["jvB"] + spoken["jvB"]
    beds = [
        (0.0, start_of["jvA"] - 1.2, "b"),
        (a_end + 1.0, start_of["jvB"] - 1.2, "a"),
        (b_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out before jvA at {start_of['jvA'] - 1.2:.1f}s (silence through jvA, "
          f"ends {a_end:.1f}s); returns; bed out before jvB at {start_of['jvB'] - 1.2:.1f}s "
          f"(silence through jvB, ends {b_end:.1f}s); returns to the card", flush=True)
    ...
    OUT = "matthew-5_love-your-enemies.mp4"
    A_KBPS = 96
    MUX = 20
