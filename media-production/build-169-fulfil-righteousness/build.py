#!/usr/bin/env python3
"""Assemble Story Video #169 — To fulfil all righteousness (Matthew 3:13-17).
MEMBER shelf verse-video → Gospel Library topics: Baptism; Godhead.
...
FACE-SHOWN: Jesus is depicted (master face, cream robe) in s1-s7, text-locked to
JESUS-MASTER-REF. The Father is shown only as warm light; the Holy Ghost as the
descending white dove (scriptural — Matt 3:16). s8 has no divine figure.

Stills are already generated in assets/<slug>.jpeg (Flow, $0). Narration is already
generated in audio/ (make_narration.SEGMENTS). CAPTIONS ARE VERBATIM. KJV scripture
lines render cream italic.

TWO SACRED SILENCES (the music bed dies to true silence for both):
  kv15  Matt 3:15  "...for thus it becometh us to fulfil all righteousness..." — the example
  kv17  Matt 3:17  "...This is my beloved Son, in whom I am well pleased." — the Father's voice
A warm bed carries it in, dies before the righteousness verse, returns, dies again before
the voice-from-heaven verse, then returns for the invitation.

SIZE: 30MB cap — kept for consistency; a verse-video runs well under it.

CAPTION BOX: river daylight throughout — box alpha 0.58 per §4b brightest-frame.
...
Output: matthew-3_fulfil-all-righteousness.mp4, 1080x1920 H.264 30fps, <30MB.
"""
...
def _find(exe):
    ...
    pat = os.path.expandvars(
        rf"%LOCALAPPDATA%\Microsoft\WinGet\Packages\Gyan.FFmpeg*\**\bin\{exe}.exe")
    hits = _glob.glob(pat, recursive=True)
    return hits[0] if hits else exe
...
S1 = "s1-comes-to-jordan.jpeg"
S2 = "s2-john-hesitates.jpeg"
S3 = "s3-fulfil-righteousness.jpeg"
S4 = "s4-the-baptism.jpeg"
S5 = "s5-heavens-open-dove.jpeg"
S6 = "s6-voice-from-heaven.jpeg"
S7 = "s7-godhead-distinct.jpeg"
S8 = "s8-the-way-for-you.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"kv15", "kv17"}

# One-line Gospel Library pointer rendered small under the closing card (MEMBER shelf).
GL_POINTER = "Learn more — Gospel Library: Baptism; Godhead"

BEATS = [
    ("n1", S1, "in"),
    ("n2", S2, "in"),
    ("kv15", S3, "in"),                      # SACRED SILENCE 1 — fulfil all righteousness (Matt 3:15, named)
    ("n3", S4, "in"),
    ("n4", S5, "in"),
    ("kv17", S6, "in"),                      # SACRED SILENCE 2 — voice from heaven, beloved Son (Matt 3:17)
    ("n5", S7, "in"),
    ("n6", S7, "out"),
    ("n7", S8, "in"),
]
...
def build_card(dur, text):
    ...
    with open(pf, "w", encoding="utf-8") as f:
        f.write(GL_POINTER)
    # main invitation, centered; plus a small one-line Gospel Library pointer near
    # the lower third (MEMBER shelf — the church supplies the doctrine's home).
    vf = (f"drawtext=fontfile={SERIF}:textfile={tf}:fontsize=52:"
          f"fontcolor={INK}:line_spacing=24:x=(w-text_w)/2:y=(h-text_h)/2-70,"
          f"drawtext=fontfile={SERIF_BI}:textfile={pf}:fontsize=30:"
...
def main():
    ...
    print(f"sacred silence 1 (fulfil all righteousness, Matt 3:15): kv15 at {start_of['kv15']:.1f}s",
          flush=True)
    print(f"sacred silence 2 (beloved Son, Matt 3:17): kv17 at {start_of['kv17']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["kv15"] + spoken["kv15"]
    p2_end = start_of["kv17"] + spoken["kv17"]
    beds = [
        (0.0, start_of["kv15"] - 1.2, "b"),
        (p1_end + 1.0, start_of["kv17"] - 1.0, "a"),
    ...
    OUT = "matthew-3_fulfil-all-righteousness.mp4"
    A_KBPS, MUX = 96, 20
