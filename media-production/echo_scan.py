...
from corpus import canonical_builds, load_build_segments
...
def scan_builds():
    hits = {}
    for _row, bdir in sorted(canonical_builds(MP).items()):
        if not os.path.exists(os.path.join(bdir, "make_narration.py")):
            continue
        ...
        for i in range(len(segs) - 1):
            ...
            # echo = a character/scripture line adjacent to a narrator line
            # (either order)
            speakers = {sp1, sp2}
            ...
            if speakers == {"narrator"}:
                continue
            # character/scripture line = the non-narrator one; narrator = repeat side
            if sp1 == "narrator":
                char_line, narr_id, narr_txt = t2, id1, t1
            else:
                char_line, narr_id, narr_txt = t1, id2, t2
            bad = echo_sentences(char_line, narr_txt)
            if bad:
                hits.setdefault(b, []).append(
                    (
                        round(overlap(char_line, narr_txt), 2),
                        "char",
                        char_line,
                        narr_id,
                        narr_txt,
