#!/usr/bin/env python3
"""Assemble Story Video #174 — Elijah, and the Hearts of the Fathers
...
Jesus does NOT appear (Elijah/John prophecy). Narrator en-US-AndrewNeural;
the exact-KJV verse (Malachi 4:5-6) is carried by the SCRIPTURE voice
(en-US-ChristopherNeural, build-161 precedent), cream-italic, split at its
colon and delivered as ONE utterance across two stills (s1a s2, s1b s3) —
s1a flows into s1b, s1b holds with the sacred gap; the pack's sacred pause
also follows n1. n2 is split across the last two stills so the family
finale (s7) lands on "whole". CONTENT-CARE: no violence; the curse is named,
never shown. Elijah (OT prophet) and John are distinct figures; no Jesus.

Golden-hour frames: caption box black@0.60 (brightest-frame rule, #40).

Output: malachi-4_hearts-of-the-fathers.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-messenger.jpeg"
S2 = "s2-resolve-and-mercy.jpeg"
S3 = "s3-the-turn-beginning.jpeg"
S4 = "s4-three-generations.jpeg"
S5 = "s5-pointing-down-the-road.jpeg"
S6 = "s6-the-same-spirit.jpeg"
S7 = "s7-the-whole-family.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1a", "s1b"}          # cream-italic verse (Malachi 4:5-6)
FLOW = {"s1a", "n2a"}         # verse-internal join + the n2 sentence join: tight flow gap
SACRED = {"s1b", "n1"}        # verse-end hold + the pack's sacred pause after n1
CARD_TEXT = ("He cares about your family. Let the healing start with your "
             "own heart.")

BEATS = [
    ("n0", S1, "in"),      # Elijah the messenger sent ahead
    ("s1a", S2, "in"),     # KJV: Behold, I will send Elijah... (his resolute face)
    ("s1b", S3, "in"),     # KJV: he shall turn the heart... (the turn beginning) [sacred hold]
    ("n1", S4, "in"),      # his work: mend, turn hearts (three generations) [sacred pause after]
    ("n3", S6, "in"),      # the same spirit rested on John, preparing the way
    ("n2a", S5, "in"),     # so when the Lord came, families ready (Elijah points ahead)
    ("n2b", S7, "out"),    # not divided, but whole (the whole family)
]
...
def main():
    ...
    OUT = "malachi-4_hearts-of-the-fathers.mp4"
    A_KBPS, MUX = 96, 20
