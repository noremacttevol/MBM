#!/usr/bin/env python3
"""Assemble Video #52 — The Demoniac in the Synagogue (Mark 1:21-28).
...
generation). He appears in s1-s8; s9 (the news spreading) has no divine figure. Captions
are VERBATIM spoken text from make_narration.SEGMENTS; his single KJV line (jv25, "Hold
thy peace, and come out of him") renders in cream italic and is the one sacred silence.
The unclean spirit's cry is reported by the narrator in the plain white style.

CARE FLAGS: R (RESTRAINT). No visible demon/creature/gore; the torment is the man's own
anguish and the miracle is his quiet freedom (s7). The single sacred silence lands on
Jesus's command.
...
Output: mark-1_demoniac-synagogue.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <30MB.
"""
import os
import shutil
import subprocess
...
S1 = "s1-sabbath-synagogue.jpeg"
S2 = "s2-taught-with-authority.jpeg"
S3 = "s3-the-tormented-man.jpeg"
S4 = "s4-the-unclean-cry.jpeg"
S5 = "s5-hold-thy-peace.jpeg"
S6 = "s6-come-out-of-him.jpeg"
S7 = "s7-in-his-right-mind.jpeg"
S8 = "s8-the-people-amazed.jpeg"
S9 = "s9-fame-through-galilee.jpeg"
...
BEATS = [
    ...
    ("n3", S3, "in"),
    ("n4", S4, "in"),
    ("jv25", S5, "in"),                         # HOLD THY PEACE, COME OUT — full silence
    ("n5", S6, "in"),
    ("n6", S7, "in"),
    ("n7", S8, "in"),
    ("n8", S9, "in"),
...
def main():
    ...
    print(f"sacred silence (hold thy peace): jv25 at {start_of['jv25']:.1f}s", flush=True)
    ...
    #      sacred silence (jv25) and returns after it ----
    p1_end = start_of["jv25"] + spoken["jv25"]
    beds = [
        (0.0, start_of["jv25"] - 1.2, "b"),
        (p1_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out before jv25 (ends {p1_end:.1f}s); returns after the command",
          flush=True)
    ...
    OUT = "mark-1_demoniac-synagogue.mp4"
    A_KBPS = 96
    MUX = 20
