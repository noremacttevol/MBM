#!/usr/bin/env python3
"""Assemble Story Video #145 — "I Am the Way, the Truth, and the Life"
(John 14:6).
...
ONLY exact KJV — John 14:6 delivered in its two halves back to back (j1a
"I am the way..." on s3, j1b "no man cometh..." on s6, the stills the prompt
sheet named for those halves). Sacred pacing via KJV_GAP on both halves.

Mostly a lamplit night interior, but s5 (the misty road) is very pale —
caption box black@0.60 tuned to the BRIGHTEST frame (#40 lesson).

Output: john-14_way-truth-life.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-upper-room.jpeg"
S2 = "s2-thomas-asks.jpeg"
S3 = "s3-i-am-the-way.jpeg"
S4 = "s4-question-met-with-promise.jpeg"
S5 = "s5-the-way-made-personal.jpeg"
S6 = "s6-but-by-me.jpeg"
...
BEATS = [
    ("n0", S1, "in"),      # the upper room, the night before the cross
    ("n1", S2, "in"),      # Thomas asks the question
    ("j1a", S3, "in"),     # KJV John 14:6 first half — his face (sacred)
    ("j1b", S6, "in"),     # KJV John 14:6 second half — but by me
    ("n2a", S4, "in"),     # not one way among many — the two-shot
    ("n2b", S7, "in"),     # standing right there in the room with them
    ("n3a", S5, "in"),     # not a map to find — the misty road
    ("n3b", S8, "out"),    # a person to follow — the glowing doorway
]
...
def main():
    ...
    OUT = "john-14_way-truth-life.mp4"
    A_KBPS, MUX = 96, 20
