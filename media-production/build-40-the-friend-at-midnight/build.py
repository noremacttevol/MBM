#!/usr/bin/env python3
"""Assemble Story Video #40 — The Friend at Midnight (Luke 11:1-13).

PHASE-1 STILLS-ONLY build (Law E): 16 painted stills with slow Ken Burns drift,
...
Face-never (the #1 Law): Jesus appears in exactly four stills (s1, s2, s12, s15),
every one of them with the camera behind him — the back of his head to us, face
never rendered. Verses 5-8 are the parable he is TELLING, so no divine figure
appears in any parable still; the householder, the traveller and the neighbour are
ordinary people with faces fully shown.

TWO SACRED SILENCES (the music bed dies to true silence for both):
  j3  Luke 11:8   "...because of his importunity..."  — THE VERDICT, the verse-card
                                                        line from PAIRING-LIST.md
  j5  Luke 11:13  "...how much more shall your heavenly Father..." — THE PAYOFF
A warm bed carries the story in, dies before the verdict, returns to carry the turn
and the ask/seek/knock, then dies again before verse 13 lands. No-Dead-Air is
unaffected — the narrator never stops; only the music does.
...
Output: luke-11_friend-at-midnight.mp4 (SCRIPTURE-NAME LAW),
...
S1 = "s1-teach-us-to-pray.jpeg"
S2 = "s2-our-father.jpeg"
S3 = "s3-the-midnight-traveller.jpeg"
S4 = "s4-nothing-in-the-house.jpeg"
S5 = "s5-out-into-the-dark.jpeg"
S6 = "s6-friend-lend-me-three-loaves.jpeg"
S7 = "s7-the-door-is-now-shut.jpeg"
S8 = "s8-trouble-me-not.jpeg"
S9 = "s9-shameless.jpeg"
S10 = "s10-as-many-as-he-needeth.jpeg"
S11 = "s11-bread-at-midnight.jpeg"
S12 = "s12-the-neighbour-is-the-contrast.jpeg"
S13 = "s13-ask-seek-knock.jpeg"
S14 = "s14-bread-not-a-stone.jpeg"
S15 = "s15-how-much-more.jpeg"
S16 = "s16-the-light-is-already-on.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j0", "j1", "j2", "j3", "j4", "j5"}
...
BEATS = [
    ...
    ("n8", S7, "in"),
    ("j2", S8, "in"), ("n9", S8, "out"),
    ("n10", S9, "in"), ("n11", S9, "out"),
    ("j3", S10, "in"),                      # THE VERDICT — lands in full silence
    ("n12", S10, "out"),
    ("n13", S11, "in"),
    ("n14a", S12, "in"), ("n14b", S12, "out"),   # THE TURN
    ("j4", S13, "in"), ("n15", S13, "out"),
    ("n16a", S14, "in"), ("n16b", S14, "out"),
    ("j5", S15, "in"),                      # THE PAYOFF — also in full silence
    ("n17", S15, "out"),
    ("n18", S16, "in"),
]
...
def caption_overlay(seg_id, dur, text, kjv):
    ...
    # Box at 0.58. The #39 lesson said "night stories 0.40, daylight stories 0.58" —
    # but THIS story is both: the parable (s3-s11, s16) is deep midnight and the
    # frame (s1, s2, s12-s15) is full golden morning. One box value has to serve
    # every frame, so it is tuned to the BRIGHTEST frame in the video, not the
    # average. A 0.58 box on a night frame is merely a slightly stronger scrim; a
    # 0.40 box on the golden-morning frames washed the white text out. Legibility
    # wins. Verified on full-res crops of both s10 (night) and s15 (brightest).
...
def main():
    ...
    print(f"sacred silence 2 (the payoff):  j5 at {start_of['j5']:.1f}s", flush=True)
    ...
    j5_end = start_of["j5"] + spoken["j5"]
    beds = [
        # carries the story in; out before the verdict
        (0.0, start_of["j3"] - 1.2, "b"),
        # returns to carry the turn and ask/seek/knock; dies before verse 13
        (j3_end + 1.0, start_of["j5"] - 1.0, "a"),
        # returns after the payoff, gone before the closing card
        (j5_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out at {start_of['j3'] - 1.2:.1f}s, silent through j3 "
          f"(ends {j3_end:.1f}s); returns under n12; silent again through j5 "
          f"(ends {j5_end:.1f}s)", flush=True)
    ...
    OUT = "luke-11_friend-at-midnight.mp4"
