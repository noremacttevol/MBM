#!/usr/bin/env python3
"""Assemble Story Video #30 — The Net / The Dragnet (Matthew 13:47-50).

PHASE-1 STILLS-ONLY build (Law E): 6 painted 2K stills with slow Ken Burns
drift, narration, serif captions, KJV red-letter lines, closing card. NO motion
clips. Parable Jesus tells — NO Jesus figure on screen (voice + KJV only); the
fishermen are characters in the story, shown fully. Face gate trivially safe.
...
Music: a warm detuned-pair bed opens over the casting and the wide gathering,
then fades to a reverent hush around j2 ("...at the end of the world: the angels
shall come forth, and sever the wicked from among the just" — the sober end); a
warm bed returns just after j2 and swells UNDER the grace close (the gathering
came first, and the net was cast for you), fading out only before the closing
card. No-Dead-Air: the only silences >2.5s are the hush around j2 and the card's
tail.

Output: matthew-13_the-net.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264
30fps, <25MB.
"""
import os
import subprocess
...
S1 = "s1-cast.jpeg"
S2 = "s2-every-kind.jpeg"
S3 = "s3-draw-shore.jpeg"
S4 = "s4-good-vessels.jpeg"
S5 = "s5-cast-bad.jpeg"
S6 = "s6-shore-close.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}   # Matthew 13:47-48 (j1), 13:49 (j2) — Jesus voice
PEAK = {"j2"}        # "...the angels shall come forth..." — hush lands here
...
BEATS = [
    ("n1", S1, "in"), ("j1", S1, "out"), ("n2", S1, "in"),
    ("n3", S2, "in"), ("n4", S2, "out"),
    ("n5", S3, "in"),
    ("n6", S4, "in"),
    ("n7", S5, "in"), ("j2", S5, "out"), ("n8", S5, "in"),
    ("n9", S6, "in"), ("n10", S6, "out"), ("n11", S6, "in"),
]
...
def main():
    ...
    #   bed a : opens over the seeking and the finding -> fades out just before
    #           j2 ("...sold all that he had, and bought it"), so the cost paid
    #           in joy lands in a reverent hush.
    #   bed b : returns just after j2 and swells warm UNDER the joy (n7, n8) and
    #           the closing turn (n9, n10 — to him, YOU are the pearl), fading
    #           out only before the closing card. No-Dead-Air: the only silences
    #           >2.5s are the hush around j2 and the card's tail.
    beds = [
        (0.0, peak_start - 1.2, "a"),
        (peak_start + audio_dur["j2"] + 1.0, card_start - 0.8, "b"),
    ]
    _ = j2_start  # (kept for the timeline print above)
