#!/usr/bin/env python3
"""Assemble Video #118 — Jonah and the God Who Relents (Book of Jonah).
...
TWO SACRED SILENCES (music bed dies to true silence for each, separately):
  jvA   Jonah 3:2    "Arise, go unto Nineveh... and preach unto it..." — the second chance
  jvB   Jonah 4:11   "And should not I spare Nineveh, that great city..." — the mercy climax
No-Dead-Air is unaffected: the narrator never stops, only the music does.

Output: jonah-1_jonah-god-who-relents.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-call.jpeg"
S2 = "s2-he-flees.jpeg"
S3 = "s3-the-great-storm.jpeg"
S4 = "s4-the-great-fish.jpeg"
S5 = "s5-prayer-in-the-deep.jpeg"
S6 = "s6-cast-upon-dry-land.jpeg"
S7 = "s7-preaching-to-nineveh.jpeg"
S8 = "s8-the-city-repents.jpeg"
S9 = "s9-god-relents.jpeg"
S10 = "s10-should-i-not-spare.jpeg"
...
BEATS = [
    ...
    ("n4", S4, "in"),
    ("n5", S5, "in"),
    ("jvA", S6, "in"),          # Arise, go unto Nineveh... — the second chance — silence 1
    ("n6", S7, "in"),           # Jonah's warning "yet forty days" (narrator, white)
    ("n7", S8, "in"),
    ("n8", S9, "in"),
    ("jvB", S10, "in"),         # And should not I spare Nineveh... — mercy climax — silence 2
]
...
def main():
    ...
    print(f"sacred silence 1 (Arise, go unto Nineveh): jvA at {start_of['jvA']:.1f}s", flush=True)
    print(f"sacred silence 2 (should not I spare Nineveh): jvB at {start_of['jvB']:.1f}s", flush=True)
    ...
    # jvA and jvB are SEPARATE beats (6 and 10); the music bed dies to true silence for
    # each God line on its own, and returns in between. jvB is the last story beat, so the
    # second silence runs right up to the card (no bed after it).
    a_end = start_of["jvA"] + spoken["jvA"]
    beds = [
        (0.0, start_of["jvA"] - 1.2, "b"),
        (a_end + 1.0, start_of["jvB"] - 1.2, "a"),
    ]
    print(f"music: bed out before jvA at {start_of['jvA'] - 1.2:.1f}s (silence through "
          f"jvA, ends {a_end:.1f}s); returns; bed out again before jvB at "
          f"{start_of['jvB'] - 1.2:.1f}s (silence through jvB to the card)", flush=True)
    ...
    OUT = "jonah-1_jonah-god-who-relents.mp4"
    A_KBPS = 96
    MUX = 20
