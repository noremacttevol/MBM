#!/usr/bin/env python3
"""Assemble Story Video #146 — The Vine and the Branches (John 15:5).
...
Two-voice: narrator en-US-AndrewNeural, Jesus en-US-ChristopherNeural speaking
ONLY exact KJV — John 15:5 delivered in two pieces back to back split at its
first colon (j1a "I am the vine..." on s3, j1b "He that abideth..." on s6,
the stills the prompt sheet named for those pieces). Sacred KJV_GAP on both.

Bright daylight vineyard throughout — caption box black@0.60 (brightest-frame
rule, #40 lesson).

Output: john-15_vine-and-branches.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-vine-and-the-teacher.jpeg"
S2 = "s2-the-living-branch.jpeg"
S3 = "s3-i-am-the-vine.jpeg"
S4 = "s4-the-severed-branch.jpeg"
S5 = "s5-the-joined-branch.jpeg"
S6 = "s6-abide-in-me.jpeg"
S7 = "s7-the-whole-vineyard.jpeg"
S8 = "s8-hands-on-the-living-branch.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1a", "j1b"}
CARD_TEXT = ("Stay connected to the Vine. Let His life flow through you.")

BEATS = [
    ("n0a", S1, "in"),     # the teacher by the vine
    ("n0b", S2, "in"),     # the living branch, heavy with fruit
    ("j1a", S3, "in"),     # KJV John 15:5 — I am the vine (sacred)
    ("j1b", S6, "in"),     # KJV John 15:5 — he that abideth in me
    ("n1", S4, "in"),      # the severed branch — disconnected
    ("n2", S5, "in"),      # the joined branch — the life flows
    ("n3a", S7, "in"),     # not asking for effort — the whole vineyard
    ("n3b", S8, "out"),    # offering connection — hands on the branch
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"KJV at j1a {j1_start:.1f}s / j1b {j2_start:.1f}s; "
    ...
    OUT = "john-15_vine-and-branches.mp4"
    A_KBPS, MUX = 96, 20
