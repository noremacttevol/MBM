#!/usr/bin/env python3
"""Assemble Story Video #127 — The Strait Gate (Matthew 7:13-14).
...
exact KJV Matthew 7:13 (j1) and 7:14 (j2). "leadeth" SPOKEN-respelled
(ear-check caught /LED-eth/); captions keep the exact KJV spelling.

SACRED SILENCES: extended holds after j1 and j2 (the KJV lines) and after
n3a on s6 — the narrow gate in light, the pack's marked silence beat.

NO MUSIC BED of any kind (HUM PURGE law, 2026-07-16): audio is narration +
intentional silence only.

Bright-day video: caption box black@0.60 (pack directive; tuned to the
brightest frame).

Output: matthew-7_the-strait-gate.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-two-roads.jpeg"
S2 = "s2-the-wide-gate.jpeg"
S3 = "s3-the-narrow-path.jpeg"
S4 = "s4-the-gentle-warning.jpeg"
S5 = "s5-pointing-the-way.jpeg"
S6 = "s6-light-through-the-narrow-gate.jpeg"
S7 = "s7-step-through.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}
# n3a ends on the pack's marked sacred-silence beat (s6, the gate in light):
# it gets the long KJV-style hold too.
LONG_HOLD = {"j1", "j2", "n3a"}
CARD_TEXT = TEXT["card"]
...
BEATS = [
    ...
    ("n1", S2, "in"),
    ("n2", S3, "in"),
    ("j1", S4, "in"),      # KJV Matt 7:13 — the gentle warning face
    ("j2", S5, "in"),      # KJV Matt 7:14 — pointing the way
    ("n3a", S6, "in"),     # the narrow gate in light — SACRED SILENCE after
    ("n3b", S7, "in"),     # the open hand at dawn
]
...
def main():
    ...
    for name, still, zdir in BEATS:
        kjv = name in KJV
        gap = KJV_GAP if name in LONG_HOLD else GAP
    ...
    OUT = "matthew-7_the-strait-gate.mp4"
    A_KBPS, MUX = 96, 20
