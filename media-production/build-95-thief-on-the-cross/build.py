#!/usr/bin/env python3
"""Assemble Story Video #95 — The Thief on the Cross (Luke 23:39-43).
...
CONTENT-CARE — CRUCIFIXION (R) + MERCY (J): the three crosses are shown with
restraint (distance, no gore, no wounds in focus); the story IS mercy — a
last-minute plea answered. The thief's plea "Lord, remember me" is the
NARRATOR's (not Jesus, not cream). Closing card is invitation, never fear.
FACE LAW v3: Jesus face-shown (master-locked, only he wears cream).

ONE SACRED SILENCE: j1 — Luke 23:43 (To day shalt thou be with me in
paradise), Jesus voice, cream italic. n1 gets the draft's extended hold.
n0 recorded in halves so the seven beats fill the seven stills.
...
paths in segs/. Output: luke-23_thief-on-the-cross.mp4,
...
ST1 = "s1-three-crosses.jpeg"
ST2 = "s2-the-mocking-thief.jpeg"
ST3 = "s3-recognition-dawning.jpeg"
ST4 = "s4-remember-me.jpeg"
ST5 = "s5-to-day-in-paradise.jpeg"
ST6 = "s6-grace-in-the-darkest-place.jpeg"
ST7 = "s7-today-paradise.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Mercy"

BEATS = [
    ("n0a", ST1, "in"),          # two crucified with him — three crosses
    ("n0b", ST2, "in"),          # one sneered — the mocking thief
    ("n1", ST3, "in"),           # the other rebuked him — recognition; extended hold
    ("n2", ST4, "in"),           # he turned and asked — remember me
    ("n3", ST6, "in"),           # a dying man asking — grace in the darkest place
    ("j1", ST5, "in"),           # SACRED SILENCE — Luke 23:43, today in paradise
    ("n4", ST7, "in"),           # today — last-minute faith enough — today paradise
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap override: n1 = the draft's sacred-silence beat.
HOLD = {"n1": 1.60}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silence: j1 (Luke 23:43) at {start_of['j1']:.1f}s", flush=True)
    ...
    OUT = "luke-23_thief-on-the-cross.mp4"
    A_KBPS = 96
    MUX = 20
