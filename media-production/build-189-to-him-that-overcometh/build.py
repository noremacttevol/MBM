#!/usr/bin/env python3
"""Assemble MEMBER Verse Video #189 — To Him That Overcometh
(Revelation 3:20-21).
...
CONTENT-CARE: hope, no judgment gloom. FACE LAW v3: Jesus face-shown,
master-locked, only he wears cream, never looming.

TWO SACRED SILENCES: j1 — Rev 3:20 (I stand at the door and knock) and
j2 — Rev 3:21 (sit with me in my throne, the centerpiece), Jesus voice,
cream italic. n1 (he waits to be invited) gets the draft's extended hold.
s5 sup-with-him is a beat for j1's supper image with no narration line of
its own and is left unused rather than creating dead air.
...
paths in segs/. Output: revelation-3_to-him-that-overcometh.mp4,
...
ST1 = "s1-the-door-at-night.jpeg"
ST2 = "s2-the-knock.jpeg"
ST3 = "s3-recognition.jpeg"
ST4 = "s4-the-open-door.jpeg"
ST6 = "s6-the-shared-seat.jpeg"
ST7 = "s7-hand-near-the-latch.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Exaltation"

BEATS = [
    ("n0", ST1, "in"),           # the door at night — just a knock
    ("j1", ST2, "in"),           # SACRED SILENCE 1 — Rev 3:20, the knock
    ("n1", ST3, "in"),           # recognition — he waits; extended hold
    ("n2", ST4, "in"),           # the open door — the one who opens
    ("j2", ST6, "in"),           # SACRED SILENCE 2 — Rev 3:21, the shared seat
    ("n3", ST7, "in"),           # the throne Christ already won — the latch
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap overrides: n1 = the draft's sacred-silence beat (he waits);
# n3 = a longer breath before the card.
HOLD = {"n1": 1.60, "n3": 1.90}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silences: j1 (Rev 3:20) at {start_of['j1']:.1f}s, j2 (3:21) at {start_of['j2']:.1f}s", flush=True)
    ...
    OUT = "revelation-3_to-him-that-overcometh.mp4"
    A_KBPS = 96
    MUX = 20
