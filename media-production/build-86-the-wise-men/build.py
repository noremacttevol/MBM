#!/usr/bin/env python3
"""Assemble Story Video #86 — The Wise Men (Matthew 2:1-12).
...
Jesus is a young CHILD here (never the adult master face). Narrator
en-US-AndrewNeural; the wise-men KJV lines (Matt 2:2 = j1, Matt 2:11 = j2)
are carried by the SCRIPTURE voice (en-US-ChristopherNeural), cream-italic,
sacred holds. j1 lands at Herod's hall (n0/n1 merged so the question does).
The child-with-mother reveal (n2b) holds with the sacred pause. n3 reworded
to plain meaning (Translation Law — the pack's original echoed j2's gifts).
CONTENT-CARE: GREEN — wonder and worship; Herod troubled, never menacing.

Warm nativity frames: caption box black@0.60 (brightest-frame, #40).

Output: matthew-2_the-wise-men.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-star-in-the-east.jpeg"
S2 = "s2-herods-hall.jpeg"
S3 = "s3-the-star-goes-before.jpeg"
S4 = "s4-the-child-with-his-mother.jpeg"
S5 = "s5-they-fell-down-and-worshipped.jpeg"
S6 = "s6-gold-frankincense-myrrh.jpeg"
S7 = "s7-another-way-home.jpeg"
S8 = "s8-the-nations-found-their-king.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}            # cream-italic wise-men verses (Matt 2:2, 2:11)
FLOW = {"n4a"}                # narration line join: tight flow gap
SACRED = {"j1", "n2b", "j2"}  # both verse holds + the child-with-mother reveal hold
CARD_TEXT = ("Strangers from far away recognized him first. The door is "
             "open — you're invited to worship too.")

BEATS = [
    ("n0", S1, "in"),      # travelers from the east follow his star (the star)
    ("j1", S2, "in"),      # KJV Matt 2:2: where is he born King of the Jews (Herod's hall)
    ("n2a", S3, "in"),     # led to a house in Bethlehem (the star goes before)
    ("n2b", S4, "in"),     # there he was, with his mother (child with his mother) [sacred pause after]
    ("n3", S5, "in"),      # the wise men knelt before the child (they worshipped)
    ("j2", S6, "in"),      # KJV Matt 2:11: gold, frankincense, myrrh (the gifts)
    ("n4a", S7, "in"),     # warned in a dream, home another way (another way home)
    ("n4b", S8, "out"),    # the nations came to bow to the King (nations found their King)
]
...
CARD_HOLD = 2.0    # Cameron denial #86 (2026-07-18): the old 13s pad left dead
                   # air after the closing question. End ~2s after the voice.
                   # Video still runs well over 60s without padding the tail.
...
def main():
    ...
    OUT = "matthew-2_the-wise-men.mp4"
    A_KBPS, MUX = 96, 20
