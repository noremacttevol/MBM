#!/usr/bin/env python3
"""Assemble Video #122 — The Mote and the Beam (Matthew 7:1-5).
...
TWO SACRED SILENCES (music bed dies to true silence for each, separately):
  jvA   Matt 7:1-2   "Judge not, that ye be not judged..."
  jvB   Matt 7:5     "Thou hypocrite, first cast out the beam out of thine own eye..."
No-Dead-Air is unaffected: the narrator never stops, only the music does.

Output: matthew-7_mote-and-beam.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S2 = "s2-the-critical-eye.jpeg"
S3 = "s3-the-tiny-mote.jpeg"
S4 = "s4-the-beam-revealed.jpeg"
S5 = "s5-blundering-about.jpeg"
S6 = "s6-the-moment-he-sees-it.jpeg"
S7 = "s7-casting-out-the-beam.jpeg"
S8 = "s8-now-he-sees-clearly.jpeg"
S9 = "s9-gently-helping.jpeg"
S10 = "s10-two-brothers-at-peace.jpeg"
...
BEATS = [
    ("n1", S1, "in"),
    ("jvA", S2, "in"),          # Judge not, that ye be not judged — silence 1
    ("n2", S3, "in"),
    ("n3", S4, "in"),
    ("n4", S5, "in"),
    ("n5", S6, "in"),
    ("jvB", S7, "in"),          # Thou hypocrite, first cast out the beam — silence 2
    ("n6", S8, "in"),
    ("n7", S9, "in"),
    ("n8", S10, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (Judge not): jvA at {start_of['jvA']:.1f}s", flush=True)
    print(f"sacred silence 2 (first cast out the beam): jvB at {start_of['jvB']:.1f}s", flush=True)
    ...
    # jvA (beat 2) and jvB (beat 7) are SEPARATE beats; the music bed dies to true silence
    ...
    OUT = "matthew-7_mote-and-beam.mp4"
    A_KBPS = 96
    MUX = 20
