#!/usr/bin/env python3
"""Assemble Story Video #164 — The Rainbow Covenant (Genesis 8:20-9:17).
...
is the exact spoken text of its segment. KJV (God's) lines render cream italic.

Face-never (the #1 Law): no divine figure is ever rendered anywhere — God is voice
only in this story; no Jesus appears (gate PASS).

TWO SACRED SILENCES (the music bed dies to true silence for both):
  jv13  Gen 9:13   "I do set my bow in the cloud" — the verse card
  jv16  Gen 9:16   "I will look upon it, that I may remember" — the Seed
A warm bed carries the story in, dies before the bow is hung, returns for the
war-bow gem, dies again before "I will look upon it," then returns for the
resolution and the invitation.
...
CAPTION BOX: daylight story (bright midday altar scene is the lightest frame) —
box alpha 0.58 per the §4b brightest-frame rule.
...
Output: genesis-9_the-rainbow-covenant.mp4, 1080x1920 H.264 30fps, <30MB.
"""
...
S1 = "s1-the-ark-at-rest.jpeg"
S2 = "s2-first-steps.jpeg"
S3 = "s3-building-the-altar.jpeg"
S4 = "s4-the-offering.jpeg"
S5 = "s5-the-gathering-clouds.jpeg"
S6 = "s6-god-speaks.jpeg"
S7 = "s7-never-again.jpeg"
S8 = "s8-the-bow-in-the-cloud.jpeg"
S9 = "s9-faces-lifted.jpeg"
S10 = "s10-god-remembers.jpeg"
S11 = "s11-every-rain-since.jpeg"
S12 = "s12-the-promise-keeper.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"jv22", "jv9", "jv11", "jv13", "jv16"}

BEATS = [
    ("n1", S1, "in"),
    ("n2", S2, "in"),
    ("n3", S3, "in"),
    ("jv22", S4, "in"), ("n4", S4, "out"),
    ("n5", S5, "in"),
    ("jv9", S6, "in"), ("n6", S6, "out"),
    ("jv11", S7, "in"), ("n7", S7, "out"),
    ("jv13", S8, "in"),                      # SACRED SILENCE 1 — the bow is hung
    ("n8", S9, "in"),
    ("jv16", S10, "in"), ("n9", S10, "out"),  # SACRED SILENCE 2 — God remembers
    ("n10", S11, "in"),
    ("n11", S12, "in"),
]
...
def main():
    ...
    print(f"sacred silence 1 (the bow is hung): jv13 at {start_of['jv13']:.1f}s",
          flush=True)
    print(f"sacred silence 2 (God remembers): jv16 at {start_of['jv16']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["jv13"] + spoken["jv13"]
    p2_end = start_of["jv16"] + spoken["jv16"]
    beds = [
        (0.0, start_of["jv13"] - 1.2, "b"),
    ...
    OUT = "genesis-9_the-rainbow-covenant.mp4"
    A_KBPS, MUX = 96, 20
