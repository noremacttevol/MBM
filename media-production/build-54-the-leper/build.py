#!/usr/bin/env python3
"""Assemble Video #54 — The Leper Made Clean (Mark 1:40-45).
...
generation). He appears in s2-s7 and s9; s1 (the outcast) and s8 (the man publishing it)
have no divine figure. Captions are VERBATIM spoken text from make_narration.SEGMENTS;
his single KJV line (jv41, "I will; be thou clean") renders in cream italic and is the one
sacred silence. The leper's own plea is reported by the narrator in the plain white style.

CARE FLAGS: R (RESTRAINT). Leprosy shown gently (pale skin, wrapped hands), never gory.
The heart is the touch; the hope-beat is the clean, restored man (s5-s6).
...
Output: mark-1_the-leper.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <30MB.
"""
import os
import shutil
import subprocess
...
S1 = "s1-the-outcast.jpeg"
S2 = "s2-if-thou-wilt.jpeg"
S3 = "s3-moved-with-compassion.jpeg"
S4 = "s4-be-thou-clean.jpeg"
S5 = "s5-the-leprosy-departed.jpeg"
S6 = "s6-made-whole.jpeg"
S7 = "s7-tell-no-man.jpeg"
S8 = "s8-he-published-it.jpeg"
S9 = "s9-they-came-to-him.jpeg"
...
BEATS = [
    ...
    ("n2", S2, "in"),
    ("n3", S3, "in"),
    ("jv41", S4, "in"),                         # I WILL; BE THOU CLEAN — full silence
    ("n4", S5, "in"),
    ("n5", S6, "in"),
    ("n6", S7, "in"),
    ("n7", S8, "in"),
...
def main():
    ...
    print(f"sacred silence (be thou clean): jv41 at {start_of['jv41']:.1f}s", flush=True)
    ...
    #      sacred silence (jv41, the touch) and returns after it ----
    p1_end = start_of["jv41"] + spoken["jv41"]
    beds = [
        (0.0, start_of["jv41"] - 1.2, "b"),
        (p1_end + 1.0, card_start - 0.8, "a"),
    ]
    print(f"music: bed out before jv41 (ends {p1_end:.1f}s); returns after the touch",
          flush=True)
