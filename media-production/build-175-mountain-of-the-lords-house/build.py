#!/usr/bin/env python3
"""Assemble Story Video #175 — The Mountain of the LORD's House
(Isaiah 2:2-3).
...
Jesus does NOT appear (Isaiah's prophecy of the last days). Narrator
en-US-AndrewNeural; TWO exact-KJV verses (Isaiah 2:2 = s1, Isaiah 2:3 =
s2a+s2b) carried by the SCRIPTURE voice (en-US-ChristopherNeural, build-161
precedent), cream-italic. Isaiah 2:3 is split at its colon and flows across
two stills (s2a s3, s2b s4); s1 and s2b hold with the sacred gap, and the
pack's sacred pause also follows n1. n2 is split across the last two stills
so the word-going-out finale (s7) lands on "everyone, everywhere".
CONTENT-CARE: peace and invitation only, no war/violence.

Bright dawn-vista frames: caption box black@0.60 (brightest-frame, #40).

Output: isaiah-2_mountain-of-the-lords-house.mp4, 1080x1920, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-mountain-at-dawn.jpeg"
S2 = "s2-beginning-to-walk.jpeg"
S3 = "s3-faces-upward.jpeg"
S4 = "s4-the-path-upward.jpeg"
S5 = "s5-the-city-of-peace.jpeg"
S6 = "s6-many-peoples-listening.jpeg"
S7 = "s7-the-word-going-out.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1", "s2a", "s2b"}    # cream-italic verses (Isaiah 2:2 and 2:3)
FLOW = {"s2a", "n2a"}         # Isa 2:3 internal join + the n2 sentence join: tight flow gap
SACRED = {"s1", "s2b", "n1"}  # verse-2 end, verse-3 end + the pack's sacred pause after n1
CARD_TEXT = ("The mountain is open to you. Come, and walk in his paths.")

BEATS = [
    ("n0", S1, "in"),      # the mountain rising at dawn
    ("s1", S2, "in"),      # KJV Isa 2:2: mountain established, all nations flow [sacred hold]
    ("s2a", S3, "in"),     # KJV Isa 2:3: come, let us go up... walk in his paths (faces upward)
    ("s2b", S4, "in"),     # KJV Isa 2:3: for out of Zion... (the path upward) [sacred hold]
    ("n1", S5, "in"),      # drawn by invitation (the city of peace) [sacred pause after]
    ("n2a", S6, "in"),     # God's teaching goes out (many peoples listening)
    ("n2b", S7, "out"),    # to everyone, everywhere (the word going out)
]
...
def main():
    ...
    OUT = "isaiah-2_mountain-of-the-lords-house.mp4"
    A_KBPS, MUX = 96, 20
