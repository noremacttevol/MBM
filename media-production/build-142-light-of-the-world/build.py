#!/usr/bin/env python3
"""Assemble Story Video #142 — "I Am the Light of the World"
(John 8:12 + John 9:5).

PHASE-1 STILLS-ONLY (Law E): 7 stills, 6 cut in as beats + s7 as the pre-card
breath, Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW §5),
cream card.

Face-shown Jesus locked to master (s1/s2/s4/s6). Blind man look-locked (black
hair/beard, grey tunic), healed gently — no gore. Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV John 8:12 (j1),
9:5 (j2). Ear-check 1.00 all segments.

SACRED SILENCES: the marked s3 hold after n1 (the lit path) and the marked s5
hold after n3 (the eyes opening). KJV holds after j1 and j2. s7 (a torch
lighting every corner) is the sanctioned short breath before the card.

NO MUSIC BED (HUM PURGE law). Mixed bright-court / dim-room video: caption box
black@0.60 (lightest-frame rule per §5b).

Output: john-8_i-am-the-light-of-the-world.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-into-the-noise.jpeg"
ST2 = "s2-the-declaration.jpeg"
ST3 = "s3-a-light-that-walks-with-you.jpeg"
ST4 = "s4-the-hand-and-the-closed-eyes.jpeg"
ST5 = "s5-eyes-opening.jpeg"
...
BEATS = [
    ("n0", ST1, "in"),     # into the noise (temple courts)
    ("j1", ST2, "in"),     # KJV John 8:12 — the declaration
    ("n1", ST3, "in"),     # a light that walks with you [SILENCE]
    ("n2", ST4, "in"),     # meeting the man born blind
    ("j2", ST6, "in"),     # KJV John 9:5 — tender and certain
    ("n3", ST5, "in"),     # the eyes opening — proved it [SILENCE]
]

# s7 (a torch lighting every corner): the sanctioned short breath.
BREATH_STILL = ST7
BREATH_DUR = 1.8
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after j1 / n1 / j2 / n3 + breath on s7; "
    ...
    OUT = "john-8_i-am-the-light-of-the-world.mp4"
    A_KBPS, MUX = 96, 20
