#!/usr/bin/env python3
"""Assemble Story Video #85 — Shepherds and Angels (Luke 2:8-20).

PHASE-1 STILLS-ONLY (Law E): 9 stills, 10 beats (s4 reused for the host intro
+ its KJV), Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW
§5), cream card.

GREEN nativity. No adult Christ; the newborn babe only (his sleeping face may
show — the face-never law is Jesus-adult only). Angel light staged as a broad
flood / whole-sky glory, never a ring around a head. SHEPHERDS/MARY/JOSEPH/
NEWBORN locks held; only-Jesus-cream is N/A here (no adult Jesus) but the
angels are white and everyone else earth-toned. Two-voice: narrator
en-US-AndrewNeural; the angelic KJV read by the scripture voice
en-US-ChristopherNeural (Luke 2:10-11 j1, 2:14 j2; the Jesus voice is reserved
for Jesus's own words, and he does not speak here). Narration written by
ASSEMBLY-B from the prompt sheet (no draft existed). Ear-check pass.

SACRED SILENCES: the marked s3 hold on j1 (fear not) and the marked s6 hold
after n4 (the stable threshold). KJV holds on both angelic lines.

NO MUSIC BED (HUM PURGE law). Night scenes with bright angelic light: caption
box black@0.60 (lightest-frame rule per §5b).

Output: luke-2_shepherds-and-angels.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-night-watch.jpeg"
ST2 = "s2-the-angel.jpeg"
ST3 = "s3-fear-not.jpeg"
ST4 = "s4-heavenly-host.jpeg"
ST5 = "s5-let-us-go.jpeg"
ST6 = "s6-they-found-him.jpeg"
ST7 = "s7-the-manger.jpeg"
...
def main():
    ...
    OUT = "luke-2_shepherds-and-angels.mp4"
    A_KBPS, MUX = 96, 20
