#!/usr/bin/env python3
"""Assemble Story Video #25 — The Wheat and the Tares (Matthew 13:24-30, 36-43).
...
Music: a warm detuned-pair bed opens and fades to a reverent hush just before j1
(the farmer's patient reply "Let both grow together until the harvest" — the
heart of the parable, his mercy); a warm bed then returns just after j1 and runs
UNDER everything that follows (the harvest, j2 "the righteous shine forth as the
sun", and the close), fading out only before the closing card. No-Dead-Air: the
only silences >2.5s are the hush around the heart line and the card's tail.

Output: matthew-13_wheat-and-tares.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264
30fps, <25MB.
"""
import os
import subprocess
...
S1 = "s1-sowing.jpeg"
S2 = "s2-enemy-night.jpeg"
S3 = "s3-both-appear.jpeg"
S4 = "s4-servants-ask.jpeg"
S5 = "s5-let-grow.jpeg"
S6 = "s6-grow-together.jpeg"
S7 = "s7-harvest.jpeg"
S8 = "s8-shine.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}   # Matthew 13:29-30 (j1), 13:43 (j2) — Jesus voice
PEAK = {"j1"}        # "Let both grow together until the harvest" — hush lands here
...
def main():
    ...
    OUT = "matthew-13_wheat-and-tares.mp4"
