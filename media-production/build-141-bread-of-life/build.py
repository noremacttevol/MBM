#!/usr/bin/env python3
"""Assemble Story Video #141 — "I Am the Bread of Life" (John 6:35,48,51).
...
Face-shown Jesus locked to master (s1/s4/s6). Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV John 6:35 (j1),
6:48 (j2), 6:51 (j3). Ear-check pass — "live for ever" reads right in j3.
The "my flesh" line is handled gently (no graphic imagery — the broken loaf
carries it).

Each image matched to its words: j1 (bread of life) → the miracle's remnant
loaves (s2); j2 (I am that bread) → the face (s4); j3 (living bread / my
flesh given for the world) → the solemn face (s6, the pack's marked SILENCE);
n3 (given like bread broken) → the offered broken bread (s7).

SACRED SILENCES: the marked s3 hold after n1 (listening faces) and the s6
hold on j3 (the solemn giving). All KJV lines get reverent holds.

NO MUSIC BED (HUM PURGE law). Bright lakeshore video: caption box black@0.60.

Output: john-6_i-am-the-bread-of-life.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-they-wanted-more-bread.jpeg"
ST2 = "s2-the-miracles-remnant.jpeg"
ST3 = "s3-hungry-for-more-than-food.jpeg"
ST4 = "s4-i-am-the-bread.jpeg"
ST5 = "s5-bread-that-lasts.jpeg"
...
BEATS = [
    ("n0", ST1, "in"),     # they wanted more bread
    ("j1", ST2, "in"),     # KJV John 6:35 — the miracle's remnant
    ("n1", ST3, "in"),     # hungry for more than food [SILENCE]
    ("j2", ST4, "in"),     # KJV John 6:48 — I am that bread
    ("n2", ST5, "in"),     # bread that lasts (vs manna)
    ("j3", ST6, "in"),     # KJV John 6:51 — given for the world [SILENCE]
    ("n3", ST7, "in"),     # come hungry — the broken bread offered
]
...
def main():
    ...
    OUT = "john-6_i-am-the-bread-of-life.mp4"
    A_KBPS, MUX = 96, 20
