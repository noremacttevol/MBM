#!/usr/bin/env python3
"""Assemble Story Video #177 — "Make me a sanctuary; that I may dwell
...
No divine figure is depicted (God appears only as the luminous cloud).
Narrator en-US-AndrewNeural; the exact-KJV verse (Exodus 25:8 = s1) is
carried by the SCRIPTURE voice (en-US-ChristopherNeural, build-161
precedent), cream-italic, sacred hold. The how (n2) plays before the why
(n1) so the building stills precede the dwell/near payoff; the pack's sacred
pause follows n2b. CONTENT-CARE: reverent; God is the cloud/presence, never
a figure. n4 ("would take a face") is abstract — the still holds no figure.

Warm desert/tent frames: caption box black@0.60 (brightest-frame, #40).

Output: exodus-25_make-me-a-sanctuary.mp4, 1080x1920, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-camp-at-dusk.jpeg"
S2 = "s2-moses-on-the-mountain.jpeg"
S3 = "s3-the-pattern-in-the-dust.jpeg"
S4 = "s4-the-craftsmen.jpeg"
S5 = "s5-the-tent-raised.jpeg"
S6 = "s6-i-am-near.jpeg"
S7 = "s7-the-camp-at-night.jpeg"
S8 = "s8-the-open-tent-flap.jpeg"
S9 = "s9-dawn-on-the-camp.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1"}                  # cream-italic verse (Exodus 25:8)
FLOW = {"n2a", "n1a", "n3a"}  # each split narration line flows into its second half
SACRED = {"s1", "n2b"}        # verse hold + the pack's sacred pause after n2
CARD_TEXT = ("He wanted to live among his people then. He wants to live "
             "among you now.")

BEATS = [
    ("n0", S1, "in"),      # Israel camped, the strange instruction (camp at dusk)
    ("s1", S2, "in"),      # KJV Ex 25:8: make me a sanctuary (Moses on the mountain) [sacred hold]
    ("n2a", S3, "in"),     # he told them exactly how (the pattern in the dust)
    ("n2b", S4, "in"),     # every detail meant: I am near (the craftsmen) [sacred pause after]
    ("n1a", S5, "in"),     # not for his sake, for theirs (the tent raised)
    ("n1b", S6, "in"),     # so he could dwell among them (I am near)
    ("n3a", S7, "in"),     # the pattern carried by a people on the move (camp at night)
    ("n3b", S8, "in"),     # yet the promise was fixed (the open tent flap)
    ("n4", S9, "out"),     # centuries later takes a face; here a tent (dawn on the camp)
]
...
def main():
    ...
    OUT = "exodus-25_make-me-a-sanctuary.mp4"
    A_KBPS, MUX = 96, 20
