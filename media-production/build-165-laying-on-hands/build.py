#!/usr/bin/env python3
"""Assemble Story Video #165 — Laying on of hands for the Holy Ghost (Acts 8:14-17).
MEMBER shelf verse-video → Gospel Library topic: Laying On of Hands.
...
Face-never (the #1 Law): God / the Holy Ghost is never rendered anywhere — heaven is
shown only as warm descending light (never a dove, never a figure); Peter and John are
the human actors (gate PASS — no divine proper noun in any shot).

TWO SACRED SILENCES (the music bed dies to true silence for both):
  kv14  Acts 8:14  "...they sent unto them Peter and John:" — authority is sent
  kv17  Acts 8:17  "...they laid their hands on them, and they received the Holy Ghost."
A warm bed carries it in, dies before the sending verse, returns, dies again before
the laying-on-of-hands verse, then returns for the invitation.
...
Output: acts-8_laying-on-of-hands.mp4, 1080x1920 H.264 30fps, <30MB.
"""
...
S1 = "s1-samaria-believes.jpeg"
S2 = "s2-news-to-jerusalem.jpeg"
S3 = "s3-peter-and-john-sent.jpeg"
S4 = "s4-they-pray.jpeg"
S5 = "s5-baptized-but-waiting.jpeg"
S6 = "s6-laying-on-hands.jpeg"
S7 = "s7-filled.jpeg"
S8 = "s8-offered-to-you.jpeg"
...
GL_POINTER = "Learn more — Gospel Library: Laying On of Hands"

BEATS = [
    ("n1", S1, "in"),
    ("n2", S2, "in"),
    ("kv14", S3, "in"),                      # SACRED SILENCE 1 — they sent Peter and John (Acts 8:14)
    ("n3", S4, "in"),
    ("n4", S5, "in"),
    ("n5", S6, "in"),
    ("kv17", S6, "out"),                     # SACRED SILENCE 2 — laid hands, received Holy Ghost (Acts 8:17, named)
    ("n6", S7, "in"),
    ("n7", S8, "in"),
]
...
def build_card(dur, text):
    ...
    vf = (f"drawtext=fontfile={SERIF}:textfile={tf}:fontsize=52:"
          ...
          f"drawtext=fontfile={SERIF_BI}:textfile={pf}:fontsize=32:"
...
def main():
    ...
    print(f"sacred silence 1 (sent Peter and John, Acts 8:14): kv14 at {start_of['kv14']:.1f}s",
          flush=True)
    print(f"sacred silence 2 (laid on hands, Acts 8:17): kv17 at {start_of['kv17']:.1f}s",
          flush=True)
    ...
    p1_end = start_of["kv14"] + spoken["kv14"]
    p2_end = start_of["kv17"] + spoken["kv17"]
    beds = [
        (0.0, start_of["kv14"] - 1.2, "b"),
    ...
    OUT = "acts-8_laying-on-of-hands.mp4"
    A_KBPS, MUX = 96, 20
