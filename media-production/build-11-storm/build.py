...
FULL REDO 2026-07-13 (Machine A), stills-only (Law E): the previous cut carried
two Veo MOTION clips (storm + a hooded-Jesus calm) and Jesus's hair flipped
short/long shot to shot, and one still put him on the water OUTSIDE the boat.
This build is pictures + narration only, with the five Jesus shots (s4,s5,s6,s7,
s8) regenerated to ONE consistent look (tan man, long dark wavy hair) and staged
so he is clearly IN the boat and never the obsessive center.

CAPTIONS ARE VERBATIM. KJV (Jesus voice) lines render in cream italic:
j0 (Mark 4:35), j1 (4:39 "Peace, be still"), j2 (4:40).

Music: a warm bed opens over the evening/voyage/storm and fades to FULL SILENCE
just before j1 ("Peace, be still"), which lands in true silence; a warm bed
returns after and is out before the closing card. No-Dead-Air: the only silences
>2.5s are the hush around j1 and the card's tail.

Output: mark-4_calming-the-storm.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import shutil
import subprocess
...
S8 = "s8-turned-to-them.jpeg"
S9 = "s9-what-manner-of-man.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j0", "j1", "j2"}
PEAK = "j1"   # "Peace, be still" — music goes to full silence here
CARD_TEXT = ("The same Jesus is in your boat. Bring him your storm — and let "
             "him speak his peace.")

# BEATS: (segment_name, still, zoom_dir). One still-drift beat per narration
# segment. n10 is the closing card (spoken over the cream card), handled below.
BEATS = [
    ("n0", S1, "in"), ("j0", S1, "out"),
    ("n1", S2, "in"),
    ("n2", S3, "in"),
...
# ---- CAPTION SYSTEM v2 (Cameron, 2026-07-15) — ported from build-48 ----
...
# in sync with the narration (timed proportionally across the spoken audio).
...
def main():
    ...
    card_start = t
    audio_place.append(("audio/n10.mp3", card_start + LEAD))
    total = t + card_vdur
    ...
    OUT = "mark-4_calming-the-storm.mp4"
