...
| 06 | Two Sons | matthew-21_two-sons | CLAIMED — 2026-07-11 — Machine A (Dev). Cameron's feedback: the cut "was okay" but still had ONE motion clip (s04 "he went" pruning clip) — replacing it with the `went.jpeg` still (minimal fix), rebuilding stills-only, keeping the rest he approved. |
| 07 | Peter walks on water | matthew-14_peter-water | CLAIMED — 2026-07-11 — Elli's Windows laptop (Cameron gave direct picture feedback, reassigned from Machine C). Fixing 3 pictures per Cameron: (1) s5 walk — Peter regenerated seen from BEHIND going toward a distant Jesus (Peter's face not needed, Jesus too far to show a face); (2) sinking beat — new "Peter sinking alone, no rescuing hand" still so the hand-grab (s7-sink-v5) no longer appears too early; (3) s9 walk-back — regenerated with the rest of the disciples visible in the boat. Then Windows stills-only rebuild + QC. |
