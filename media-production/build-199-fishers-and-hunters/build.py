#!/usr/bin/env python3
"""Assemble MEMBER Verse Video #199 — Fishers and Hunters (Jeremiah 16:16).
...
CONTENT-CARE — J-STORY (judgment): mercy is spoken aloud (n2, n4) and the
closing card is an invitation ("let yourself be found"), never a fear-question.
No gore, no embodied punisher — the search is nets and hunt imagery only.
Jesus does not appear (Jeremiah); the Fisher-of-men reframe is narrator gloss.

ONE SACRED SILENCE: s1 — Jeremiah 16:16 (fishers and hunters, the centerpiece),
scripture voice, cream italic. Only five stills, so the hunters still is HELD
across the KJV verse and its judgment gloss n1 (in-drift then out-drift).
...
paths in segs/. Output: jeremiah-16_fishers-and-hunters.mp4,
...
ST1 = "s1-the-fishers.jpeg"
ST2 = "s2-the-hunters.jpeg"
ST3 = "s3-found-by-an-open-hand.jpeg"
ST4 = "s4-the-weight-of-love-in-the-warning.jpeg"
ST5 = "s5-the-search-reframed.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1"}

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Gathering of Israel"

BEATS = [
    ("n0", ST1, "in"),           # Jeremiah's hard word — the fishers
    ("s1", ST2, "in"),           # SACRED SILENCE — Jer 16:16, the hunters
    ("n1", ST2, "out"),          # the nets and the hunt find everyone (held still)
    ("n2", ST3, "in"),           # mercy underneath — found by an open hand
    ("n3", ST4, "in"),           # He won't let them slip — weight of love
    ("n4", ST5, "in"),           # the picture flips to hope — the search reframed
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap override: n1 = the draft's sacred-silence beat after the verse.
HOLD = {"n1": 1.60}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred silence: s1 (Jer 16:16) at {start_of['s1']:.1f}s", flush=True)
    ...
    OUT = "jeremiah-16_fishers-and-hunters.mp4"
    A_KBPS = 96
    MUX = 20
