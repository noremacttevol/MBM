#!/usr/bin/env python3
"""Assemble Story Video #147 — Joseph Forgives His Brothers
(Genesis 45:5; 50:20).
...
Old Testament — Jesus does NOT appear. Narrator en-US-AndrewNeural; Joseph's
two exact-KJV lines (Gen 45:5, 50:20) carried by the SCRIPTURE voice
(en-US-ChristopherNeural, build-161 precedent — the narrator never quotes
KJV), rendered cream-italic with the sacred KJV_GAP. Gen 45:5 lands on the
embrace still; Gen 50:20 ("to save much people alive") on grain-for-the-
hungry. CONTENT-CARE: reconciliation, no violence in frame.

Bright daylight/palace-light story — caption box black@0.60 (brightest-frame
rule, #40 lesson).

Output: genesis-45_joseph-forgives.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-caravan-to-egypt.jpeg"
ST2 = "s2-before-the-governor.jpeg"
ST3 = "s3-i-am-joseph.jpeg"
ST4 = "s4-the-embrace.jpeg"
ST5 = "s5-grain-for-the-hungry.jpeg"
ST6 = "s6-peace-over-their-fear.jpeg"
ST7 = "s7-hands-clasped.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1", "s2"}   # scripture-voice verses (cream italic)
# Cameron 2026-07-18: "only Jesus's words in red." These verses are
# Joseph (Gen 45:5, 50:20) speaking/writing — NOT Jesus — so they get the narrator voice and a
...
CARD_TEXT = ("What others meant for harm, God can mean for good. Let it go.")

BEATS = [
    ("n0a", ST1, "in"),    # sold into slavery — the caravan to Egypt
    ("n0b", ST2, "in"),    # second only to Pharaoh — before the governor
    ("n1", ST3, "in"),     # the reveal — his face (sacred hold)
    ("s1", ST4, "in"),     # KJV Gen 45:5 — over the embrace
    ("n2", ST6, "in"),     # their father died; fear of revenge
    ("s2", ST5, "in"),     # KJV Gen 50:20 — to save much people alive
    ("n3", ST7, "out"),    # hands clasped — evil turned into rescue
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"KJV at s1 {j1_start:.1f}s / s2 {j2_start:.1f}s; "
    ...
    OUT = "genesis-45_joseph-forgives.mp4"
    A_KBPS, MUX = 96, 20
