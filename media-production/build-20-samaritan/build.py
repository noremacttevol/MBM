#!/usr/bin/env python3
"""Assemble Story Video #20 — The Good Samaritan (Luke 10:25-37).
...
Face-never (#18): this is a parable, so every character in it is shown fully;
the Lord appears ONLY in the bookend stills (s1, s7, s8) as a seated storyteller
seen from behind, his face never shown.

Timing is COMPUTED from measured mp3 durations (no hand offsets to drift).
Music: a warm detuned-pair bed opens and fades to FULL SILENCE just before n9
("...moved with compassion" — the emotional peak), so compassion lands in true
silence; a warm bed returns for the practical-care beats, then fades out before
j1 so the entire turn-around and sending (j1 "which of these three...", the
scholar's answer, j2 "Go, and do thou likewise.") plays in reverent quiet.

Output: luke-10_samaritan.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-question.jpeg"
S2 = "s2-half-dead.jpeg"
S3 = "s3-crossed-over.jpeg"
S4 = "s4-compassion.jpeg"
S5 = "s5-own-animal.jpeg"
S6 = "s6-the-inn.jpeg"
S7 = "s7-turned-around.jpeg"
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"peak (compassion silence) at {peak_start:.1f}s; "
          f"j1 (quiet turn) at {j1_start:.1f}s", flush=True)
    ...
    #   bed a : opening -> silent just before the compassion peak (n9)
    #   bed b : returns after the peak -> out before j1, so the whole
    #           turn-around + sending (j1, n14, j2, n15) + card is quiet.
    beds = [
        (0.0, peak_start - 1.2, "a"),
        (peak_start + audio_dur["n9"] + 1.0, j1_start - 1.2, "b"),
    ]
