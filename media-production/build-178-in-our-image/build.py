#!/usr/bin/env python3
"""Assemble Story Video #178 — "Let us make man in our image, after our
...
No figure of Christ is depicted (OT creation). Narrator en-US-AndrewNeural;
TWO exact-KJV verses (Genesis 1:26 = s1, Genesis 1:27 = s2) carried by the
SCRIPTURE voice (en-US-ChristopherNeural, build-161 precedent), cream-italic,
each whole on its own still with the sacred hold. Narration is reordered so
the creation act (n3) and dominion (n2) precede the image-meaning (n1); the
pack's sacred pause follows n2. The Spirit and God's shaping are shown as
light only — no divine figure; Adam/Eve are ordinary humans, not the master
face. CONTENT-CARE: reverent, no violence.

Warm creation frames: caption box black@0.60 (brightest-frame, #40).

Output: genesis-1_in-our-image.mp4, 1080x1920, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-the-formless-deep.jpeg"
S2 = "s2-the-spirit-over-the-waters.jpeg"
S3 = "s3-shaped-by-light.jpeg"
S4 = "s4-the-first-breath.jpeg"
S5 = "s5-the-steward.jpeg"
S6 = "s6-image-bearers.jpeg"
S7 = "s7-and-it-was-good.jpeg"
S8 = "s8-the-image-of-god.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1", "s2"}            # cream-italic verses (Genesis 1:26 and 1:27)
FLOW = {"n1a"}                # the image-meaning line flows into its second half
SACRED = {"s1", "s2", "n2"}   # both verse holds + the pack's sacred pause after n2
CARD_TEXT = ("You are made in his image. That is worth more than you know.")

BEATS = [
    ("n0", S1, "in"),      # the start of all things, the counsel (the formless deep)
    ("s1", S2, "in"),      # KJV Gen 1:26: let us make man in our image (Spirit over the waters) [hold]
    ("s2", S3, "in"),      # KJV Gen 1:27: created man in his own image (shaped by light) [hold]
    ("n3", S4, "in"),      # formed from dust, breathed, came alive (the first breath)
    ("n2", S5, "in"),      # dominion, stewards not owners (the steward) [sacred pause after]
    ("n1a", S6, "in"),     # bearing something of God himself (image-bearers)
    ("n1b", S7, "in"),     # the capacity to know, choose, reflect him (and it was good)
    ("n4", S8, "out"),     # every person carries that dignity (the image of God)
]
...
def main():
    ...
    OUT = "genesis-1_in-our-image.mp4"
    A_KBPS, MUX = 96, 20
