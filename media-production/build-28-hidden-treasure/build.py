...
Phase-1 STILLS-ONLY + Face Law. Parable (no Jesus figure).
...
The one-verse parable used to play a single j1 audio across two stills; it is now
j1 + j1b — same words, same two pictures, same edit.
...
CARD = "card"
# PEAK: the beat the music bed dies for. j1 is the verse itself, the sacred seal.
PEAK = "j1"
...
def build_still(seg_id, src, dur, zdir, spoken_end, cap_text, speaker, first):
    ...
    tail = ",fade=t=in:st=0:d=1.2" if first else ""
...
def main():
    ...
    run([FF, "-y"] + inputs + ["-filter_complex", ";".join(filters), "-map", "[aout]",
         "-t", f"{total:.2f}", "-c:a", "aac", "-b:a", "160k", f"{S}/audio_mix.m4a"])

    probe = subprocess.run([FF, "-i", f"{S}/audio_mix.m4a", "-af", "ebur128", "-f", "null", "-"],
