#!/usr/bin/env python3
"""Assemble Story Video #97 — The Empty Tomb (Luke 24:1-9).
...
CONTENT-CARE: awe and hope — the crucifixion is referenced (n5a) but never
shown. Jesus does NOT appear (the tomb is empty, he is risen); the two figures
are angels (faces fine). NO KJV cream-italic here — the angels' words are the
narrator's paraphrase (not Jesus, not exact KJV), so all captions are white
serif. Sacred holds on the rolled-away stone (n1) and "he is risen" (n4).
...
paths in segs/. Output: luke-24_the-empty-tomb.mp4,
...
ST1 = "s1-spices-before-dawn.jpeg"
ST2 = "s2-the-stone-rolled-away.jpeg"
ST3 = "s3-the-empty-ledge.jpeg"
ST4 = "s4-fear-and-awe-in-the-doorway.jpeg"
ST5 = "s5-two-in-dazzling-clothing.jpeg"
ST6 = "s6-she-remembers.jpeg"
ST7 = "s7-running-into-sunrise.jpeg"
ST8 = "s8-death-undone.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = set()  # no exact-KJV Jesus line in this build

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Resurrection"

BEATS = [
    ("n0", ST1, "in"),           # spices before dawn
    ("n1", ST2, "in"),           # the stone rolled away — extended quiet hold
    ("n2a", ST3, "in"),          # the body was gone — the empty ledge
    ("n2b", ST4, "in"),          # confused and afraid — fear and awe
    ("n3", ST5, "in"),           # two in dazzling clothing
    ("n4", ST8, "in"),           # he is not here, he is risen — death undone; hold
    ("n5a", ST6, "in"),          # remember Galilee — she remembers
    ("n5b", ST7, "in"),          # and they remembered — running into sunrise
]

LEAD = 0.28
GAP = 0.65
KJV_GAP = 1.60
# Per-beat gap overrides: n1 = the draft's sacred hold (open tomb); n4 = a
# weightier pause on "he is risen".
HOLD = {"n1": 1.60, "n4": 1.60}
CARD_HOLD = 5.0
...
def main():
    ...
    print(f"sacred holds: n1 (stone) at {start_of['n1']:.1f}s, n4 (he is risen) at {start_of['n4']:.1f}s", flush=True)
    ...
    OUT = "luke-24_the-empty-tomb.mp4"
    A_KBPS = 96
    MUX = 20
