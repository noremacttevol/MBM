#!/usr/bin/env python3
"""Assemble Story Video #70 — The Temptations (Matt 4:1-11).
...
(Matt 4:4, 4:7, 4:10 — exact KJV). "live" SPOKEN-respelled (homograph law).
NO music bed of any kind (HUM PURGE 2026-07-16): narration + intentional silence only.
CONTENT-CARE (A,L): the adversary is PRESENCE, never portrait — never depicted, never
voiced (his words are narrator-paraphrased); Jesus is alone in every frame.

Beat map (L1's plan): each of the three temptations answered with exact KJV and a
sacred silence — j1 [SILENCE] s4, n3 reflects on s4-out; j2 [SILENCE] s6; j3 [SILENCE]
s8; the dawn-after (n6/n7) plays over s9. Long narrator beats swap their caption
chunks in sync over a single still.

Caption box black@0.55 — bare wilderness, verified on the lightest frame.

Output: matt-4_the-temptations.mp4, 1080x1920 H.264 30fps, <30MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-led-into-the-wilderness.jpeg"
S2 = "s2-forty-days.jpeg"
S3 = "s3-stones-like-loaves.jpeg"
S4 = "s4-not-by-bread-alone.jpeg"
S5 = "s5-the-pinnacle.jpeg"
S6 = "s6-thou-shalt-not-tempt.jpeg"
S7 = "s7-all-the-kingdoms.jpeg"
S8 = "s8-get-thee-hence.jpeg"
S9 = "s9-angels-ministered.jpeg"
...
BEATS = [
    ("n0", [(S1, "in")], None),    # led by the Spirit into the wilderness
    ("n1", [(S2, "in")], None),    # forty days, hungrier every day
    ("n2", [(S3, "in")], None),    # IF you are the Son of God — stones to bread
    ("j1", [(S4, "in")], None),    # "man shall not live by bread alone" [SILENCE]
    ("n3", [(S4, "out")], None),   # he did not argue — scripture, as a man
    ("n4", [(S5, "in")], None),    # the pinnacle — throw yourself down
    ("j2", [(S6, "in")], None),    # "thou shalt not tempt the Lord" [SILENCE]
    ("n5", [(S7, "in")], None),    # all the kingdoms — one bow, a shortcut
    ("j3", [(S8, "in")], None),    # "get thee hence, Satan" [SILENCE]
    ("n6", [(S9, "in")], None),    # the devil left; angels ministered — dawn
    ("n7", [(S9, "out")], None),   # why it matters: tempted in all points like us
]
...
def main():
    ...
    run([FF, "-y", "-i", f"{S}/video_silent.mp4", "-i", f"{S}/audio_mix.m4a",
        ...
        ["-pass", "2", "-maxrate", f"{vmax}k", "-bufsize", f"{vmax*2}k",
