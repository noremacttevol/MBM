#!/usr/bin/env python3
"""Assemble Story Video #139 — The Lamp on a Stand (Matt 5:14-16).
...
Face-shown Jesus locked to master (s1/s4). Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV Matt 5:14 (j1),
5:15 (j2), 5:16 (j3). Ear-check pass all segments — "hid" reads correctly in
the whole j1 clip (a short split clipped it to "hit", so j1 kept whole).

Each KJV image matched to its words: j1 (city set on a hill cannot be hid) →
the glowing night city (s3); j2 (candle under a bushel) → the bushel lifted
off the lamp (s5); j3 (let your light so shine) → the warm inviting face
(s4, the pack's marked SILENCE beat).

SACRED SILENCES: the marked s4 hold (carried by j3) and the hold after n2
(before the closing declaration). All KJV lines get reverent holds.

NO MUSIC BED (HUM PURGE law). Mixed bright/dark frames: caption box
black@0.60 (picked for the lightest still per §5b so white text never washes).

Output: matthew-5_lamp-on-a-stand.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-teaching-on-the-hillside.jpeg"
ST2 = "s2-onto-the-stand.jpeg"
ST3 = "s3-a-city-on-a-hill.jpeg"
ST4 = "s4-ye-are-the-light.jpeg"
ST5 = "s5-out-from-under-the-bushel.jpeg"
...
BEATS = [
    ("n0", ST1, "in"),     # teaching on the hillside
    ("j1", ST3, "in"),     # KJV Matt 5:14 — a city on a hill cannot be hid
    ("n1", ST2, "in"),     # onto the stand — light the room
    ("j2", ST5, "in"),     # KJV Matt 5:15 — out from under the bushel
    ("n2", ST6, "in"),     # let your light shine, look up [SILENCE after]
    ("j3", ST4, "in"),     # KJV Matt 5:16 — the inviting face [SILENCE]
    ("n3", ST7, "in"),     # light to every corner — lit on purpose
]
...
def main():
    ...
    OUT = "matthew-5_lamp-on-a-stand.mp4"
    A_KBPS, MUX = 96, 20
