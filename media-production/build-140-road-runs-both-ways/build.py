#!/usr/bin/env python3
"""Assemble Story Video #140 — The Road to the Far Country Runs Both Ways
(the Prodigal Son, Luke 15:11-32).

PHASE-1 STILLS-ONLY (Law E): 10 stills, 9 cut in as beats + s10 as the
pre-card breath, Ken Burns drift, caption-v2 bottom-band split/synced
(CAPTION LAW §5), cream card.

Parable told — Jesus is NEVER depicted; the one KJV line (Luke 15:11, the
teller's opening) is read by the scripture voice en-US-ChristopherNeural.
Characters look-locked: younger son (black curly hair, olive-green ragged
tunic), father (white hair/beard, red robe), older brother (brown beard,
blue tunic). CARE: pigs kept at a dignified distance, never disgust-framed;
older brother not villainized; the embrace is the mercy beat.

SACRED SILENCES: the marked holds after n1 (empty far country), on s4 after
n2 (came to himself), and on s7 after n4 (the embrace). KJV hold after j1.
s10 (two roads, one lit door) is the sanctioned breath before the card.

NO MUSIC BED (HUM PURGE law). Mixed day/dusk/lamp video: caption box
black@0.60 (lightest-frame rule per §5b).

Output: luke-15_road-runs-both-ways.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-restless-son.jpeg"
ST2 = "s2-taking-the-road.jpeg"
ST3 = "s3-the-far-country.jpeg"
ST4 = "s4-among-the-pigs.jpeg"
ST5 = "s5-the-decision.jpeg"
ST6 = "s6-the-long-road-back.jpeg"
ST7 = "s7-the-father-runs.jpeg"
ST8 = "s8-robe-ring-feast.jpeg"
ST9 = "s9-the-older-brother.jpeg"
ST10 = "s10-two-roads-one-door.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}   # Luke 15:11, scripture voice — cream italic
# Long holds: the KJV, the marked silence after n1, s4 after n2, s7 after n4.
LONG_HOLD = {"j1", "n1", "n2", "n4"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0", ST1, "in"),     # the restless son
    ("j1", ST2, "in"),     # KJV Luke 15:11 — a certain man had two sons
    ("n1", ST3, "in"),     # the far country [SILENCE]
    ("n2", ST4, "in"),     # among the pigs — came to himself [SILENCE]
    ("n3a", ST5, "in"),    # the decision
    ("n3b", ST6, "in"),    # the long road back — as a servant
    ("n4", ST7, "in"),     # the father runs, the embrace [SILENCE]
    ("n5", ST8, "in"),     # robe, ring, feast
    ("n6", ST9, "in"),     # the older brother, the door open both ways
]

# s10 (two roads, one lit door): the sanctioned short breath before the card.
BREATH_STILL = ST10
BREATH_DUR = 1.2
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after j1 / n1 / n2 / n4 + breath on s10; "
    ...
    OUT = "luke-15_road-runs-both-ways.mp4"
    A_KBPS, MUX = 96, 20
