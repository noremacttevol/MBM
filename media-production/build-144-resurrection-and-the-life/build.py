#!/usr/bin/env python3
"""Assemble Story Video #144 — "I Am the Resurrection and the Life"
(John 11:25-26).
...
ONLY exact KJV (John 11:25 = j1, John 11:26 = j2). Sacred pacing: the KJV beats
carry the longer KJV_GAP. CONTENT-CARE: death carried with hope — the art has
no decay, no gore; light does the preaching (s5/s8).

Daylight video: caption box black@0.60 (per the prompt sheet; tuned to the
BRIGHTEST frame per the #40 lesson).

Output: john-11_resurrection-and-the-life.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-outside-the-tomb.jpeg"
S2 = "s2-marthas-faith.jpeg"
S3 = "s3-steady-and-tender.jpeg"
S4 = "s4-the-promise.jpeg"
S5 = "s5-light-from-the-seam.jpeg"
S6 = "s6-believest-thou-this.jpeg"
...
BEATS = [
    ("n0", S1, "in"),      # four days; mourners at the tomb
    ("n1a", S2, "in"),     # Martha's faith, close on her face
    ("n1b", S3, "in"),     # his face — steady and tender (sacred hold)
    ("j1", S4, "in"),      # KJV John 11:25 — the promise, two-shot
    ("n2", S5, "in"),      # light from the seam — He Himself is the life
    ("j2", S6, "in"),      # KJV John 11:26 — believest thou this?
    ("n3a", S7, "in"),     # Martha said yes — the weight lifting
    ("n3b", S8, "out"),    # out of the tomb
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"KJV at j1 {j1_start:.1f}s / j2 {j2_start:.1f}s; "
    ...
    OUT = "john-11_resurrection-and-the-life.mp4"
    A_KBPS, MUX = 96, 20
