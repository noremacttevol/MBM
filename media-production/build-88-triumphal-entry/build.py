#!/usr/bin/env python3
"""Assemble Story Video #88 — The Triumphal Entry (Matthew 21:1-11).

PHASE-1 STILLS-ONLY (Law E): all 9 stills cut in as beats (no separate breath
still), Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW §5),
cream card.

CARE-GREEN: joy and peace; celebration, never mob energy. Face-shown Jesus
locked to master (s3/s5/s7/s9), only-Jesus-in-cream held (crowd earth tones);
the mother donkey and her colt both present (Matthew account). Two-voice:
narrator en-US-AndrewNeural; the CROWD's KJV read by the scripture voice
en-US-ChristopherNeural (Matt 21:9 j1, 21:10-11 j2 — the Jesus voice is
reserved and he does not speak here). Ear-check pass (card 0.92 = whisper
spacing "war horse"; audio says "warhorse"). n0/n1 split.

SACRED SILENCES: the marked s4 hold after n1a (the road laid) and reverent
holds on both crowd KJV lines.

NO MUSIC BED (HUM PURGE law). Bright spring daylight: caption box black@0.60.

Output: matthew-21_triumphal-entry.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-go-into-the-village.jpeg"
ST2 = "s2-they-brought-the-colt.jpeg"
ST3 = "s3-riding-toward-the-gate.jpeg"
ST4 = "s4-cloaks-and-branches.jpeg"
ST5 = "s5-the-kings-face.jpeg"
ST6 = "s6-hosanna.jpeg"
ST7 = "s7-entering-jerusalem.jpeg"
ST8 = "s8-who-is-this.jpeg"
ST9 = "s9-the-humble-king.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}   # Matt 21:9 / 21:10-11 (crowd), scripture voice — cream italic
# Long holds: the marked s4 silence after n1a + both crowd KJV lines.
LONG_HOLD = {"n1a", "j1", "j2"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0a", ST1, "in"),    # sent two ahead into the village
    ("n0b", ST2, "in"),    # the colt brought — as the prophecy said
    ("n1b", ST3, "in"),    # the King was coming (riding to the gate)
    ("n1a", ST4, "in"),    # cloaks and branches — the road laid [SILENCE]
    ("n2", ST5, "in"),     # the people shouted (the King's face)
    ("j1", ST6, "in"),     # KJV Matt 21:9 — Hosanna [SILENCE]
    ("n3", ST7, "in"),     # the whole city stirred (entering Jerusalem)
    ("j2", ST8, "in"),     # KJV Matt 21:10-11 — who is this [SILENCE]
    ("n4", ST9, "in"),     # the humble King on a donkey
]
...
def main():
    ...
    OUT = "matthew-21_triumphal-entry.mp4"
    A_KBPS, MUX = 96, 20
