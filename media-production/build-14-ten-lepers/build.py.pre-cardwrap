...
edge-tts narration, serif captions, closing question card on cream #F7F2E9,
read aloud (Readable-Card Law).
...
for the sacred beat, then returns and is fully out before the closing card,
...
in the flesh and stay red; the ten lepers' cry (s13) is SCRIPTURE blue. Beat
...
The still variables S1..S12 are introduced here — the old template referenced
the jpegs by literal filename.
...
CARD = "n15"
# PEAK: the beat the music bed dies for. s13 is the cry across the gap,
# the first verbatim scripture in the film.
PEAK = "s13"
...
def bed_filter(idx, start, end, style):
    ...
    if style == "a":
        ...
        src = ("aevalsrc='0*(sin(2*PI*110*t)+sin(2*PI*110.5*t))"
               "+0*(sin(2*PI*146.83*t)+sin(2*PI*147.5*t))"
...
def main():
    ...
    # ---- audio: narration at derived offsets + warm beds ----
    # Bed out for the peak beat, back after, fully out before the card.
