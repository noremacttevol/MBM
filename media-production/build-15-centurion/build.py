...
PHASE-1 STILLS-ONLY build (Law E, 2026-07-11): 12 painted 2K stills with slow
Ken Burns drift, narration, serif captions, KJV red-letter lines, closing card.
NO motion clips (Cameron: Phase 1 is stills-only).
...
Timing is COMPUTED from the measured mp3 durations at build time (no hand
offsets to drift out of sync). Music: a warm detuned-pair bed opens and fades to
FULL SILENCE just before "he marveled" / j2, so j2 lands in true silence; a warm
bed returns after and is out before the closing card.
...
# segment; every word of that segment is captioned on it. Zoom alternates for
# life. The card is handled separately.
BEATS = [
    ("n1", S1, "in"), ("n2", S1, "out"),
    ("n3", S2, "in"), ("n4", S2, "out"),
    ("n5", S3, "in"), ("n6", S3, "out"),
    ("n7", S4, "in"),
    ("n8", S5, "in"), ("j1", S5, "out"), ("n9", S5, "in"),
    ("n10", S6, "in"), ("n11", S6, "out"),
    ("n12", S7, "in"), ("n13", S7, "out"),
    ("n14", S8, "in"), ("j2", S8, "out"), ("n15", S8, "in"),
    ("n16", S9, "in"), ("j2b", S9, "out"), ("n17", S9, "in"),
    ("n18", S10, "in"), ("j3", S10, "out"), ("n19", S10, "in"),
    ("n20", S11, "in"), ("n21", S11, "out"),
    ("n22", S12, "in"),
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"j2 at {j2_start:.1f}s", flush=True)
    ...
    # ---- audio: narration at computed offsets + warm beds (silence at j2) ----
    beds = [
        (0.0, j2_start - 1.2, "b"),          # out just before j2 (the marvel)
        (j2_start + audio_dur["j2"] + 1.0, total - 12.0, "a"),  # returns after
    ]
