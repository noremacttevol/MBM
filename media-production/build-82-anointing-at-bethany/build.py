#!/usr/bin/env python3
"""Assemble Story Video #82 — The Anointing at Bethany (Mark 14:3-9).

PHASE-1 STILLS-ONLY (Law E): 8 stills, 12 beats (stills reused for the
KJV+context pairs; the closing memorial reflection bookends on the table),
Ken Burns drift, caption-v2 bottom-band split/synced (CAPTION LAW §5), cream
card.

CARE: dignity throughout — the woman's extravagance is worship, never drama;
her cheeks dry (no tears). Face-shown Jesus locked to master (s1/s4/s6);
WOMAN + JAR continuity locked (green shawl, white alabaster jar) across
s2/s4/s8; only-Jesus-in-cream held. Two-voice: narrator en-US-AndrewNeural;
Jesus en-US-ChristopherNeural exact KJV Mark 14:6 (j1), 14:8 (j2), 14:9 (j3).
Ear-check pass (j2 tie-break on medium.en). n0/n1 split so arrival, jar,
breaking, and pouring each land on their still.

SACRED SILENCES: the marked s4 hold after n1b (the pour) and reverent holds
on all three KJV lines.

NO MUSIC BED (HUM PURGE law). Evening lamplight: caption box black@0.55.

Output: mark-14_anointing-at-bethany.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-table-at-bethany.jpeg"
ST2 = "s2-she-came-with-a-jar.jpeg"
ST3 = "s3-she-broke-it-open.jpeg"
ST4 = "s4-poured-on-his-head.jpeg"
ST5 = "s5-the-indignation.jpeg"
ST6 = "s6-let-her-alone.jpeg"
ST7 = "s7-she-hath-done-what-she-could.jpeg"
ST8 = "s8-poured-out.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2", "j3"}   # Mark 14:6/8/9, Jesus voice — cream italic
# Long holds: all three KJV lines + the marked silence after n1b (the pour).
LONG_HOLD = {"j1", "j2", "j3", "n1b"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0a", ST1, "in"),    # the table at Bethany
    ("n0b", ST2, "in"),    # she came with a jar
    ("n1a", ST3, "in"),    # she broke it open
    ("n1b", ST4, "in"),    # poured on his head [SILENCE]
    ("n2", ST5, "in"),     # the indignation
    ("n3", ST6, "in"),     # Jesus stopped them
    ("j1", ST6, "out"),    # KJV Mark 14:6 — let her alone
    ("n4", ST7, "in"),     # not with you much longer
    ("j2", ST7, "out"),    # KJV Mark 14:8 — done what she could [SILENCE]
    ("n5", ST8, "in"),     # the only one who prepared him
    ("j3", ST8, "out"),    # KJV Mark 14:9 — a memorial of her
    ("n6", ST1, "out"),    # the memorial, still told — bookend the table
]
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after each KJV line + n1b; "
    ...
    OUT = "mark-14_anointing-at-bethany.mp4"
    A_KBPS, MUX = 96, 20
