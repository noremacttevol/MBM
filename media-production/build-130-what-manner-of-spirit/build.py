#!/usr/bin/env python3
"""Assemble Story Video #130 — "Ye Know Not What Manner of Spirit Ye Are
Of" (Luke 9:51-56).

PHASE-1 STILLS-ONLY (Law E): 7 stills painted, 6 cut in (s2 sorrow-not-wrath
was redundant with the rebuke beat — precedented drop), Ken Burns drift,
caption-v2 bottom-band split/synced (CAPTION LAW §5), cream closing card.

Face-shown Jesus locked to master (s4/s6). Two-voice: narrator
en-US-AndrewNeural; Jesus en-US-ChristopherNeural exact KJV Luke 9:55-56.
Ear-check pass 1.00 on every segment ("lives" noun read naturally).

SACRED SILENCES: the rebuke hold after j1 (pack-marked) and the two-fires
hold after n3 (pack-marked s5 SILENCE). s7 (warmth-not-fire, the open hand
over the lamp) is the sanctioned short breath before the card.

NO MUSIC BED (HUM PURGE law). Late-afternoon video: caption box black@0.50
(pack directive).

Output: luke-9_what-manner-of-spirit.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-the-closed-gate.jpeg"
ST3 = "s3-ready-for-fire.jpeg"
ST4 = "s4-the-rebuke.jpeg"
ST5 = "s5-two-fires.jpeg"
ST6 = "s6-walking-on.jpeg"
ST7 = "s7-warmth-not-fire.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1"}   # Luke 9:55-56, Jesus voice — cream italic
# Long holds: the rebuke hold after j1 and the marked SILENCE on the
# two-fires still after n3.
LONG_HOLD = {"j1", "n3"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0", ST1, "in"),     # the closed gate — refused; the heat brewing
    ("n1", ST3, "in"),     # ready for fire — the ask
    ("n2", ST4, "in"),     # Jesus turns — the rebuke
    ("j1", ST4, "out"),    # KJV Luke 9:55-56 over the rebuke [SILENCE]
    ("n3", ST5, "in"),     # two fires — the hearth chosen [SILENCE]
    ("n4", ST6, "in"),     # walking on — peace
]

# s7 (warmth, not fire — the open hand over the lamp): the sanctioned
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after j1 (KJV) / n3 + breath on s7; "
    ...
    OUT = "luke-9_what-manner-of-spirit.mp4"
    A_KBPS, MUX = 96, 20
