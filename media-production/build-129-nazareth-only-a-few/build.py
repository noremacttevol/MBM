#!/usr/bin/env python3
"""Assemble Story Video #129 — Nazareth: Only a Few (Mark 6:5-6).

PHASE-1 STILLS-ONLY (Law E): 7 painted stills, Ken Burns drift, narration,
serif captions (caption-v2: bottom band only, chunked + synced — CAPTION LAW
§5), cream closing card. NO AI motion clips.

Face-shown Jesus locked to master (s1/s4/s5). Two-voice: narrator
en-US-AndrewNeural; Mark 6:5-6a exact KJV read by the scripture voice
en-US-ChristopherNeural (the evangelist's line, not Jesus-speech — W1's
voice-law note). Ear-check pass, no overrides.

SACRED SILENCES: extended holds after the KJV verse (on the healing touch)
and after n3 (mercy line, on the marveling face — the pack's marked s4→s5
silence). s7 (the open door in light) is the sanctioned short breath before
the card, flowing into "the door is the same today."

NO MUSIC BED (HUM PURGE law). Bright-day village video: caption box
black@0.60 (pack directive).

Output: mark-6_nazareth-only-a-few.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
ST1 = "s1-back-in-nazareth.jpeg"
ST2 = "s2-isnt-this-the-carpenter.jpeg"
ST3 = "s3-the-few-who-came.jpeg"
ST4 = "s4-hands-laid-on-the-sick.jpeg"
ST5 = "s5-he-marvelled.jpeg"
ST6 = "s6-the-healed-few.jpeg"
ST7 = "s7-the-door-faith-opens.jpeg"

TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"s1"}   # Mark 6:5-6a, scripture voice — cream italic
# Long holds: the KJV verse (healing touch) and n3 (mercy — the marked
# silence on the marveling face).
LONG_HOLD = {"s1", "n3"}
MID_HOLD = set()
CARD_TEXT = TEXT["card"]

BEATS = [
    ("n0", ST1, "in"),     # back in Nazareth, teaching
    ("n1", ST2, "in"),     # the doubt of the familiar
    ("n2", ST3, "in"),     # only a few — the ones who stepped out
    ("s1", ST4, "in"),     # KJV Mark 6:5-6a — hands laid [SILENCE]
    ("n3", ST5, "in"),     # mercy over the marveling face [SILENCE]
    ("n4", ST6, "in"),     # the healed few, quiet joy
]

# s7 (the open door in light): the sanctioned short breath before the
# closing card — silent, captionless, flows into the card's door line.
BREATH_STILL = ST7
BREATH_DUR = 1.2
...
def main():
    ...
    print(f"total runtime: {total:.1f}s ({total/60:.2f} min); "
          f"silences after s1 (KJV) / n3 + breath on s7; "
    ...
    OUT = "mark-6_nazareth-only-a-few.mp4"
    A_KBPS, MUX = 96, 20
