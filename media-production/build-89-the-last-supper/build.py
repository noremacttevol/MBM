#!/usr/bin/env python3
"""Assemble Story Video #89 — The Last Supper (Luke 22:14-20).
...
FACE-SHOWN Jesus (master-locked). Narrator en-US-AndrewNeural, Jesus
en-US-ChristopherNeural speaking ONLY exact KJV — the sacrament words
Luke 22:19b (j1, my body) and 22:20b (j2, my blood), cream-italic, sacred
holds. The emblem words land on the emblem stills (j1 on the bread s3, j2 on
the cup s4). n2 and n3 reworded to plain action only (Translation Law — the
pack echoed j1's "his body" and j2's "new promise in his blood"). n5 closes
on the emblems-remain still. CONTENT-CARE: reverent sacrament, no gore.

Warm upper-room frames: caption box black@0.60 (brightest-frame, #40).

Output: luke-22_the-last-supper.mp4, 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
CARD_TEXT = ("He left a simple meal to remember him by. Come to his table — "
             "he is still the host.")

BEATS = [
    ("n0a", S1, "in"),     # the Passover had come (the glowing window)
    ("n0b", S2, "in"),     # gathered his friends in the upper room (gathered at the table)
    ("n1", S7, "in"),      # he had wanted this meal with them (love in his eyes)
    ("n2", S5, "in"),      # took the bread, broke it, gave it (resting near him)
    ("j1", S3, "in"),      # KJV Luke 22:19b: this is my body... remembrance (he broke the bread)
    ("n3", S6, "in"),      # lifted the cup, gave it to them too (turned toward him)
    ("j2", S4, "in"),      # KJV Luke 22:20b: this cup is the new testament in my blood (the cup lifted)
    ("n4", S8, "in"),      # not the fruit of the vine again; sang into the night (singing)
    ("n5", S9, "out"),     # bread and cup remain — a gift to remember (emblems remain)
]
...
def main():
    ...
    OUT = "luke-22_the-last-supper.mp4"
    A_KBPS, MUX = 96, 20
