#!/usr/bin/env python3
"""Assemble Story Video #92 — Peter's Denial and the Look (Luke 22:54-62).
...
CONTENT-CARE: Peter's grief tender — wet eyes at most, no painted teardrops.
The Look is compassion, not scorn. NO KJV cream-italic — narrator throughout
(even the Look is wordless), so all captions are white serif. FACE LAW v3:
Jesus face-shown for the Look (master-locked, only he wears cream), warm and
knowing.

Sacred holds on n2 (the denials mount) and n4 (the Look — wordless). n5
recorded in three so the eight beats fill the eight stills per the CAPTION LAW.
...
paths in segs/. Output: luke-22_peters-denial.mp4,
...
ST1 = "s1-fire-in-the-courtyard.jpeg"
ST2 = "s2-the-servant-girl.jpeg"
ST3 = "s3-i-do-not-know-him.jpeg"
ST4 = "s4-the-rooster.jpeg"
ST5 = "s5-the-look.jpeg"
ST6 = "s6-peter-remembers.jpeg"
ST7 = "s7-out-through-the-gate.jpeg"
ST8 = "s8-the-dying-fire-at-dawn.jpeg"

TEXT = {s[0]: (s[4] if len(s) >= 5 else s[2]) for s in make_narration.SEGMENTS}
KJV = set()  # narrator-only build (the Look is wordless)

# MEMBER shelf: one-line Gospel Library pointer under the closing invitation.
GL_POINTER = "Learn more — Gospel Library: Repentance"

BEATS = [
    ("n0", ST1, "in"),           # warming by the fire in the courtyard
    ("n1", ST2, "in"),           # the servant girl accuses
    ("n2", ST3, "in"),           # each denial harder — extended hold
    ("n3", ST4, "in"),           # the rooster crowed
    ("n4", ST5, "in"),           # Jesus turned and looked — the Look; hold
    ("n5a", ST6, "in"),          # Peter remembered
    ("n5b", ST7, "in"),          # out through the gate, wept bitterly
    ("n5c", ST8, "in"),          # the look was love — the dying fire at dawn
]
...
def main():
    ...
    print(f"sacred holds: n2 at {start_of['n2']:.1f}s, n4 (the Look) at {start_of['n4']:.1f}s", flush=True)
