#!/usr/bin/env python3
"""Assemble Story Video #35 — The Great Banquet (Luke 14:15-24).
...
Face-never (#18): this is a parable told from off-screen — the host, the
servant, the excusing guests, and the poor are all shown fully; there is NO
Jesus figure in any still.

Milk framing: when the invited make excuses, the host opens the doors WIDER to
the poor and overlooked — God's answer to rejection is MORE invitation.
...
("...that my house may be filled" — the full table, the peak), so it lands in
true silence; a warm bed returns after and is out before the closing card.

Output: luke-14_great-banquet.mp4 (SCRIPTURE-NAME LAW), 1080x1920 H.264 30fps, <25MB.
"""
import os
import subprocess
import textwrap
...
S1 = "s1-feast-ready.jpeg"
S2 = "s2-servant-sent.jpeg"
S3 = "s3-excuses.jpeg"
S4 = "s4-servant-returns.jpeg"
S5 = "s5-to-the-streets.jpeg"
S6 = "s6-bringing-them-in.jpeg"
S7 = "s7-table-full.jpeg"

# Caption text = verbatim spoken text, keyed by segment name.
TEXT = {s[0]: s[4] for s in make_narration.SEGMENTS}
KJV = {"j1", "j2"}
PEAK = "j2"   # "...that my house may be filled" — music goes to silence here
