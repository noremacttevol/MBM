...
> 12e. **THE COMPLAINT-FIRST LAW (Cameron, 2026-08-06: "you should always do the ones that are complained about asap. that should be a new rule"): a row with an OPEN complaint outranks ALL other work in every queue — complaint-fix re-cuts before stranded resumes before audio-fixes before fresh builds, and within each queue complained rows go first. A complaint is Cameron having already asked; making him wait behind new production is the failure. Enforced mechanically in autopilot.sh (next_cfix + complaint-first ordering in every picker).**
> 12f. **THE LOW-NUMBER LAW (Cameron, 2026-08-07: "the smaller the number of videos the more priority it has — 01-09 are good but 10-20 have been waiting"): row numbers are his viewing order; within every queue, after complained-rows-first, the LOWEST row number always goes next. Cheapest-first/banked-stills ordering is demoted to never override his order. Parked fix classes must always have a lane that picks them up (NEEDS-AUDIO → audio lane; NEEDS-REBUILD → author lane) — a park with no pickup is a complaint left waiting.**
> 12g. **REVIEWER LIST CLARIFICATION (Cameron, 2026-08-13): "lowest" means the lowest-numbered card in the live red `Complained — machine is fixing` list. That list outranks every production/author board. Do not skip its first row merely because an old board entry says RUNNING: verify recent process/commit activity; if the claim is stale and nobody is actually fixing it, take it over. At this clarification the correct first row is #44 Pentecost.**
> 12h. **EVERY COMPLAINED TIMESTAMP GETS AN HONEST FIX (Cameron, enforced 2026-08-13 after row 63): if Cameron lists four bad pictures, fix and verify four distinct pictures. Never copy an adjacent still into a complained slot, extend a hold, crop around the defect, or declare "same face" from age/hair alone. Compare every encoded timestamp to the actual locked face reference. Reuse is allowed only when Cameron explicitly asked to remove the shot or the reused image truly depicts that same beat and was not rejected.**
> 12i. **A PRONUNCIATION FIX MUST CHANGE THE SOURCE CONTROL (Cameron, enforced 2026-08-13 after row 117): re-rendering the same plain word and measuring a different waveform is not a fix. Save a persistent alias/CMU/IPA instruction, use a TTS model that actually supports it, preserve the visible spelling, transcribe the output, and verify the encoded MP4. Cameron's complaint about what he heard outranks F0/formant claims that the rejected cut was “objectively correct.”**
> 12j. **THE OFFLINE-OPERATOR LAW (Cameron, 2026-08-16): Machine A's unattended dispatcher defaults to ONE Codex + Ollama `qwen3.5:27b` lane. Claude is an optional fallback, not a dependency. It obeys the same complaint-first, live-lowest-number, claim-by-push and full-cut gates. Classify before spending: caption-only fixes preserve pictures and audio; audio-only fixes generate no pictures; new pictures are made only when the current rendered cut proves the complaint is visual. “Offline” means the reasoning worker; Gemini, ElevenLabs, GitHub, Reviewer and Firebase remain online services.**
> 12k. **CAPTION SPELLING IS A SEPARATE, RENDERED LAYER (Cameron, row 95, 2026-08-16): if the spoken KJV source uses an archaic spelling such as “To day” but Cameron asks for the visible caption to say “today,” use a build-local `TEXT_OVERRIDES` caption correction. Do not re-voice, regenerate pictures, edit V1, or change the shared wrapping engine. Extract the complained timestamp from the encoded replacement and visually prove the corrected one-word spelling.**
> 12l. **SCREEN-SIDE AND LITERAL-DEATH LAW (Cameron, row 179, 2026-08-17): turn relational staging into viewer coordinates before generation. For Stephen's vision, Jesus at the Father's right hand is on the VIEWER'S LEFT and the Father is on the VIEWER'S RIGHT when they face us. Never confuse a person's anatomical right with the viewer's right edge. A modern narrator must plainly say a martyr was killed when that is the event; restraint may omit impact/blood/wounds, but the aftermath must not look or sound like a nap merely because scripture says “fell asleep.”**
> 12m. **A PLACE PLATE LOCKS THE WORLD, NOT THE CAMERA (Cameron, row 181, 2026-08-17): pictures may never be duplicates with pieces merely added or removed. Continuity references preserve identity, material, terrain, weather and time of day only; each beat needs genuinely different camera geometry. A crop, zoom, mirror, relight, mask, or add/remove edit is not a new picture. Inspect the full contact sheet and a similarity matrix, then verify suspicious pairs at full size. If a place plate clones composition, disable it for that row, retain character references, and regenerate the duplicate family from distinct viewpoints.**
>
> **App**
...
4. Read `media-production/PRODUCTION-BIBLE.md` and `CREW-GUIDE.md`. Every law binds
   this session (one-locked-face, two-voice, stills-only, ear-check, no-dead-air,
...
8. **The Jesus-framing laws (PRODUCTION-BIBLE §1, "The Standing Laws") are non-negotiable and Cameron has rejected finished videos over each of them. THE #1 LAW: Jesus has ONE locked face, identical in every video, enforced by the JESUS-MASTER-REF image + JESUS LOCK v3 — see (f)/(g).** (The old numbered "Corrections #1–#18" were consolidated into the un-numbered Standing Laws on 2026-07-11; the numbered history is archived in media-production/CORRECTIONS-HISTORY.md, provenance only.) The ones violated most: (a) reverence and consistency govern how he is framed — his ONE locked face is shown, identical in every picture of every video (f/g below); (b) NEVER a small detached Jesus at the frame edge while the crowd's eyes wander — gazes must visibly converge on him; (c) NO rim-light, halo, or glow that outlines his face or head — a glowing silhouette highlights him, the opposite of the goal; (d) still BEFORE motion clip when a beat has both; (e) the anti-panel sentence is mandatory in every wide multi-figure still prompt; (f) **JESUS HAS ONE LOCKED FACE, SHOWN THE SAME IN EVERY VIDEO (Cameron, 2026-07-15 — this REVERSED the old "face is never shown" rule). Consistency is enforced by IMAGE, not wording: attach `media-production/JESUS-MASTER-REF/jesus-face.jpeg` as a `REF:` line to EVERY shot Jesus is in, and paste the byte-identical JESUS LOCK v3 paragraph into the prompt. The reference picture IS his face; prose never re-invents it. He may be shown from any natural angle now. Run the mechanical gate before any credit: `python3 media-production/jesus_face_gate.py --dir <build-folder>` must exit 0 — it fails a build if a Jesus shot is missing the lock or the REF line, or if any wrong-Jesus drift word appears**; (g) **He is a real, warm, painted MIDDLE EASTERN Jewish man (~33), NOT a hooded void / cutout / "Assassin's Creed" ghost. THE JESUS LOOK STANDARD, identical in every video: warm tan/olive-brown skin, NEVER white/Caucasian/pale/blue-eyed/blond; shoulder-length dark brown-black wavy hair (never short, never changing length); a full dark beard; eyes EXACTLY as the active pipeline's reference image shows them — V1 (`JESUS-MASTER-REF/jesus-face.jpeg`, LOCK v3): warm brown; **V2 (`JESUS-V2-REF/jesus-v2-face.jpeg`, LOCK v5): green/hazel — the ref image IS his face and outranks this sentence; never reroll or iris-edit a frame AWAY from the ref (rubric lesson 20, 2026-08-12, reversing the wrong brown-edits on rows 71/89/98/120)**; ONE plain undyed off-white/cream wool robe every scene and every video (never a fur/fleece look, never changing) — and ONLY Jesus wears cream. No halo, glow, or rim-light. The Twelve and other recurring characters are locked the same way (approved reference image + text lock) via media-production/CAST-REF/CAST-BIBLE.md, so everyone looks like the same person across all 200. Compose scenes around the PEOPLE. See PRODUCTION-BIBLE §1 'How Jesus is depicted'**; (h) ACTION-LOGIC LAW — every figure's action must read correctly at a glance (storm V1 rejected: fishermen appeared to pour water INTO their own boat and pull a rope leading outside the hull); bailing throws water OUT over the gunwale, ropes connect to rigging inside the boat; QC every frame by asking "what does this person appear to be doing?"; (i) TIME-OF-DAY LAW — scene lighting must match the scripture's stated time of day; Mark 4:35 is night: limited light, moon/stars/lightning, NEVER sunset or sunrise coloring (Cameron, 2026-07-10, storm-11 V2 rejection); (j) figures stay visibly INSIDE the boat — deck under feet, gunwale connecting around them; nobody stands on the water unless the scripture puts them there; (k) CYCLICAL-MOTION LAW — repeated actions in clips must visibly cycle (scoop-lift-fling for bailing), never a static pose with a continuous hose-like stream; (l) **FACE-BOARD LAW (Cameron, 2026-08-01): a REF line is not proof of identity. Use one non-conflicting canonical anchor per person, compare every visible appearance side-by-side before assembly, identity-edit any actor drift, then recheck the complete frame because a face edit may change crop, anatomy, people, props, lighting, or geometry. One face-board failure blocks reviewer publication.** Whenever Cameron gives a new correction, write it into PRODUCTION-BIBLE §1 AND this list in the same session.
