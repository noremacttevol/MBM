## ✅ PICTURE C-FIX SHIPPED 2026-08-12 — "1:40 doesnt look like jesus / propping his feet up / smirking looking more evil" — TRACED, AUTOPSIED, fixed touch-once (Machine A `Dev`, Opus runner)

**Cameron's complaint (reportedAgainst the LIVE cut `d8fdc8731`):**
*"That new one you put at 1:40 is bad it doesnt look like jesus and he is propping his feet up and smirkijg lookijg more evil than Jesus would."*

**COMPLAINT LEDGER — "1:40 …smirking / feet up / not Jesus":** FIXED in **s21 / beat b21** (window 97.96–107.78s — 1:40 lands in it). This was the very frame the 08-12 fix rerolled to `jesus:True`; that reroll fixed the "2 places" defect but the model rendered a **smug up-glance grin** and **feet stretched onto the table**, which is what Cameron is now naming.

**TRACE (never guessed — extracted from the live mp4 + beat windows):** 1:40 = 100.0s → beats_v2 window 97.962–107.777 → `v2-r074-b21` → `s21-and-he-was-right-he.jpeg`. Confirmed by extracting the 100s frame from the delivered mp4 (matched s21).

**PROMPT AUTOPSY (rubric meta-law 3) — verdict ALLOWED:** the b21 scene attached the REF and said "his face the locked face" but gave **zero expression direction** and **zero foot-placement constraint**, so the model was free to render (a) a lopsided **smug grin with the eyes cut up-and-away** — which over a weeping woman reads as amused/smug and stops the face reading as the reverent locked Jesus ("doesnt look like jesus"), and (b) his bare feet **stretched up onto the table edge** ("propping his feet up") — even though the narration itself established feet stretch *away* from the table. NOT caused (no bad word forced it) and NOT ignored (the ref was present); the prompt simply **permitted** it. FIX per the autopsy = rewrite the words + add the missing constraint (no reference swap needed — the ref face is correct; the expression was the failure): added to b21 must_show "Jesus's face tender, grave and compassionate, his gaze lowered toward the weeping woman"; added to must_not_show "Jesus NEVER smirks, grins, smiles smugly, or looks amused/mocking/sly/smug/self-satisfied … Jesus's feet are NEVER propped, raised, or resting up on the table; his feet stay LOW on the floor toward the kneeling woman, stretched away from the table"; and the scene now reads "his face the locked face — serene, tender and grave, his eyes lowered toward the weeping woman … his feet rest LOW toward her on the floor, stretched away from the table, never raised or propped up onto the table." ONE reroll (`--only b21 --redo`), landed clean first take: grave lowered gaze, feet on the floor being washed, exactly one cream Jesus, guests older/earth-tone.

**FULL-CUT GATE §6b (rendered final mp4, one frame per beat — 32 rendered beats viewed; 4 sliver <0.3s transition beats skipped):** PASS. s21 was the ONLY complaint-worthy frame; every other beat is clean — realistic photography throughout (Law-14, no cartoon/mix), ≤ONE cream figure (Jesus only) on every frame, Jesus face-locked + tender/grave (b05/b07/b09/b15/b17/b22/b25/b26/b28/b33/b35/b36), WOMAN dignified + identity-consistent (wine-dark dress, grey shawl, loosed dark hair), SIMON consistent indigo, anatomy/hands/feet/scale correct, no modern objects, lamplit-night TOD held, captions 3-colour bottom-band only. This is the touch-once law working: unlike row 11, no other bad frame is riding along.

**AUDIO kept BYTE-IDENTICAL to the live cut (no re-voice, no timing move):** `AUDIO_FROM_V1_SEGMENTS=True` rebuilds from the V1 source mp3s and REVERTS the 2026-08-11 de-muffle (it EQ'd the final mux, not the sources — a segment-rebuild reproduces the muffled `bc8ed8e0` hash). So after v2_assemble rebuilt the video with new s21, the new video stream was **re-muxed with the de-muffled audio track pulled from the live mp4** (`-c:v copy -c:a copy`). Final audio md5 `93ce28bd6d5f5b67974217254b283753` == the live cut Cameron already has. Final mp4 sha256 `aa113ef4…`, 184.5s / 20.8 MB. (The **audio HALF** — "sound STILL wrong/off" — remains open and out of scope for a picture C-FIX; still routed to NEEDS-AUDIO for a row-27-style ElevenLabs re-voice.)

**Cost / touch-once:** 1 reroll / 34 non-sliver beats ≈ 2.9% (≤15% budget), 0 re-rerolls, ~$0.13 Gemini (meter $616.53→$616.94), $0 audio — far under the $6.10/row + 19%-reroll running average; COST LAW trend DOWN.
