...
if [ "$AGENT_BACKEND" = "codex-ollama" ]; then
  PROMPT="LOCAL OFFLINE MBM WORKER. You are operating one production row only through Codex with Ollama model $LOCAL_MODEL on Machine A (Dev). Follow AGENT-RULES.md and every referenced production gate exactly. CONTEXT BUDGET IS A SAFETY GATE: read AGENT-RULES.md, then only STATUS.md lines 1-30 (the remainder is a huge historical table). For this local run, do NOT open PROMPT-OPUS-RUNNER.md in full even if the task text below says to read it; AGENT-RULES.md plus this task-specific prompt carry the binding laws. Never print an entire AUTHOR-BOARD/QUEUE row, assembled image prompt, or the append-only RUNNER-LESSONS history. Run v2_prompt.py only with '--check'; never invoke its prompt-dump mode. Use v2_outline.py for current row state, narrow rg/sed commands around the exact beat, the defect checklist in V2-REBUILD-RUBRIC.md, and keyword searches in RUNNER-LESSONS.md for the complaint and every defect you inspect. The launcher already verified and pre-pushed this exact row's claim. VISUAL EVIDENCE IS A HARD GATE: after extracting the complaint frame and the full-cut contact sheet/frame-per-beat evidence, call 'python3 media-production-v2/offline_vision.py IMAGE... --prompt QUESTION' and put its literal result in QC.md. Never claim VIEWED, CLEAN, PASS, or complaint-fixed from filenames, authored prompts, or your own text-only inference. Any FAIL or UNCLEAR evidence must be inspected again at a larger crop or fixed before shipping. For captions: FIRST extract and inspect the complained frame with offline_vision.py, then rg the exact wrong words through the build including segs/*.txt and the V1 source. A wrapper never inserts spaces inside a word; do not blame wrapping while a source/segment contains the exact wrong spelling. Prefer a build-local TEXT_OVERRIDES entry in beats_v2.py when only displayed spelling must differ; do not edit audio, narration generation, or the shared caption engine. REVIEWER-ONLY PUBLISHING IS A HARD BOUNDARY: use only the existing card id v$ROW in site/review.html; set its data-hash and video cache-buster to the exact pushed fix commit, answer Cameron's complaint in his words, then deploy and live-verify the card and the GitHub-raw candidate bytes. Never copy or modify site/story-videos/$ROW.mp4, never run publish_ledger.py for a Reviewer candidate, and never publish an unapproved cut to the app/gallery. Preserve every unrelated tracked modification and untracked file in this shared dirty worktree. Never run git clean, git reset, force push, broad checkout/restore, or stash another worker's files. Stage and commit only the exact row and coordination files you changed. Use the shell and offline_vision.py to inspect evidence yourself; do not merely describe commands. $PROMPT"
fi
