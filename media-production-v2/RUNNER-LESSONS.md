...
- **A beat can render LETTERBOXED — a 16:9 scene padded with grey/black bars top+bottom inside the 9:16 portrait canvas (2026-08-13, row 126 by-their-fruits b04).** The generator sometimes returns a cinematic wide letterbox even on a `wide:False` beat; the asset is the right pixel size (1536×2752) but the actual scene fills only a central band, so the delivered vertical video shows grey bars above and below the Ken-Burns crop. It is per-frame drift (a plain `--redo` cleared it; the sibling beats of the same lock rendered full-frame), NOT a systemic prompt defect — PROMPT-AUTOPSY = generator-IGNORED / drift, so reroll, don't edit text. DETECT in the FULL-CUT GATE: confirm each frame fills the canvas top-to-bottom with no flat grey/black bars. (Row 126 b04 also drifted MODERN on its first take — galvanized wire fence + buttoned overcoat/trousers/boots on a "false shepherd dressed almost-right" beat whose FOLD lock never banned modern costume/objects; b02 rendered period with the same lock, so it too was per-frame drift, cleared by --redo. When a "stranger dressed almost-right as a shepherd/leader" beat has no explicit first-century-costume + no-modern-object clause, gate it hard for modern dress.)

- **Do NOT `--promote` a place plate from a frame that contains a distinctive SUBJECT — it injects that subject into every beat of the place (2026-08-13, row 126).** The author's QC suggested promoting FOLD from b02, but b02's b-frame is the fleece-WOLF among the sheep; promoting it would have carried the wolf into b04 (which must be a human false-shepherd). Skipped the FOLD promote and let the FOLD lock text carry its 2 beats — b04 rendered clean, no wolf. Rule: only promote a place plate from a PEOPLE/animal-free (or incidental-only) establishing frame; a 2-3-beat place is fine on lock text alone; and never wire a sibling place plate that contains Jesus (rows 121/122 hillside plates both show Jesus → HILLSIDE stayed text-only here). Extends lesson 11 + the build-21 shepherd note.
...
- **Name-marked props render as READABLE ENGLISH/LATIN text (row 158 "two sticks", 2026-08-13 QC-VERIFY).** When a beat prompt says "mark it with a name" / "write upon it" (Ezekiel's writing-rods, labeled scrolls, inscribed tablets), the model tends to carve *legible* English/Latin sentences (e.g. b05: "For Judah, and for the children of Israel his companions"), not the intended indistinct paleo-glyphs — an anachronism against a "NO readable text" author law. NOT a Cameron-complaint class on its own (his 150+ logged complaints have zero text-on-prop entries, and correct on-story labels reinforce the narration), so do NOT blind-reroll it on a verify pass — that risks reproducing text and blows the 15% budget. Durable fix belongs to the AUTHOR: pin the prop lock to "indistinct incised paleo-Hebrew glyph strokes, NO legible letters/words in any language." Flag it, don't chase it.

- **A recurring one-off character with a real REFERENCE lock can still drift on ONE hero close-up while matching every other frame — and the auto-runner will (correctly, by its bar) FIX-WAVE it; the QC-VERIFY gate must promote it to BLOCKING (row 126 by-their-fruits b12, 2026-08-13).** The FARMER (short thinning grey hair + grey beard, `CAST-REF-V2/farmer.jpeg`) rendered on-model in b05/b06/b09/b14/b15 but b12 gave him fuller/darker hair+beard on the doctrinal good-fig-vs-bad-fig close-up — reads as a different, younger man. Within-story identity drift against a reference lock is Cameron's single most-repeated complaint class (rows 32/55/60/69/90/91/147), so on a full-cut verify it is a SHIP-BLOCKER even though only 1 of 6 frames is off. FIX that HELD: fresh `--only b12 --redo` against the FARMER ref + place plate (NOT `rough_ref` — per row-52, rough_ref preserves the draft's over-full hair and cannot REDUCE it; a fresh redo against the corrected anchor rebuilds identity). The two-fig composition was fully in the scene text, so the redo reproduced blocking + hands cleanly. 1 reroll 5.9%, ~$0.13, audio byte-identical. RULE: on a reference-locked recurring cast member, compare EVERY appearance side-by-side; a single close-up that disagrees with the ref (and the other frames) blocks the ship — don't leave it to a fix wave that may never reach the row before Cameron does.

- **The wide "Valley of Hinnom below Jerusalem's wall" establishing framing has a strong PRESENT-DAY prior that drifts MODERN even WITH a clean VALLEY plate attached, and it survives 2 rerolls — fix by crop-reusing the clean plate frame (2026-08-13, row 133 what-jesus-called-hell b03/b04).** A person-free VALLEY plate (b02) rendered a clean period Valley of Hinnom (Herodian wall, ash heaps, potsherds), and its plate-siblings b05/b06/b07/b13 were all clean — but the two WIDE "here is the real ravine by Jerusalem" establishing beats (b03 "Gehenna is the Greek form…", b04 "…darkest apostasies") drifted to a literal contemporary photo of the modern Hinnom neighbourhood: modern apartment blocks, satellite dishes, red-tile roofs, rooftop water-tanks, hanging laundry, and PLASTIC TRASH in the ravine (modern-object / Law-6 fail). The place plate anchors the wall+ravine but a WIDE city-panorama framing lets the model fill the background with the present-day city. b04 cleared on ONE `--redo`; b03 stayed modern across BOTH rerolls (structural prior for that framing — the actual site today is a modern neighbourhood with a litter-strewn ravine). DON'T burn a 3rd: since the beat's intent (first sight of the valley by the wall) equals the clean plate anchor b02 and they're ADJACENT, fix at $0 with a tighter PUNCH-IN CROP of b02 (reads as a natural push-in, not a dup — lesson 1354/1355). GATE any "real place today / the actual valley/site" establishing wide of a location that still exists as a modern city (Hinnom, Kidron, a modern Jerusalem overlook) for present-day buildings/satellite-dishes/laundry/plastic-trash, not just a lone modern object.

- **A from-scratch / REPLACED row can ship an ENTIRELY DIFFERENT narrator caption script than its audio — `make_narration.py` SEGMENTS hold an OLD draft while the mp3s speak the FINAL rewrite; the FULL-CUT GATE transcribe-diff catches it and the fix is TEXT_OVERRIDES for ALL narrator segments + the card (2026-08-13, row 133 what-jesus-called-hell).** Unlike the row-84/131/108 partial drift (a few tightened segments), here EVERY narrator segment (n0-n6) AND the card differed: SEGMENTS said an old "You were probably taught that hell is a torture chamber… burning garbage dump…" draft, but the audio spoke the final Gehenna/valley script ("Many of the pictures people carry of hell…", "Gehenna is the Greek form of the name Valley of Hinnom…"). Captions render from SEGMENTS (extract_beats `text` = s[2]/s[4]), so without overrides the whole narrator caption track prints words the voice never says — a guaranteed Cameron complaint that a contact-sheet/source QC never sees (the burned caption only appears in the rendered mp4). DETECT (every build, esp. authored-from-scratch/REPLACED): after assemble, transcribe the delivered mp4 OR diff each segment's `make_narration` SEGMENTS text vs its `audio/<seg>.timing.json` joined text (the word-timed audio truth). FIX: `TEXT_OVERRIDES = {seg: <timing.json spoken text>}` in beats_v2.py for every mismatched narrator segment AND "card" (the override loop covers `data["card"]`); j-segments usually match (KJV). Re-assemble → captions match audio, AUDIO byte-identical (same SHA). The Jesus red-letter captions matching is NOT proof the narrator captions match — check them separately.

- **On a RESUME, a BANKED frame from an earlier (often DIED) gen run can itself be a Law-14 cartoon/painterly frame — the FULL-CUT GATE must view EVERY beat, not just the ones you regenerated (2026-08-13, row 142 light-of-the-world b03/s03, Opus runner).** This row was an author-parked resume: the fix regenerated ONLY s07/s08 (the blind-man face-board), and the other 8 stills were "banked, on-lock, verified" per the author park note. But s03 — banked from an earlier autopilot run that had died mid-build — had rendered as a smooth digital-PAINTING (illustration temple, painted candelabra, painted Jesus) sitting among photographic siblings = Law-14 mixed-cut (a single stylised frame fails the whole cut). It was invisible until the per-rendered-frame gate viewed it. A `--only b03 --redo` with the current pipeline landed a clean realistic photograph on the first try ($0.13, 1 reroll), audio byte-identical (the reroll doesn't touch audio; re-assemble keeps the same AUDIO SHA). RULE: "banked/PRESERVE" from a prior session's park note means "identity/composition are good," NOT "passes today's realism bar" — the FULL-CUT GATE (Cameron 2026-08-10) still owns every frame of the delivered mp4, banked or fresh. Early gen runs (esp. pre-realism-hardening or a crashed partial) are the most likely to hide a cartoon frame; view all of them.

- **A JESUS↔WOMAN "reach / touch-me-not" beat: HARDEN `must_not_show` with an explicit NO-CONTACT ban FIRST — blind rerolls OSCILLATE touch↔face-drift and burn the budget (2026-08-13, row 134 today-in-paradise b11, first-attempt build).** b11 illustrates John 20:17 "Touch me not," and its scene prose ("close on the halted reach — Mary's fingers stopped a hand's-breadth from his sleeve, his palm turned gentle between them") is the row-98 proximity-contact trap: the tight two-shot + "reach" wording reliably pulls the pair into hand-contact (a doctrinal contradiction of its OWN red-letter caption). FIVE takes: 1/3/4 touched with a good canonical Jesus face; take 2 landed the no-contact GAP but drifted Jesus YOUNGER/sparser-beard/off-ref; take 5 (first with the ban) fixed the face but hands still overlapped. Only after HARDENING `must_not_show` ("ABSOLUTELY NO CONTACT… a clear hand's-breadth of open air… his hand does NOT rest on her and hers does NOT rest on him; no hands touching/clasped/overlapping… this frame illustrates TOUCH ME NOT") did a redo land BOTH the canonical face AND a clear gap. LESSON: the moment a "reach/touch-me-not/near-embrace" two-shot renders contact, do NOT spend 3–4 blind rerolls (they oscillate the two defects) — add the no-contact `must_not_show` clause enforcing the scene's own stated gap (this ENFORCES the author's intent, it does not change the story), THEN reroll. Verify the gap by ZOOMING the hands (a full-frame glance reads "close" as "touching"). Companion to row-98 (Mary/Jesus lovers-proximity) and row-49 (mother/son romantic read).

- **An "old book / open manuscript / two ages of a page" beat can render a decorative script that resembles a REAL FICTIONAL alphabet (Tolkien Tengwar / "Elvish") — non-readable but RECOGNIZABLE, on a sacred-text prop (2026-08-13, row 154 everlasting-gospel b10, Opus runner).** The generator's default for "ancient book with writing" is a flowing connected cursive; on some takes it lands close enough to Tengwar that a viewer could clock it as Lord-of-the-Rings Elvish — jarring on a Revelation "everlasting gospel" page. It is NOT the same as legible-English/Hebrew text (which is an obvious-garbage reroll, e.g. b05's Hebrew this same row) — it's non-readable and contextually a real book, so in a short Ken-Burns pan it reads as "ancient manuscript" to most viewers = a subtle FIX-WAVE style risk, not an obvious-garbage runner reroll (don't blow the ≤15% budget on it once the row already spent its rerolls on true garbage). Block+reroll ONLY if the script is prominent/foregrounded/legible; otherwise log FIX-WAVE. DURABLE FIX (author must_not_show, all book/scroll/page beats): "no lettering or characters of any kind, no readable or decorative script, only worn indistinct stains, creases and ink-shadows." Block Hebrew-BLOCK script is period-fine (b09 passed); flowing-cursive-that-could-be-a-conlang is the risk.

- **CORRECTION to the b10 lesson above — at the FULL-CUT GATE / QC-VERIFY stage (the cut about to reach Cameron) fictional/Elvish glyphs on a sacred-text prop are a SHIP-BLOCKER, not a deferrable FIX-WAVE (2026-08-13, row 154 everlasting-gospel b10, Opus QC-VERIFY pass).** The runner's "first-attempt, no obvious garbage" bar and the reviewer-facing FULL-CUT GATE 6b bar are DIFFERENT: 6b blocks "anything that would make Cameron type a complaint." On tight zoom b10 was unmistakable Tolkien Tengwar on a Revelation "everlasting gospel" page — a real trigger (his row-7 "burned text" + row-50 typo-square history). Row 11 reaching him with 7 bad frames is exactly why 6b kills the "defer it" call. **$0 FIX WHEN THE IMAGE ENDPOINT IS DOWN (no reroll possible/needed):** de-ink the source asset mechanically in PIL — (1) `bg = GaussianBlur(orig, ~22)`; replace pixels darker than the local bg (the thin ink strokes) with bg, restricted to a FEATHERED band mask over the script → removes ink, keeps sharp parchment (NO redaction bar; a whole-band blur of the ORIGINAL dark ink instead makes an obvious dark bar — don't); (2) the de-ink leaves faint pale ghost strokes still legible as letters, so composite a band-limited `GaussianBlur(~9)` through the same feathered mask (safe now — no dark ink left to smear) to melt the ghosts into an indistinct faded stain. Result satisfies both must_show ("same indistinct line on both pages") and must_not_show ("no letterforms"). Re-assemble pictures-only (AUDIO REBUILD PASS = byte-identical), verify on the RENDERED mp4 at video scale (not just the source). Keep the original as `.pre-elvish.bak`.

- **An INSCRIBED-STONE prop (an altar/plinth/stele the story is ABOUT — esp. Acts 17's "To the unknown God" altar) has a carved-text prior so strong that a plain `--redo` keeps re-carving legible letters (row 138 b03/b08, 2026-08-13).** b02/b08 `must_not_show` was ABSOLUTE-no-legible-text yet the dead-run AND reroll #1 both rendered readable Greek; reroll #2 of b03 even produced clean CORRECT Greek "ΑΓΝΩΣΤΩ ΘΕΩ" — but the author's design + inter-beat continuity (the SAME altar reads illegible traces in b02) require traces on every altar beat, so correct-legible is still a reject here. PROMPT-AUTOPSY = ALLOWED/IGNORED, but the runner may NOT add a no-text clause (beat-text is locked) and blind rerolls burn the 2-pull cap re-carving text. **DURABLE $0 FIX (same family as row 154 Elvish de-ink): mechanically de-ink the inscription in cv2 — grayscale ROI, `bg=medianBlur(51)`, mask = `bg-gray > ~10` restricted to the text sub-rect, dilate, `cv2.inpaint(...,INPAINT_TELEA)`, blend a ~10% GaussianBlur ghost so it reads as worn stone, feather the ROI back into the frame; keep a `.pre-deink.bak`.** GOTCHA: size the mask sub-rect to the FULL letter width — a too-narrow box leaves the rightmost column of letters surviving (row 138 s08 needed a 2nd wider pass). VERIFY by re-extracting the beat from the RENDERED mp4, not just the source. When a stone prop's beat lacks a no-text clause AND the story centers on that stone, expect legible text and plan the de-ink instead of spending >2 rerolls.

- **MODERN PLASTIC EAR-TAGS ON SHEEP — a pervasive historical-coherence artifact (lesson 6) the model bakes into flocks (2026-08-13, row 159 other-sheep, Opus runner).** Any beat with sheep tends to render small yellow/orange plastic livestock ID ear-tags on some animals — a modern object in a first-century scene. Tiny at Ken-Burns/phone scale and always background (never the narrated subject), so a single frame is not a Cameron-complaint trigger, but a whole shepherd-story is full of them. A blind `--redo` does NOT reliably remove them (the model's sheep training data carries tags) and chasing every sheep frame blows the 15% reroll budget. DURABLE FIX is AUTHOR-lane: add "no plastic ear-tags, no modern livestock tags, clean natural ears" to `must_not_show` on every sheep beat of any shepherd/flock row (159, and the John 10 / lost-sheep family: 21, 143). Runner: log it FIX-WAVE, do not reroll past budget.

- **A static closing question card encoded with default H.264 B-frames can decode as intermittent enlarged/cropped text after the art→card stream-concat boundary — inspect the whole card HOLD, not one frame (2026-08-13, row 135).** The art, captions, audio, source stills, and first card frame all passed, but rendered-frame sampling near the end exposed a card where the same text suddenly filled/cropped beyond the 9:16 edges. The source card showed the same intermittent decoded corruption on some B-frames; a controlled all-I test made every non-fade card frame pixel-identical. DURABLE FIX in `v2_assemble.py`: encode `card.mp4` separately with `-g 1 -bf 0` before concat. Then re-cut with `--resume-base` (no need to rebuild the Ken Burns chunks), extract multiple PNGs across the card hold including the last readable second, and verify none crop or jump. A single opening-card screenshot is not a card gate.
