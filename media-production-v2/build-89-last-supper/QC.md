...
## ✅ C-FIX RE-OPEN #5 SHIPPED ($0, 0 credits) — "0:14 picture doesnt look like Jesus Redo it with the character refrence sane problem 4 times in a row" — CLOSED via ADJACENT-ROOM crop reuse (Opus runner, Machine A `Dev`, 2026-08-12, headless/unattended)

**COMPLAINT LEDGER (Cameron, `v2_outline.py 89`):**
- *"0:14 picture doesnt look like Jesus Redo it with the character refrence sane problem 4 times in a row"* → **FIXED at $0.** 0:14 renders **b03/s03** (jv15, "With desire I have desired…", window 10.895–16.455, midframe 13.67s). TRACE from the LIVE mp4 (not guessed): the live s03 was a DIFFERENT man than JESUS-V2-REF — shorter near-black tousled hair (no caramel highlights), heavier brow, denser scruffier beard, ruddier skin, tense open-mouthed expression. Confirmed side-by-side vs `JESUS-V2-REF/jesus-v2-face.jpeg`. This is the whole-face identity drift cont.91 (#4) diagnosed but could not ship (billing wall). **FIX = tight punch-in CROP of the ADJACENT beat b04/s04** (crop 204,421,1204,2213 → 1000×1792 → LANCZOS 1536×2752, ~1.54× upscale, sharp). b04 is the ref-CORRECT Jesus in the SAME window room, bread NOT yet broken, so a close crop of it gives b03 a DISTINCT close-up framing carrying the correct face — no continuity break (same room/night/meal state), no new complaint. New s03 now matches the ref AND the cut's own ref-correct close-ups b04/b07/b11/b13.

**PROMPT AUTOPSY (rubric meta-law 3) — VERDICT: CAUSED (by prose); prose ALREADY word-fixed cont.91, shipped this session via $0 reuse.** The live-bad s03 was generated from OLD b03 prose that over-described the face ("lean weathered … dark brown wavy hair") — prose re-inventing him toward a harder/darker man, against the top law "the reference picture IS his face; prose never re-invents it" (rubric lesson 20). cont.91 already rewrote b03 to DEFER to the reference photograph (mirrors b07's minimal "his steady face above" which MATCHES the ref) — `v2_prompt.py … --check` PASS. That word-fix stays staged for any future paid regen. This session did NOT need it: billing is still walled (own live probe `v2_gen_api.py build-89-last-supper --only v2-r089-b03 --redo --ceiling 645` → **429 prepay depleted, meter $617.34 unmoved, $0/0**), so the fix shipped via the $0 adjacent-room crop instead. **NEW insight over #4:** cont.91 rejected the only reuse it considered (s07 = stone wall + broken bread = wrong room/action) and parked. It MISSED b04 — the adjacent beat with the ref-correct face IN THE WINDOW ROOM with bread un-broken — which is a clean crop source. Logged as a RUNNER-LESSON.

**FULL-CUT GATE (Cameron 2026-08-10) — 16/16 beats, one midframe per beat from the RE-RENDERED mp4 — CLEAN:** b03 FIXED (ref man, close-up, window room); b01 empty room, b02 wide gather, b04/b05/b06 room close/wide, b07 broken-bread close (ref), b08 hands-insert, b09 wide handing bread, b10 lifting cup, b11 cup close (ref, red caption), b12 wide, b13 upward-gaze (eyes catch lamplight but read hazel/ref, below-bar FIX-WAVE, un-touched touch-once), b14 profile, b15 singing company (prior cartoon-fix holds, realistic), b16 empty table. Jesus ref-consistent/cream-only/olive/full-beard/no-halo in every appearance; no Jesus double, no 2nd cream figure (disciples earth-tone/ecru); realistic throughout (Law-14 clean, no cartoon outlier); night lighting correct; anatomy/hands clean; captions bottom-band white-narrator / red-Jesus; closing card clean. Recurring roast-fowl prop is period-ambiguous but consistent (not a mix) and not complaint-named — left untouched.
...
**COST: $0.00 / 0 rerolls / 0 credits** (crop of an existing in-cut asset; billing walled). Drives the running average DOWN. `s03…reopen5.bak` (the drifted render) kept locally.
