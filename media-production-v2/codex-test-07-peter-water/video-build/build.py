...
is copied into this directory, regenerated, shortened, re-timed, re-encoded, or
otherwise altered.

Same STORY-COVERAGE-capable structure as the existing assembler: a BEATS row may
list several stills for one narration segment, switching at the timestamp where
the marker words are actually spoken.
...
REPO = HERE.parents[2]
V1 = REPO / "media-production" / "build-07-peter-water"
LOCKED_FINAL = V1 / "matthew-14_peter-walks-on-water.mp4"

# Load the authoritative Story 7 declarations in place. Its local helper imports
# must resolve from the V1 directory, never from the shortened V2 test copy.
sys.path.insert(0, str(V1))
_spec = importlib.util.spec_from_file_location(
    "story_07_locked_narration", V1 / "make_narration.py")
make_narration = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(make_narration)
...
def complete_video(path, expected, tolerance=0.15):
    """Allow an interrupted isolated test render to resume safely."""
    if not os.path.exists(path) or os.path.getsize(path) == 0:
        return False
    try:
        return abs(dur_of(path) - expected) <= tolerance
...
def marker_time(seg_id, marker):
    ...
    with open(V1 / "audio" / f"{seg_id}.timing.json") as f:
        timing = json.load(f)
...
def main():
    ...
    spoken = {
        n: spoken_of(str(V1 / "audio" / f"{n}.mp3"))
        for n, _, _ in BEATS
    }
    card_dur = dur_of(str(V1 / "audio" / f"{CARD}.mp3"))
    ...
    for name, src, zdir in BEATS:
        ...
        a_start = t + LEAD
        audio_place.append((str(V1 / "audio" / f"{name}.mp3"), a_start))
        timeline.append((name, src, zdir, vdur, a_start, speaker))
        t += vdur
    card_vdur = LEAD + card_dur + TAIL
    card_start = t
    audio_place.append((str(V1 / "audio" / f"{CARD}.mp3"), card_start + LEAD))
    total = t + card_vdur
    locked_duration = dur_of(str(LOCKED_FINAL))
    # Concat's 30 fps frame rounding makes the calculated visual duration a
    # fraction of a second longer than the muxed reference. Anything beyond half
    # a second indicates a real segment/timing mismatch.
    if abs(total - locked_duration) > 0.50:
        raise SystemExit(
            f"AUDIO LOCK: visual timeline {total:.3f}s does not match locked "
            f"V1 final {locked_duration:.3f}s")
    ...
    for i, (seg_id, src, zdir, vdur, _a, speaker) in enumerate(timeline):
        seg_path = f"{S}/{seg_id}.mp4"
        if complete_video(seg_path, vdur):
            print(f"resume: keeping complete {seg_path}", flush=True)
            continue
        build_still(seg_id, src, vdur, zdir, LEAD + spoken[seg_id],
                    TEXT[seg_id], speaker, first=(i == 0), last=(i == n - 1))
    if complete_video(f"{S}/card.mp4", card_vdur):
        print(f"resume: keeping complete {S}/card.mp4", flush=True)
    else:
    ...
    for crf in (20, 21, 22, 23, 24):
        run([FF, "-y", "-i", f"{S}/video_silent.mp4",
             "-i", str(LOCKED_FINAL), "-map", "0:v", "-map", "1:a:0",
             "-c:v", "libx264", "-preset", "slow", "-crf", str(crf),
             "-maxrate", f"{vcap}k", "-bufsize", f"{vcap*2}k",
             "-pix_fmt", "yuv420p",
             "-c:a", "copy", "-t", f"{locked_duration:.6f}",
