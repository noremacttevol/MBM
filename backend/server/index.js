...
// ── API: Connect — missionary contact request from mobile app ─────────────────
app.post('/api/connect', (req, res) => {
  const { session_id, name, contact, message } = req.body;
  if (!contact) return res.status(400).json({ error: 'contact required' });
  const id = crypto.randomUUID();
  db.prepare(`
    INSERT INTO connect_requests (id, session_id, name, contact, message, created_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(id, session_id || '', name || '', contact, message || '', Date.now());
  res.json({ ok: true, id });
});

app.get('/api/admin/connect_requests', requireAdmin, (req, res) => {
  const rows = db.prepare('SELECT * FROM connect_requests ORDER BY created_at DESC').all();
  res.json(rows);
});
