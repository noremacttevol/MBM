...
| Worker | On | State |
|---|---|---|
| Codex (VS Code, Machine A) | Story 13 (Through the Roof) realistic rebuild | ✅ V3 passed visual, identity, script, Alexander-voice provenance, and MP4 gates; publishing to reviewer as Unwatched with the prior complaint retained |
...
| 02 | The Prodigal Son (Luke 15) | **Realistic cut shipped 2026-08-01 — awaiting Cameron** 🟡 (Machine A `Dev`) — all 24 pictures regenerated at native 2K on the API ($3.75, 28 gens): V2 Jesus face in the two frame-story shots, one father/one son/one brother held by image anchors across every frame, directional light, candid mid-action staging. 4 rerolls caught by QC (ring on the wrong hand, a torch-bearer sneaking into a two-man frame twice — the old rough itself carried him, so b20 dropped its rough — and a dark-haired stranger replacing the father at the open door). AUDIO LOCK PASS (byte-identical to the V1 final, no music bed), verify-mp4 OK 157.9s, no dead air, captions verified on extracted frames. Board card v2 → realistic cut (new hash, returns to Unwatched). | [▶ Prodigal realistic V2](media-production-v2/build-02-prodigal/luke-15_prodigal-son-realistic-v2.mp4) |
...
| 13 | The Man Through the Roof (Mark 2) | **Realistic V3 publishing 2026-08-01 — awaiting Cameron** 🟡 — 45 rebuilt pictures; same paralysed man, same four distinct friends, and locked V2 Jesus across the full story; all six recurring identities passed hash-backed face boards; exactly four carriers and four functional ropes; full roof-to-healing sequence rendered and inspected. The complete 4:59 script is present. Jesus-voice provenance is proven Alexander by exact source hashes, and punctuation pauses were measured. New hash returns the cut to Unwatched while retaining the prior complaint. | [▶ Roof realistic V3](media-production-v2/build-13-roof/mark-2_man-through-the-roof-realistic-v3.mp4) |
