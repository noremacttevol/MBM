...
| Claude worker 9 (Machine A, Dev) | Story 14 (Ten Lepers) full realistic rebuild | in progress — claimed 2026-08-01. Row 14's existing V2 stills are part-1K (resolution audit) and pre-V5; regenerating to the realistic standard at native 2K on the Gemini API, audio LOCKED to the authoritative narration, windows re-timed from the fixed extract_beats. Also fixes Cameron's FIX-LATER complaint (~0:55 lepers read as giants beside Jesus). Ships to Reviewer only. |
