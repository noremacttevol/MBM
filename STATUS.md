...
| Worker | On | State |
|---|---|---|
| Codex (VS Code, Machine A) | Story 01 (Woman Who Touched His Cloak) realistic replacement | in progress — all 20 realistic frames passed composition review; 17 received identity-only repairs, 3 were preserved byte-for-byte, and hash-backed Jesus/woman face boards pass. Repairing only the complained Alexander room noise before assembly; no words or timing may change. |
...
|---|-------|----------|--------------------|
| 01 | Woman Who Touched His Cloak (Mark 5) | **Realistic replacement in progress on Codex** 🔵 — 20-picture V3 visual set now passes the V4 prompt checklist and hash-backed identity boards. Seventeen existing compositions received face-only repairs; `s08`, `s09`, and `s19` remain byte-identical. Next gate is a selective denoise of the two already-confirmed Alexander Jesus takes, preserving every word, pause, and timeline position. App feed remains untouched. | — |
